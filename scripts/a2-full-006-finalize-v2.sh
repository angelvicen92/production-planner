...
if count != 1:
    raise SystemExit(f"Expected exactly one stale itinerant availability assertion, found {count}")
...
if ! grep -Fq "$HEADING" "$README"; then
  cat >> "$README" <<'EOF'
...
if (JSON.stringify(evidence.requiredCreationInputs) !== JSON.stringify(expected)) fail('requiredCreationInputs=' + JSON.stringify(evidence.requiredCreationInputs));
...
for line in lines:
    path = line[3:]
    if path in allowed or path.startswith('.github/') or path in {
        'scripts/a2-full-006-finalize.mjs',
        'scripts/a2-full-006-finalize-v2.sh',
    }:
        continue
