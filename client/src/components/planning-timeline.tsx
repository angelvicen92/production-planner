...
function computeCascadeEdits(params: {
  laneTasks: Task[];
  movedTask: Task;
  targetStart: number;
  startMin: number;
  endMin: number;
  pendingManualEdits: Record<number, { start: string; end: string }>;
}): { nextEdits: Record<number, { start: string; end: string }>; shiftedIds: number[]; clampedStart: number } {
  const { laneTasks, movedTask, targetStart, startMin, endMin, pendingManualEdits } = params;
  const movedEdit = pendingManualEdits[Number(movedTask.id)];
  const currentStart = timeToMinutes(movedEdit?.start ?? movedTask.startPlanned ?? minutesToHHMM(targetStart));
  const currentEnd = timeToMinutes(movedEdit?.end ?? movedTask.endPlanned ?? minutesToHHMM(currentStart + 30));
  const taskDur = Math.max(5, currentEnd - currentStart);
  const clampedStart = Math.max(startMin, Math.min(endMin - taskDur, targetStart));
...
function TaskStatusMenuTrigger({
  ...
  return (
    ...
    >
      ...
      >
          ...
          <div className="p-1">
            {!task.isManualBlock && (!onTaskStatusChange || actions.length === 0) ? (
              <button
                type="button"
              ...
              actions.map((action) => (
                <button
                  ...
                  onClick={() => {
                    handleSelect(action).catch((error) => {
                      toast({
              ...
              <>
                ...
                <button
                  ...
                  disabled={taskStatusPending}
                  onClick={() => {
                    if (!onSetManualBlockDuration) return;
                    const startPlanned = task.startPlanned ?? "";
                    const endPlanned = task.endPlanned ?? "";
                    const startMin = timeToMinutes(startPlanned || "00:00");
                    const endMin = timeToMinutes(endPlanned || "00:00");
                    const currentDuration = Math.max(5, endMin - startMin || 30);
                    const raw = window.prompt("Nueva duración (min)", String(currentDuration));
                    if (!raw) return;
                    const durationMinutes = Math.round(Number(raw));
                    if (!Number.isFinite(durationMinutes) || durationMinutes < 5) {
                      toast({ title: "Duración inválida", description: "Usa un número de al menos 5 minutos.", variant: "destructive" });
                      return;
                    }
                    Promise.resolve(onSetManualBlockDuration(task, durationMinutes))
                      .then(() => setOpen(false))
                      .catch((error) => {
                        toast({
          ...
          <div className="border-t p-1">
            {task.isManualBlock ? null : hasTimeLock ? (
              <button
                type="button"
                className="w-full rounded px-2 py-1 text-left text-sm hover:bg-muted disabled:opacity-50"
                disabled={taskStatusPending}
                onClick={() => {
                  ...
                  onUnpinTask(task).then(() => setOpen(false)).catch((error) => {
                    toast({ title: "No se pudo liberar el lock", description: error instanceof Error ? error.message : "Inténtalo de nuevo.", variant: "destructive" });
                  });
                }}
              >
                Quitar fijación
              ...
              <button
                ...
                onClick={() => {
                  if (!onPinTask) return;
                  onPinTask(task).then(() => setOpen(false)).catch((error) => {
                    toast({ title: "No se pudo fijar el lock", description: error instanceof Error ? error.message : "Inténtalo de nuevo.", variant: "destructive" });
                  });
                }}
              >
                📌 Fijar
  ...
  const [manualBlockEditor, setManualBlockEditor] = useState<null | { task: Task; title: string; color: string; durationMinutes: number }>(null);
  ...
  const handleLanePointerMove = (event: PointerEvent<HTMLDivElement>, laneId: string) => {
    if (!manualDrag || manualDrag.laneId !== String(laneId)) return;
    const rect = event.currentTarget.getBoundingClientRect();
    if (rect.width <= 0) return;
    const x = event.clientX - rect.left - manualDrag.pointerOffsetPx;
  ...
  const applyManualDragDrop = (laneTasks: Task[]) => {
    if (!manualMode || isApplying || !manualDrag) return;
    const selectedTask = laneTasks.find((t) => Number(t.id) === Number(manualDrag.taskId));
  ...
  const handleTaskCardClick = (event: MouseEvent<HTMLButtonElement>, task: Task) => {
    if (viewMode === "contestants" && taskSortArmed) {
      event.preventDefault();
      event.stopPropagation();
      setContestantSort({
        mode: "task",
                      ...
                      {spaceVerticalMode === "timeline" || spaceVerticalMode === "list" ? (
                        ...
                        <div className="flex gap-4" >
                          ...
                          <div className="flex-1 overflow-x-auto">
                            ...
                            onSetManualBlockDuration={onSetManualBlockDuration}
                            laneId={String(sp.id)}
            ...
            <div className="min-w-[800px] relative pb-4" data-planning-zoom-target>
              ...
              <div className="mb-3 flex flex-wrap items-center gap-3">
                <div className="flex items-center gap-2">
                  <Switch
                    ...
                    onCheckedChange={(v) => {
                      ...
                      setManualMode(next);
                      if (!next) { setManualDrag(null); setManualDragPreviewStart(null); }
                    }}
                    disabled={isApplying}
                  />
                ...
                {viewMode === "contestants" ? (
                  <div className="flex items-center gap-2 rounded-md border px-3 py-1.5 bg-muted/20">
                    <Button
                      size="sm"
                      variant={taskSortArmed ? "default" : "outline"}
                      onClick={() => setTaskSortArmed((prev) => !prev)}
                    >
                      {taskSortArmed ? "Click en una tarea…" : "Orden por tarea"}
                    </Button>
                    {taskSortArmed ? (
                      <span className="text-xs text-muted-foreground">Click en una tarea…</span>
                    ) : null}
                    <span className="text-xs">
                      Orden actual: {contestantSort.mode === "name" ? "Nombre" : `Tarea → ${contestantSort.templateName}`}
                    </span>
                    {contestantSort.mode === "task" ? (
                      <Button size="sm" variant="ghost" onClick={() => { setContestantSort({ mode: "name" }); setTaskSortArmed(false); }}>Ordenar por nombre</Button>
                    ) : null}
                  </div>
                ) : null}
                {manualMode ? (
                  <div className="flex items-center gap-2 rounded-md border px-3 py-1.5 bg-muted/40">
                    ...
                    {manualDrag !== null ? (
                      <div className="rounded border bg-background px-2 py-1 text-xs">
                        Moviendo: {taskById.get(Number(manualDrag.taskId))?.template?.name ?? `#${manualDrag.taskId}`}
                        <Button size="sm" variant="ghost" className="ml-2 h-6 px-2" onClick={() => { setManualDrag(null); setManualDragPreviewStart(null); }}>Cancelar</Button>
                      </div>
                    ) : null}
                  </div>
                ) : null}
              ...
              {validationResult && !validationResult.feasible ? (
                <Card className="mb-3 p-3 border-red-300/50 bg-red-50/40">
                  ...
                  <div className="mt-2 flex gap-2">
                    <Button size="sm" variant="outline" onClick={() => { discardManualEditsAndReload().catch((error) => { toast({ title: "No se pudieron descartar los cambios", description: error instanceof Error ? error.message : "Inténtalo de nuevo.", variant: "destructive" }); }); }}>
                      Descartar cambios
                    </Button>
              ...
              <Dialog open={Boolean(manualBlockEditor)} onOpenChange={(open) => { if (!open && !manualBlockEditorBusy) setManualBlockEditor(null); }}>
                <DialogContent className="max-w-md">
                  <DialogHeader>
                    ...
                    <DialogDescription>Ajusta título, color, duración o elimina el bloqueo desde aquí.</DialogDescription>
                  </DialogHeader>
                  <div className="space-y-3">
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        min={5}
                        step={1}
                        value={String(manualBlockEditor?.durationMinutes ?? 30)}
                        disabled={manualBlockEditorBusy}
                        onChange={(e) => {
                          const next = Math.max(5, Math.round(Number(e.target.value) || 30));
                          setManualBlockEditor((prev) => prev ? { ...prev, durationMinutes: next } : prev);
                        }}
                      />
                    </div>
                  </div>
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          toast({ title: "No se pudo eliminar el bloqueo", description: error instanceof Error ? error.message : "Inténtalo de nuevo.", variant: "destructive" });
                        } finally {
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          if (onSetManualBlockDuration) {
                            await onSetManualBlockDuration(manualBlockEditor.task, manualBlockEditor.durationMinutes);
                          }
                          setManualBlockEditor(null);
                        } catch (error) {
                          toast({ title: "No se pudo guardar el bloqueo", description: error instanceof Error ? error.message : "Inténtalo de nuevo.", variant: "destructive" });
                        } finally {
                          setManualBlockEditorBusy(false);
                        }
                      }}
                    ...
                    <div className="flex gap-2">
                      <Button
                        ...
                        onClick={() => {
                          if (isPostApplyWorking) return;
                          setIsPostApplyWorking(true);
                          discardManualEditsAndReload().catch((error) => {
                              toast({ title: "No se pudieron descartar los cambios", description: error instanceof Error ? error.message : "Inténtalo de nuevo.", variant: "destructive" });
                            }).finally(() => setIsPostApplyWorking(false));
                        }}
                      >
                        Descartar cambios
                      </Button>
                ...
                >
                  ...
                  <div
                    ...
                    onPointerUp={() => {
                      ...
                      applyManualDragDrop(lane.tasks);
                    }}
                  >
                    {/* Grid 5 min (like PDF) */}
                      ...
                      return (
                        <TaskStatusMenuTrigger key={task.id}
                            ...
                            onPointerDown={(event) => {
                              ...
                              const currentEnd = timeToMinutes(edit?.end ?? task.endPlanned ?? minutesToHHMM(currentStart + 30));
                              const durationMinutes = Math.max(5, currentEnd - currentStart);
                              const pointerOffsetPx = event.clientX - cardRect.left;
                              setManualDrag({
                                taskId: Number(task.id),
                                laneId: String(id),
                                startAtMinutes: currentStart,
                                durationMinutes,
                                pointerOffsetPx,
                              });
                              setManualDragPreviewStart(currentStart);
                            ...
                            onClick={(event) => {
                              ...
                              if (manualMode && task.isManualBlock) {
                                ...
                                setManualBlockEditor({
                                  ...
                                  durationMinutes: Math.max(5, timeToMinutes(task.endPlanned ?? "00:30") - timeToMinutes(task.startPlanned ?? "00:00") || 30),
                                });
                                return;
                              }
