...
function formatDayValue(capabilityId:string,value:unknown){const item=value as {start?:string;end?:string;mode?:string;window?:{start?:string;end?:string}};if(capabilityId==="WORKDAY_WINDOW")return `${item.start ?? "—"}–${item.end ?? "—"}`;if(capabilityId==="GLOBAL_MEAL_BREAK"){const window=item.window??item;return `${window.start ?? "—"}–${window.end ?? "—"} · ${item.mode==="flexible_meal_window"?"Ventana flexible":"Parada global"}`;}return null;}
