...
export function EffectiveValueField({value}:{value:EffectiveConfigurationValue}){const needsAttention=value.blockers.length>0||value.implementationStatus!=="PRODUCTIVE";return <div className="rounded-lg border p-3 space-y-2" data-testid={`effective-${value.capabilityId}`}><div className="flex flex-wrap items-center gap-2"><h4 className="font-medium">{value.label}</h4><CapabilityStatus value={value}/><SourceBadge source={value.source}/><SeverityBadge severity={value.severity}/></div><p className="text-sm">{valueSummary(value)}</p>{needsAttention?<p className="text-xs text-amber-700">Esta opción requiere revisión antes de poder aplicarse con seguridad.</p>:null}{value.fingerprint?<p className="text-xs text-muted-foreground">Huella efectiva disponible</p>:null}</div>}
