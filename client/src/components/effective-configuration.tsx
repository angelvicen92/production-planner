...
export const sourceLabels = {DAY_SNAPSHOT:"Snapshot del día",MIXED:"Fuentes diarias combinadas",PROTECTED:"Estado protegido",UNKNOWN:"Origen no demostrado"} as const;
...
export function CapabilityStatus({value}:{value:EffectiveConfigurationValue}){
  const label=value.implementationStatus==="MISSING"?"Capacidad aún no soportada":value.implementationStatus==="PARTIAL"||value.implementationStatus==="BLOCKED"?"Soporte parcial de producto":value.availability==="UNKNOWN"?"Valor desconocido":value.validationStatus==="INCOMPATIBLE"?"Incompatible":value.validationStatus==="INCOMPLETE"?"Falta configurar hoy":"Disponible";
...
export function EffectiveValueField({value}:{value:EffectiveConfigurationValue}){return <div className="rounded-lg border p-3 space-y-2" data-testid={`effective-${value.capabilityId}`}><div className="flex flex-wrap items-center gap-2"><h4 className="font-medium">{value.label}</h4><CapabilityStatus value={value}/><SourceBadge source={value.source}/><Badge variant="outline">{value.implementationStatus}</Badge><SeverityBadge severity={value.severity}/></div><p className="text-sm">{valueSummary(value)}</p>{value.blockers.length?<ul className="list-disc pl-5 text-xs text-amber-700">{value.blockers.map(blocker=><li key={blocker}>{blocker}</li>)}</ul>:null}{value.fingerprint?<p className="text-xs text-muted-foreground">Huella efectiva disponible</p>:null}</div>}
