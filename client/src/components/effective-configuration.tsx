...
export function EffectiveValueField({value,onRestore,restoring}:{value:EffectiveConfigurationValue,onRestore?:()=>void,restoring?:boolean}){const needsAttention=value.blockers.length>0||value.implementationStatus!=="PRODUCTIVE";return <div className="rounded-lg border p-3 space-y-2" data-testid={`effective-${value.capabilityId}`}><div className="flex flex-wrap items-center gap-2"><h4 className="font-medium">{value.label}</h4><CapabilityStatus value={value}/><SourceBadge source={value.source}/><SeverityBadge severity={value.severity}/></div><p className="text-sm">{valueSummary(value)}</p>{value.baseline?<p className="text-xs text-muted-foreground">Valor heredado: {JSON.stringify(value.baseline)}</p>:null}{value.defaultGeneral&&JSON.stringify(value.defaultGeneral)!==JSON.stringify(value.baseline)?<p className="text-xs text-muted-foreground">La configuración habitual actual ha cambiado: {JSON.stringify(value.defaultGeneral)}</p>:null}{value.canRestoreInherited?<Button size="sm" variant="outline" disabled={restoring} onClick={onRestore}>Restaurar heredado</Button>:null}{needsAttention?<p className="text-xs text-amber-700">Esta opción requiere revisión antes de poder aplicarse con seguridad.</p>:null}{value.fingerprint?<p className="text-xs text-muted-foreground">Revisión {value.effectiveRevision ?? "—"} · huella {value.fingerprint.slice(0,8)}</p>:null}</div>}
