...
export function AssistedPlanningWorkspace({planId,plan,timelineProps,spaces=[]}:{planId:number;plan:any;timelineProps:Record<string,unknown>;spaces?:any[]}) {
  ...
  return <div className="space-y-3" data-testid="assisted-workspace">
    ...
    {configPreview?<><p className="text-sm">Revisa y elige qué cambios incorporar. Las etapas aceptadas y sus decisiones no se modificarán.</p>{configPreview.changes.length===0?<p className="text-sm text-muted-foreground">La configuración del día ya está actualizada.</p>:<div className="space-y-1">{configPreview.changes.map((change:any)=><label key={change.key} className="flex items-start gap-2 rounded border p-2 text-sm"><Checkbox checked={configSelection.includes(change.key)} onCheckedChange={value=>setConfigSelection(old=>value?[...old,change.key]:old.filter(key=>key!==change.key))}/><span><b>{change.label}</b> · {change.kind==="NEW"?"Nuevo":change.kind==="REMOVED"?"Eliminado":"Modificado"}{change.localOverride?<Badge variant="outline" className="ml-2">Ajuste local del día</Badge>:null}</span></label>)}</div>}<div className="flex gap-2"><Button size="sm" disabled={!!busy||configSelection.length===0} onClick={applyConfig}>Aplicar selección</Button><Button size="sm" variant="outline" onClick={()=>setConfigSelection(configPreview.changes.map((x:any)=>x.key))}>Seleccionar todo</Button><Button size="sm" variant="ghost" onClick={()=>setConfigPreview(null)}>Cerrar</Button></div></>:null}</Card>
