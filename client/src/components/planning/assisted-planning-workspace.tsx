...
export function AssistedPlanningWorkspace({planId,plan,timelineProps,spaces=[]}:{planId:number;plan:any;timelineProps:Record<string,unknown>;spaces?:any[]}) {
  ...
  const [configPreview,setConfigPreview]=useState<any>(null);const [configSelection,setConfigSelection]=useState<string[]>([]);
  ...
  const previewConfig=async()=>{setBusy("config-preview");setMessage(null);try{const result=await apiRequest<any>("GET",`/api/plans/${planId}/assisted/config-refresh`);setConfigPreview(result);setConfigSelection(result.changes.map((x:any)=>x.key));}catch(e:any){setMessage(operatorError(e));}finally{setBusy(null);}};
  const applyConfig=()=>mutate("config-apply",()=>apiRequest("POST",`/api/plans/${planId}/assisted/config-refresh`,{expectedConfigRevisionId:configPreview.expectedConfigRevisionId,selectedChangeKeys:configSelection})).then(()=>{setConfigPreview(null);setConfigSelection([]);});
  ...
  return <div className="space-y-3" data-testid="assisted-workspace">
    <Card className="p-3 space-y-2" data-testid="assisted-config-refresh"><div className="flex items-center gap-2"><strong>Configuración del día</strong><Button size="sm" variant="outline" disabled={!!busy} onClick={previewConfig}>Actualizar desde configuración general</Button></div>
    {configPreview?<><p className="text-sm">Revisa y elige qué cambios compatibles incorporar. Las etapas aceptadas y sus decisiones no se modificarán.</p>{configPreview.localOverrides?.length>0?<div className="rounded border p-2 text-sm"><b>Ajustes locales existentes en el día:</b><ul className="list-disc pl-5">{configPreview.localOverrides.map((item:any)=><li key={item.key}>{item.label} <Badge variant="outline">Ajuste local del día</Badge></li>)}</ul></div>:null}{configPreview.changes.length===0?<p className="text-sm text-muted-foreground">{configPreview.unsupportedAuthorities.length===0?"La configuración del día ya está actualizada.":"No hay cambios en las categorías compatibles."}</p>:<div className="space-y-1">{configPreview.changes.map((change:any)=><label key={change.key} className="flex items-start gap-2 rounded border p-2 text-sm"><Checkbox checked={configSelection.includes(change.key)} onCheckedChange={value=>setConfigSelection(old=>value?[...old,change.key]:old.filter(key=>key!==change.key))}/><span><b>{change.label}</b> · {change.kind==="NEW"?"Nuevo":change.kind==="REMOVED"?"Eliminado":"Modificado"}{change.localOverride?<Badge variant="outline" className="ml-2">Ajuste local del día</Badge>:null}</span></label>)}</div>}{configPreview.unsupportedAuthorities.length>0?<div className="rounded border border-amber-300 bg-amber-50 p-2 text-sm"><b>Categorías aún no comparables:</b><ul className="list-disc pl-5">{configPreview.unsupportedAuthorities.map((item:any)=><li key={item.authority}>{item.label}</li>)}</ul></div>:null}<div className="flex gap-2"><Button size="sm" disabled={!!busy||configSelection.length===0} onClick={applyConfig}>Aplicar selección</Button><Button size="sm" variant="outline" disabled={configPreview.changes.length===0} onClick={()=>setConfigSelection(configPreview.changes.map((x:any)=>x.key))}>Seleccionar todos los cambios compatibles</Button><Button size="sm" variant="ghost" onClick={()=>setConfigPreview(null)}>Cerrar</Button></div></>:null}</Card>
