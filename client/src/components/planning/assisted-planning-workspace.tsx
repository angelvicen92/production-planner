...
export function AssistedPlanningWorkspace({planId,plan,timelineProps,spaces=[]}:{planId:number;plan:any;timelineProps:Record<string,unknown>;spaces?:any[]}) {
  ...
  const toolbar=<div className="flex flex-wrap items-center gap-1 text-xs"><Badge>S{active?.ordinal??0}</Badge><Badge variant={modified?"destructive":"secondary"}>{modified?"MODIFIED":"CLEAN"}</Badge><Badge variant={validationCurrent?"default":"outline"}>{validationCurrent?"VALID":state.session?.draftValidationId?"STALE":"NOT VALIDATED"}</Badge><Badge variant="outline">config {state.currentConfigRevisionId}</Badge>{runId?<Badge variant="outline">run {runId}</Badge>:null}<Button size="sm" variant="outline" disabled={!!busy} onClick={()=>mutate("validate",()=>apiRequest("POST",`/api/plans/${planId}/assisted/validate`,guard()))}>Validar Draft</Button><Button size="sm" disabled={!!busy||!validationCurrent} onClick={()=>mutate("accept",()=>apiRequest("POST",`/api/plans/${planId}/assisted/accept-stage`,guard()),true)}>Aceptar Stage</Button></div>;
  return <div className="space-y-3" data-testid="assisted-workspace">
    ...
    <FullscreenPlanningPanel title="Planning asistido" viewKey={`assisted-${planId}`} supportsZoom toolbarRight={toolbar}><PlanningTimeline {...timelineProps as any} plan={assistedPlan} mode="assisted-draft" taskVisualStates={visual}/></FullscreenPlanningPanel>
  </div>;
}
