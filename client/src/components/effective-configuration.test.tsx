...
test("reusable configuration components expose Spanish text, not internal codes",()=>{
  assert.equal(sourceLabels.DAY_SNAPSHOT,"Snapshot del día");
  ...
  for(const text of ["Snapshot del día","Obligatorio","Capacidad aún no soportada","Esta capacidad aún no tiene un valor productivo","Revisa sólo las excepciones del día"])assert.match(html,new RegExp(text));
