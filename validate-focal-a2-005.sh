...
artifact="planner-next-focal-a2-band-required-audit-v1.json"
legacy="planner-next-focal-a2-band-preferred-v2.json"
failed="planner-next-focal-a2-band-required-audit-v1.failed.json"
...
fs.writeFileSync(path,JSON.stringify({version:"planner-next-focal-a2-band-required-audit-v1",accepted:false,reason},null,2)+"\n");
NODE
  exit 1
}
...
npx tsx --test engine/planner-next/resourcePresence.spec.ts engine/planner-next/requiredContinuousResource.spec.ts || fail REQUIRED_TESTS_FAILED
...
expect(a.version==="planner-next-focal-a2-band-required-audit-v1"&&a.status==="BAND_REQUIRED_POLICY_ACCEPTED_FOCAL_REQUIRED_INFEASIBLE","IDENTITY");
...
expect(r.fingerprint===manifest.frozenCurrentRequiredFailure.fingerprint&&r.branches===4541&&r.branchBudgetConsumed===4541,"REQUIRED_FROZEN");
...
NODE
  ...
  node -e 'let x=require("./planner-next-focal-a2-band-required-audit-v1.json");if(!x.acceptance.accepted||Object.keys(x.scenarios).length!==24)process.exit(1)' || fail PUBLISHED_ARTIFACT_INVALID
  rm -f "$legacy"
fi
