...
export function spaceMealCandidateStarts(problem:PlannerNextProblem,spaceId:string,tasks:ScheduledTask[],meals:ScheduledSpaceMeal[]):number[]{const p=spaceMealPolicy(problem,spaceId),s=problem.spaces.find(x=>x.id===spaceId);if(!p||!s)return [];const out:number[]=[];for(let start=p.window.start;start+p.duration<=p.window.end;start+=5){const m=createScheduledSpaceMeal(spaceId,start,p.duration);if(spaceMealWithinDay(problem,m)&&spaceMealWithinAvailability(s,m)&&spaceMealAvoidsTasks(m,tasks)&&spaceMealAvoidsMeals(m,meals))out.push(start)}return out.sort((a,b)=>a-b)}
