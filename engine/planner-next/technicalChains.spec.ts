...
test("explicit REQUIRED policies preserve productive task authorities",()=>{const p=technicalChainScenario(),technical=getTechnicalChains(p.tasks)[0]!;
 p.resourceTransitionMinutes=0;for(const resource of p.resources)resource.transitionMinutes=0;
 for(const space of p.spaces){space.availability=[{start:0,end:1000}];space.secondaryContinuity=undefined;space.setupPolicy=undefined;}
 p.participants=[{id:"participant-a",availability:[{start:0,end:1000}]},{id:"participant-b",availability:[{start:0,end:1000}]}];
 p.coaches=[{id:"coach-a",availability:[{start:0,end:1000}]}];
 p.resources.push({id:"coach-a",availability:[{start:0,end:1000}],presencePreference:"OFF"});
 p.tasks=technical.map((task,index)=>({...task,kind:"auxiliary" as const,participantId:`participant-${index?"b":"a"}`,coachId:index?undefined:"coach-a",requiredResourceIds:["technical-chain-unit",...(index?[ ]:["coach-a"])]}));
 p.technicalChains=[{id:"productive-chain",orderedTaskIds:p.tasks.map(task=>task.id),adjacency:"REQUIRED",resourceContinuity:"REQUIRED",requiredResourceIds:["technical-chain-unit"]}];
 const chain=getTechnicalChains(p.tasks,p.technicalChains);assert.deepEqual(chain.map(tasks=>tasks.map(task=>task.id)),[p.technicalChains[0]!.orderedTaskIds]);
 const candidate=generateTechnicalChainCandidates(p,chain[0]!,[],10_000,"PROBE",1).candidates[0]!;
 assert.ok(candidate);assert.equal(candidate.tasks[0]!.participantId,"participant-a");assert.equal(candidate.tasks[0]!.coachId,"coach-a");assert.deepEqual(candidate.tasks[0]!.requiredResourceIds,["technical-chain-unit","coach-a"]);
 const validation=validatePlan(p,candidate.tasks);assert.equal(validation.hardValid,true,JSON.stringify(validation));
});
