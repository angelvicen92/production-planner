...
test("explicit phases preserve REQUIRED continuity while allowing internal permutations",()=>{const p=technicalChainScenario(),chain=getTechnicalChains(p.tasks)[0]!;
 p.searchPolicy="EXACT_CONSTRUCTIVE";
 p.day={start:0,end:100};for(const space of p.spaces)space.availability=[{start:0,end:100}];for(const resource of p.resources){resource.availability=[{start:0,end:100}];resource.transitionMinutes=0;}for(const task of chain){task.duration=10;task.availability=[{start:0,end:100}];}
 const extra={...chain[0]!,id:"phase-peer",dependencies:[]};p.tasks.push(extra);
 p.technicalChains=[{id:"chain",orderedTaskIds:[chain[0]!.id,extra.id,chain[1]!.id],phases:[{taskIds:[chain[0]!.id,extra.id]},{taskIds:[chain[1]!.id]}],adjacency:"REQUIRED",resourceContinuity:"REQUIRED",requiredResourceIds:["technical-chain-unit"]}];
 const members=getTechnicalChains(p.tasks,p.technicalChains)[0]!,result=generateTechnicalChainCandidates(p,members,[],50_000,"PROBE",10_000);
 assert.deepEqual(new Set(result.candidates.map(candidate=>candidate.tasks[0]!.id)),new Set([chain[0]!.id,extra.id]));
 assert.ok(result.candidates.every(candidate=>candidate.tasks.every((task,index)=>index===0||candidate.tasks[index-1]!.end===task.start)));
 assert.ok(result.candidates.every(candidate=>validatePlan({...p,tasks:members},candidate.tasks).technicalChainViolationCount===0));
});
