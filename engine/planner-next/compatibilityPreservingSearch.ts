...
function emptyMetrics(
  ...
  return {
    ...
    participantMealViolationCount: 0,
    resourceMealViolationCount: 0,
    itinerantUnitMealViolationCount: 0,
...
export function planCompatibilityPreserving(problem: PlannerNextProblem): PlanResult {
  ...
  for (let index = 0; index < retained.length; index += 1) {
    ...
    const metrics: PlanMetrics = {
      ...
      jointGroupParticipantIdsById:Object.fromEntries([...new Set(problem.tasks.map(t=>t.jointGroupId).filter((x):x is string=>Boolean(x)))].sort().map(id=>[id,problem.tasks.filter(t=>t.jointGroupId===id).map(t=>t.participantId).filter((participantId): participantId is string => participantId !== undefined).sort()])),
      ...
      technicalChainCount:getTechnicalChains(problem.tasks,problem.technicalChains).length,
      technicalChainPlannedCount:getTechnicalChains(problem.tasks,problem.technicalChains).filter(chain=>chain.every(t=>ordered.some(x=>x.id===t.id))).length,
      technicalChainScheduledTaskCount:getTechnicalChains(problem.tasks,problem.technicalChains).flat().filter(t=>ordered.some(x=>x.id===t.id)).length,
      technicalChainCandidateCountWhenSelectedByRootId:auxiliary?.technicalChainCandidateCounts??{},
      technicalChainTaskIdsByRootId:Object.fromEntries(getTechnicalChains(problem.tasks,problem.technicalChains).map(c=>[c[0]!.id,c.map(t=>t.id)])),
      technicalChainStartByRootId:Object.fromEntries(getTechnicalChains(problem.tasks,problem.technicalChains).map(c=>[c[0]!.id,ordered.find(t=>t.id===c[0]!.id)?.start??null])),
      technicalChainEndByRootId:Object.fromEntries(getTechnicalChains(problem.tasks,problem.technicalChains).map(c=>[c[0]!.id,ordered.find(t=>t.id===c.at(-1)!.id)?.end??null])),
