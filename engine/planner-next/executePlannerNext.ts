...
function withParticipantMeals<T extends { complete: boolean; scheduledTasks: ScheduledTask[]; scheduledSpaceMeals: unknown[]; scheduledParticipantMeals?: ScheduledParticipantMeal[]; scheduledResourceMeals?:ScheduledResourceMeal[];scheduledItinerantUnitMeals?:ScheduledItinerantUnitMeal[] }>(problem: PlannerNextProblem, result: T): T & { scheduledParticipantMeals: ScheduledParticipantMeal[];scheduledResourceMeals:ScheduledResourceMeal[];scheduledItinerantUnitMeals:ScheduledItinerantUnitMeal[] } {
  if (!result.complete) return { ...result, scheduledTasks: [], ...( "scheduledSetupPreparations" in result ? { scheduledSetupPreparations: [] } : {}), ...( "scheduledRoundPreparations" in result ? { scheduledRoundPreparations: [] } : {}), scheduledSpaceMeals: [], scheduledParticipantMeals: [],scheduledResourceMeals:[],scheduledItinerantUnitMeals:[] };
...
export function executePlannerNext(problem: PlannerNextProblem, options:{causalDiagnostic?:boolean;acceptsValidation?:(validation:import("./contracts").ValidationSummary)=>boolean;fixedPlacements?:readonly ScheduledTask[];fixedPlacementsAsContext?:boolean;assistedPrerequisiteOrdering?:boolean}={}): PlannerNextExecution {
  ...
  return {
    kind: "EXACT_CONSTRUCTIVE",
    policyResolution,
      result: withParticipantMeals(problem, constructExactItinerantPlan(problem,options.causalDiagnostic,options.acceptsValidation,options.fixedPlacements,options.fixedPlacementsAsContext,options.assistedPrerequisiteOrdering)),
  };
}
