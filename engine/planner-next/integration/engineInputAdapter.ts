...
function canonicalProblem(problem: PlannerNextProblem): unknown {
  ...
  return {
    ...
    ...(problem.technicalChains ? { technicalChains: sorted(problem.technicalChains,(entry)=>entry.id).map(entry=>({...entry,orderedTaskIds:[...entry.orderedTaskIds],...(entry.phases?{phases:entry.phases.map(phase=>[...phase])}:{}),requiredResourceIds:[...entry.requiredResourceIds].sort(compare)})) } : {}),
...
export function adaptEngineInputToPlannerNextProblem(input: EngineInput): EngineInputAdapterResult {
  ...
  const technicalChains:TechnicalChainPolicy[]=sourceTechnicalChains.map(policy=>({
    ...
    ...(policy.phases?{phases:policy.phases.map(phase=>phase.map(id=>canonical("task",id)))}:{}),
