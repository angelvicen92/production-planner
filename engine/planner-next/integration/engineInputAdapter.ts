...
export function adaptEngineInputToPlannerNextProblem(input: EngineInput): EngineInputAdapterResult {
  ...
  const problem: PlannerNextProblem = {
    ...
    ...(operationalMealPolicies.length ? { operationalMealPolicies: operationalMealPolicies.map((meal) => ({ id: canonical("break", meal.id), window: { ...meal.window }, duration: meal.duration, resourceIds: meal.resourceIds.map((id) => canonical("plan-resource", id)), spaceIds: meal.spaceIds.map((id) => canonical("space", id)), ...(meal.futureReservation ? { futureReservation: meal.futureReservation } : {}) })) } : {}),
