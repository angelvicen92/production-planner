...
export type ResourceMealDefect = "INVALID_ID" | "INVALID_RESOURCE" | "INVALID_TIME" | "MIXED_SCOPE" | "MISSING_RESOURCE" | "OUTSIDE_AVAILABILITY" | "OVERLAP";
...
export function resolveAssignedResourceMealBreaks(input: EngineInput): AssignedResourceMealsResolution {
  ...
  const meals=input.tasks.filter(task=>task.breakKind==="resource_meal"&&task.status!=="cancelled").map(task=>{
    ...
    const resourceIds=[...new Set((task.assignedResourceIds??[]).filter(id=>Number.isInteger(id)&&id>0))].sort((a,b)=>a-b);
    ...
    if(fixed.status!=="EXACT")defects.push("INVALID_TIME"); else {interval={...fixed.interval};minuteInterval={start:engineTimeToMinute(interval.start),end:engineTimeToMinute(interval.end)};}
