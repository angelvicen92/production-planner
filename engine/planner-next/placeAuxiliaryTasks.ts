...
export function placeAuxiliaryTasks(problem: PlannerNextProblem, initial: ScheduledTask[], branchAllowance: number, initialMeals: ScheduledSpaceMeal[] = []): AuxiliaryPlacementResult {
  ...
  let beam: State[] = [{ placed: initial, preparations: [], meals:[...initialMeals], pending: problem.tasks.filter((x) => (x.kind === "auxiliary" || x.kind === "technical")&&!initiallyPlaced.has(x.id)), order: [], workOrder: [], counts: {}, mealCounts:{}, jointCounts:{}, chainCounts:{}, blockCounts: {}, cost: 0, futureMin: 0, futureTotal: 0, pathMin: Number.POSITIVE_INFINITY }];
  ...
  while (beam.length && (beam[0]!.pending.length || pendingSpaceMealIds(problem,beam[0]!.meals).length)) {
    ...
    for (const state of beam) {
      ...
      const individual = state.pending.filter((task) => !chainIds.has(task.id) && !required.has(task.spaceId) && task.jointGroupId===undefined).map((task) => ({ kind: "task" as const, key: `task:${task.id}`, duration: task.duration, resources: task.requiredResourceIds?.length ?? 0, task, starts: startsFor(problem, task, state.placed,state.meals) }));
      ...
      const choices = [...individual, ...joints, ...blocks, ...chains,...mealChoices].sort((a, b) => alternatives(a) - alternatives(b) || b.duration - a.duration || b.resources - a.resources || a.key.localeCompare(b.key));
