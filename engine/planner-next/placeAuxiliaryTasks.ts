...
export function placeAuxiliaryTasks(problem: PlannerNextProblem, initial: ScheduledTask[], branchAllowance: number, initialMeals: ScheduledSpaceMeal[] = []): AuxiliaryPlacementResult {
  ...
  while (beam.length && (beam[0]!.pending.length || pendingSpaceMealIds(problem,beam[0]!.meals).length)) {
    ...
    for (const state of beam) {
      ...
      const saturated=saturatedBlocks.map(block=>{const generated=constructSaturatedResourceWindowBlockCandidates(problem,block,state.placed,branchAllowance-branches,state.meals);branches+=generated.branchesExplored;saturatedBlockBranches+=generated.branchesExplored;return {kind:"saturated" as const,key:block.key,duration:block.window.end-block.window.start,resources:block.resourceIds.length,block,candidates:generated.candidates.map(candidate=>({tasks:candidate.tasks,preparations:[],meals:[],cost:scoreAuxiliaryTaskSequence(problem,candidate.tasks,state.placed),order:candidate.order})),alternativeCount:generated.candidates.length,exhausted:generated.exhausted};});
