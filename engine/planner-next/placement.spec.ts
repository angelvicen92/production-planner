...
test("prepared authority subsumes every fixed hard-placement authority on a discontinuous grid",()=>{
 const availability=[{start:2,end:42},{start:47,end:122}];
 const task:Task={id:"feeder",kind:"vocal",participantId:"participant",coachId:"coach",itinerantUnitId:"unit",
  duration:10,spaceId:"space",requiredResourceIds:["resource"],dependencies:["dependency"],availability};
 const problem:PlannerNextProblem={day:{start:2,end:122},protectedMeal:{start:97,end:102},
  spaces:[{id:"space",availability},{id:"other",availability},{id:"resource-meal-space",availability}],
  participants:[{id:"participant",availability}],coaches:[{id:"coach",availability}],
  resources:[{id:"resource",availability,presencePreference:"OFF",assignedSpaceId:"resource-meal-space",transitionMinutes:5}],
  itinerantUnits:[{id:"unit",availability}],itinerantUnitMeals:[{itinerantUnitId:"unit",interval:{start:82,end:87}}],
  tasks:[task],mainFlow:{spaceId:"space",preferredEnd:122,continuity:"REQUIRED",maxBlocksByKey:1,minTasksPerBlock:1},
 ...
 const placed:ScheduledTask[]=[
  {...task,id:"dependency",dependencies:[],start:2,end:12},
  {...task,id:"coach-blocker",participantId:"other-participant",requiredResourceIds:[],itinerantUnitId:undefined,
    dependencies:[],spaceId:"other",start:32,end:42},
  {...task,id:"participant-blocker",coachId:undefined,requiredResourceIds:[],itinerantUnitId:undefined,
    dependencies:[],spaceId:"other",start:57,end:67},
  {...task,id:"resource-blocker",participantId:"other-participant",coachId:undefined,itinerantUnitId:undefined,
    dependencies:[],spaceId:"other",start:72,end:77},
  {...task,id:"dependent",dependencies:[task.id],start:112,end:122},
 ];
 const meals=[{id:"meal",kind:"space-meal" as const,spaceId:"resource-meal-space",entryIndex:0,duration:5,start:22,end:27}];
 const prepared=prepareTaskPlacementAuthority(problem,task,placed,meals);
 assert.deepEqual([...prepared.baseDomain.starts()],[...exactTaskStartDomain(problem,task,placed,meals).starts()]);
 for(let start=problem.day.start;start+task.duration<=problem.day.end;start+=5)
  assert.equal(prepared.accepts(start,prepared.baseDomain),canPlaceTask(problem,task,start,placed,meals),`start ${start}`);
 assert.ok(prepared.baseDomain.intervals.length>1,"discontinuous authorities must remain non-convex");
});
