...
import { itinerantUnitMealAnchoredScenario, itinerantUnitMealScenario } from "../scenarios/itinerantUnitMealScenario";
...
export function runScopedMealProbe(scope: "participant" | "resource" | "itinerant-unit", options: ScopedMealProbeOptions = {}): CapabilityProbeResult {
  ...
  if(scope==="itinerant-unit"&&firstAdapter.status==="SUPPORTED"){
    const obligation=firstAdapter.problem.itinerantUnitMeals?.[0],runs=["COMPATIBILITY_PRESERVING","EXACT_CONSTRUCTIVE"].map(policy=>{const problem=itinerantUnitMealScenario(policy as "COMPATIBILITY_PRESERVING"|"EXACT_CONSTRUCTIVE"),task=problem.tasks.find(x=>x.id==="flexible-productive")!,result=executePlannerNext(problem).result!,validation=validatePlan(problem,result.scheduledTasks,"scheduledSetupPreparations" in result?result.scheduledSetupPreparations:[],result.scheduledSpaceMeals,result.scheduledParticipantMeals,result.scheduledResourceMeals,result.scheduledItinerantUnitMeals);return {policy,complete:result.complete,hardValid:validation.hardValid,mealCount:result.scheduledItinerantUnitMeals.length,overlapRejected:!canPlaceTask(problem,task,705,[]),alternative:result.scheduledTasks.find(x=>x.id===task.id)?.start,unit8:result.scheduledTasks.some(x=>x.id==="other-during-meal"&&x.start<780&&720<x.end),productive:result.scheduledTasks.filter(x=>x.itinerantUnitId==="itinerant-team:7").length,noAlias:!problem.resources.some(x=>x.id==="itinerant-team:7")};}),anchored=itinerantUnitMealAnchoredScenario(),contract=anchored.anchoredAccompaniments![0]!,anchor=anchored.tasks.find(x=>x.id===contract.anchorTaskId)!,atomic=materializeAnchoredOperation(anchored,anchor,735,[])===null&&materializeAnchoredOperation(anchored,anchor,660,[])?.tasks.length===3;
    return Object.freeze({id,functionsExecuted:["preflightEngineInputForPlannerNext","adaptEngineInputToPlannerNextProblem","canPlaceTask","executePlannerNext","validatePlan","materializeAnchoredOperation"],observations:Object.freeze([
      observe(id,"meal.itinerant-unit.preflightStatus","PREFLIGHT","unit meal is supported",firstPreflight.status,"SUPPORTED"),observe(id,"meal.itinerant-unit.reason","PREFLIGHT","valid unit meal has no reason",firstPreflight.reasonCodes,[]),observe(id,"meal.itinerant-unit.adapterStatus","ADAPTER","adapter publishes unit meal",firstAdapter.status,"SUPPORTED"),observe(id,"meal.itinerant-unit.scope","ADAPTER","meal occupies exact unit only",obligation?.itinerantUnitId,`itinerant-team:${options.itinerantTeamId??7}`),observe(id,"meal.itinerant-unit.window","ADAPTER","fixed interval is preserved",obligation?.interval,{start:engineTimeToMinute(start),end:engineTimeToMinute(end)}),observe(id,"meal.itinerant-unit.identity","ADAPTER","unit identity is reversible",obligation?.itinerantUnitId,`itinerant-team:${options.itinerantTeamId??7}`),observe(id,"meal.itinerant-unit.entity","ADAPTER","break identity is reversible",obligation?.id,"break:unit-meal"),observe(id,"meal.itinerant-unit.bothPolicies","VALIDATION","both policies publish hard-valid plans",runs.every(x=>x.complete&&x.hardValid&&x.mealCount===1),true),observe(id,"meal.itinerant-unit.productiveLoad","SEARCH","unit 7 has work and a later alternative",runs.every(x=>x.productive>=2&&x.overlapRejected&&x.alternative===780),true),observe(id,"meal.itinerant-unit.foreignUnit","SEARCH","unit 8 works during unit 7 meal",runs.every(x=>x.unit8),true),observe(id,"meal.itinerant-unit.atomicOperation","VALIDATION","anchored operation is rejected atomically and has an alternative",atomic,true),observe(id,"meal.itinerant-unit.noResourceAlias","VALIDATION","unit identity is never a hard resource",runs.every(x=>x.noAlias),true)
    ]),reasonCodes:firstPreflight.reasonCodes,deterministic:stable(firstPreflight)===stable(secondPreflight),inputImmutable:before===stable(input)});
  }
