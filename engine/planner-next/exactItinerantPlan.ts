...
export interface ExactItinerantPlanEvidence {
  ...
  firstArrivalInjectiveAbstention:{reason:string;taskCount:number;distinctParticipantCount:number;fixedPrefixTaskCount:number}|null;
  ...
  deferredPrerequisiteReservationChecks:number; deferredPrerequisiteReservationRepairs:number;
...
const mergeRoundEvidence = (delta: ExactRoundSynchronizationEvidence): void => {
  ...
  evidence.geometryPrerequisiteEnvelopeChecks+=delta.geometryPrerequisiteEnvelopeChecks;
  evidence.geometryPrerequisiteEnvelopePrunes+=delta.geometryPrerequisiteEnvelopePrunes;
  evidence.logicalMatchingCandidatesAvoidedByEnvelope+=delta.logicalMatchingCandidatesAvoidedByEnvelope;
  if(evidence.firstFeedableCompactStart===null&&delta.firstFeedableCompactStart!==null)
...
const selectionReason = (selected: ReturnType<typeof macroConstrainedness>, candidates: ReturnType<typeof macroConstrainedness>[]): string => {
  ...
  if (peers.some((item) => item.domainExact !== selected.domainExact)) return "mixed-domain-exact-mrv";
...
const searchMacroUnits = (remainingUnits: MacroUnit[], placed: ScheduledTask[], preparations: ScheduledSetupPreparation[],
  ...
  const recurse = (tasks: ScheduledTask[], nextPreparations = preparations, nextRoundPreparations = roundPreparations): StandaloneOutcome => {
    ...
    if(!checked.feasible){const affectedParticipantId=checked.pendingArrivalDeadline?.firstCertificate?.participantIds[0]??(checked.blockingTaskId?problem.tasks.find(task=>task.id===checked.blockingTaskId)?.participantId:null)??null;
      ...
      return "DEAD_END";}
    if((problem.operationalMealPolicies?.length??0)>0){
      const operationalProbe=probeOperationalMealFutureFeasibility(problem,[...coreTasks,...placed,...tasks],tasks,operationalReservations);
      evidence.operationalMealFutureChecks+=1;
      evidence.operationalMealFutureAnalyticChecks+=operationalProbe.checkedPolicyIds.length;
      evidence.operationalMealFutureBranchesExplored+=operationalProbe.branchesExplored;
      evidence.operationalMealReservationChecks+=1;evidence.operationalMealReservationAnalyticChecks+=operationalProbe.checkedPolicyIds.length;
...
const searchOutcome = initialReservation.exhausted?"BUDGET_EXHAUSTED":!initialReservation.feasible||!initialOperationalReservation.feasible?"DEAD_END":searchMacroUnits(macroUnits, [], [], [], 0, [],initialOperationalReservation.reservations);
...
export function runExactItinerantPlanSearch(problem: PlannerNextProblem,
  ...
  const evidence: ExactItinerantPlanEvidence = {
    ...
    geometryPrerequisiteEnvelopeChecks:0,geometryPrerequisiteEnvelopePrunes:0,logicalMatchingCandidatesAvoidedByEnvelope:0,
    firstFeedableCompactStart:null,
    arrivalInjectiveEnvelopeChecks:0,arrivalInjectiveEnvelopePrunes:0,arrivalInjectiveFixedPrefixTasksChecked:0,
    arrivalInjectiveEdgeDeadlineChecks:0,arrivalInjectiveMaxMatchingChecks:0,firstArrivalInjectiveCertificate:null,
    firstArrivalInjectiveCertificateFixedPrefixTaskCount:null,firstArrivalInjectiveAbstention:null,
