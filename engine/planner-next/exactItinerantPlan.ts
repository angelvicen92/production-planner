...
export interface ExactItinerantPlanEvidence {
  ...
  operationalMealCheapProbes:number; operationalMealAffectedPoliciesChecked:number; operationalMealAnalyticDomainBuilds:number; operationalMealLogicalGridStarts:number; operationalMealAnalyticallyEliminatedStarts:number; operationalMealActuallyEvaluatedStarts:number; operationalMealZeroDomainPrunes:number; operationalMealExactSearchesAvoided:number; operationalMealBlockingPolicyIds:string[]; operationalMealFirstPrune:{policyId:string;causingTaskId?:string;causingCoreTaskIds?:string[];depth:number;context:"CORE"|"ORDINARY";logicalStartsBefore:number;logicalStartsAfter:number}|null;
...
function searchStandaloneForCoreCandidate(problem: PlannerNextProblem, coreTasks: ScheduledTask[], coreMeals: ScheduledSpaceMeal[],
  ...
  const search = (remaining: Task[], placed: ScheduledTask[], preparations: ScheduledSetupPreparation[], roundPreparations: ScheduledRoundPreparation[], depth: number, selectionOrder: string[]): StandaloneOutcome => {
    ...
    for (const { scheduled } of orderedStarts) {
      ...
      if((problem.operationalMealPolicies?.length??0)>0){const probe=probeOperationalMealFutureFeasibility(problem,provisionalPlaced,[scheduled]);evidence.operationalMealCheapProbes++;evidence.operationalMealAffectedPoliciesChecked+=probe.affectedPoliciesChecked;evidence.operationalMealAnalyticDomainBuilds+=probe.analyticDomainBuilds;evidence.operationalMealLogicalGridStarts+=probe.logicalGridStarts;evidence.operationalMealAnalyticallyEliminatedStarts+=probe.analyticallyEliminatedStarts;evidence.operationalMealActuallyEvaluatedStarts+=probe.actuallyEvaluatedStarts;evidence.operationalMealZeroDomainPrunes+=probe.zeroDomainPrunes;if(!probe.feasible){evidence.operationalMealExactSearchesAvoided++;for(const id of probe.blockingPolicyIds)if(!evidence.operationalMealBlockingPolicyIds.includes(id))evidence.operationalMealBlockingPolicyIds.push(id);const policyId=probe.blockingPolicyIds[0]!;evidence.operationalMealFirstPrune??={policyId,causingTaskId:scheduled.id,depth,context:"ORDINARY",logicalStartsBefore:probe.logicalStartCountByPolicyId[policyId]??0,logicalStartsAfter:probe.candidateCountByPolicyId[policyId]??0};evidence.standaloneBacktracks++;continue;}}
...
export function runExactItinerantPlanSearch(problem: PlannerNextProblem,
  ...
  const evidence: ExactItinerantPlanEvidence = {
    ...
    operationalMealCheapProbes:0,operationalMealAffectedPoliciesChecked:0,operationalMealAnalyticDomainBuilds:0,operationalMealLogicalGridStarts:0,operationalMealAnalyticallyEliminatedStarts:0,operationalMealActuallyEvaluatedStarts:0,operationalMealZeroDomainPrunes:0,operationalMealExactSearchesAvoided:0,operationalMealBlockingPolicyIds:[],operationalMealFirstPrune:null,
  };
  ...
  const core = runExactMainAndFeederSearch(problem, { ledger, ...options.coreOrderer, causalDiagnostic:options.causalDiagnostic, onPartialCoreCandidate(candidate) {
    ...
    if((problem.operationalMealPolicies?.length??0)>0){const probe=probeOperationalMealFutureFeasibility(problem,candidate.tasks,candidate.addedTasks);evidence.operationalMealCheapProbes++;evidence.operationalMealAffectedPoliciesChecked+=probe.affectedPoliciesChecked;evidence.operationalMealAnalyticDomainBuilds+=probe.analyticDomainBuilds;evidence.operationalMealLogicalGridStarts+=probe.logicalGridStarts;evidence.operationalMealAnalyticallyEliminatedStarts+=probe.analyticallyEliminatedStarts;evidence.operationalMealActuallyEvaluatedStarts+=probe.actuallyEvaluatedStarts;evidence.operationalMealZeroDomainPrunes+=probe.zeroDomainPrunes;if(!probe.feasible){evidence.operationalMealExactSearchesAvoided++;for(const id of probe.blockingPolicyIds)if(!evidence.operationalMealBlockingPolicyIds.includes(id))evidence.operationalMealBlockingPolicyIds.push(id);const policyId=probe.blockingPolicyIds[0]!;evidence.operationalMealFirstPrune??={policyId,causingCoreTaskIds:candidate.addedTasks.map(({id})=>id).sort(),depth:candidate.depth,context:"CORE",logicalStartsBefore:probe.logicalStartCountByPolicyId[policyId]??0,logicalStartsAfter:probe.candidateCountByPolicyId[policyId]??0};return "REJECT";}}
