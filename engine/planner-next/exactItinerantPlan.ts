...
export interface ExactItinerantPlanEvidence {
  ...
  arrivalReadinessFirstCertificate:{cutoff:number;demand:number;maximumPossible:number;requiredParticipantIds:string[]}|null;
...
function addArrivalReadinessEvidence(evidence:ExactItinerantPlanEvidence,value:{arrivalReadinessChecks:number;arrivalReadinessPrunes:number;logicalSlots:number;analyticallyDerivedSlots:number;arrivalReadinessFirstCertificate:ExactItinerantPlanEvidence["arrivalReadinessFirstCertificate"]}):void { evidence.arrivalReadinessChecks+=value.arrivalReadinessChecks;evidence.arrivalReadinessPrunes+=value.arrivalReadinessPrunes;evidence.logicalSlots+=value.logicalSlots;evidence.analyticallyDerivedSlots+=value.analyticallyDerivedSlots;evidence.arrivalReadinessFirstCertificate??=value.arrivalReadinessFirstCertificate; }
...
function searchStandaloneForCoreCandidate(problem: PlannerNextProblem, coreTasks: ScheduledTask[], coreMeals: ScheduledSpaceMeal[],
  ...
  const search = (remaining: Task[], placed: ScheduledTask[], preparations: ScheduledSetupPreparation[], roundPreparations: ScheduledRoundPreparation[], depth: number, selectionOrder: string[], reservation:DeferredPrerequisiteReservation|null, operationalReservations:readonly OperationalMealReservation[]): StandaloneOutcome => {
    ...
    for (const { scheduled } of orderedStarts) {
      ...
      addArrivalReadinessEvidence(evidence,reserved);
...
const searchMacroUnits = (remainingUnits: MacroUnit[], placed: ScheduledTask[], preparations: ScheduledSetupPreparation[],
  roundPreparations: ScheduledRoundPreparation[], depth: number, selectionOrder: string[], reservation:DeferredPrerequisiteReservation|null, operationalReservations:readonly OperationalMealReservation[]): StandaloneOutcome => {
  if (remainingUnits.length === 0) return search(ordinaryPending, placed, preparations, roundPreparations, placed.length, selectionOrder,reservation,operationalReservations);
  ...
  const recurse = (tasks: ScheduledTask[], nextPreparations = preparations, nextRoundPreparations = roundPreparations): StandaloneOutcome => {
    ...
    if(checked.arrivalReadiness)addArrivalReadinessEvidence(evidence,{arrivalReadinessChecks:checked.arrivalReadiness.checks,
      arrivalReadinessPrunes:checked.arrivalReadiness.prunes,logicalSlots:checked.arrivalReadiness.logicalSlots,
      analyticallyDerivedSlots:checked.arrivalReadiness.analyticallyDerivedSlots,
      arrivalReadinessFirstCertificate:checked.arrivalReadiness.firstCertificate});
...
addArrivalReadinessEvidence(evidence,initialReservation);
...
const searchOutcome = initialReservation.exhausted?"BUDGET_EXHAUSTED":!initialReservation.feasible||!initialOperationalReservation.feasible?"DEAD_END":searchMacroUnits(macroUnits, [], [], [], 0, [],initialReservation.reservation,initialOperationalReservation.reservations);
...
export function runExactItinerantPlanSearch(problem: PlannerNextProblem,
  ...
  const evidence: ExactItinerantPlanEvidence = {
    ...
    arrivalReadinessChecks:0,arrivalReadinessPrunes:0,logicalSlots:0,analyticallyDerivedSlots:0,arrivalReadinessFirstCertificate:null,
