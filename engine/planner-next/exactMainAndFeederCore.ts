...
export interface ExactSetupCandidateOutcomeCertificate {
  ...
  samples:Array<{familyOrder:string[];start:number;end:number;interFamilyGap:number;
    firstRejection:{depth:number;failureKind:string;authorityId:string|null;blockingTaskId:string|null;participantId:string|null}|null;
    deepestRejection:{depth:number;failureKind:string;authorityId:string|null;blockingTaskId:string|null;participantId:string|null}|null}>;
}
export interface ExactSetupGeometryMatchingOutcomeCertificate {
  geometryKey:string;start:number;end:number;familyOrder:string[];idleMinutes:number;spanMinutes:number;
  matchingsDeliveredToChild:number;
  outcomes:Array<{rejectionSignature:string;frequency:number;percentage:number;
    immediateOutcome:"REJECTED_IMMEDIATELY"|"ENTERED_DESCENDANTS"|"BUDGET_EXHAUSTED";
    firstRejectionKind:string;authorityId:string|null;blockingTaskId:string|null;participantId:string|null;
    samples:Array<{repairIndex:number;maximumDescendantDepth:number}>}>;
}
export interface ExactMacroParticipantMealCertificate { macroDepth:number;frontierDepth:number;macroUnitId:string;macroKind:string;
  addedTasks:Array<{id:string;start:number;end:number;participantId:string|null;kind:string}>;blockingMealTaskIds:string[];
  blockedObligations:Array<{mealTaskId:string;participantId:string}>;reasonCodes:string[];candidateCountByTaskId:Record<string,number>;
  remainingMacroUnits:Array<{id:string;kind:string}>;branchesExplored:number;frontierFingerprint:string }
export interface ExactMacroParticipantMealDiagnostic { firstDestruction:ExactMacroParticipantMealCertificate|null;
  aggregatesByKindDepth:Record<string,{candidates:number;feasible:number;infeasible:number;firstDestructions:number}>;
  deepestFeasibleFrontier:{macroDepth:number;frontierDepth:number;macroUnitId:string;macroKind:string;remainingMacroUnitIds:string[];
    branchesExplored:number;frontierFingerprint:string}|null }
export interface ExactCoreCausalDiagnostic { waterfallByDepth:Record<string,ExactDepthWaterfall>; feederByDepth:Record<string,ExactDepthFeeder>; feederRejections:ExactCriticalFeederRejection[]; feederCoachDomainEliminations:ExactFeederCoachDomainElimination[]; feederMatching:ExactFeederMatchingCausalSummary; futureFeasibility:ExactFutureFeasibilityCausalSummary; standaloneFrontier:ExactStandaloneFrontierCausalSummary;deepestStandaloneFrontier:ExactDeepestStandaloneFrontier|null;macroParticipantMeals:ExactMacroParticipantMealDiagnostic;macroPendingPrerequisiteCapacityCertificates:ExactMacroCapacityCertificate[];macroPendingPrerequisiteCapacityCertificateOverflow:number;deferredArrivalFirstRepair:import("./deferredPrerequisiteReservation").DeferredArrivalCausalCertificate|null;setupCandidateOutcomeCertificates:ExactSetupCandidateOutcomeCertificate[];setupCandidateOutcomeCertificateOverflow:number;setupGeometryMatchingOutcomeCertificates:ExactSetupGeometryMatchingOutcomeCertificate[];setupGeometryMatchingOutcomeCertificateOverflow:number }
...
export function runExactMainAndFeederSearch(problem: PlannerNextProblem,
  ...
  const diagnostic:ExactCoreCausalDiagnostic|null=options.causalDiagnostic?{waterfallByDepth:{},feederByDepth:{},feederRejections:[],feederCoachDomainEliminations:[],feederMatching:{contexts:[],overflowContexts:0},futureFeasibility:{totalEvaluations:0,uniqueAuthorityStates:0,repeatedEvaluations:0,authorityResultCollisions:0,negativeEvaluations:0,repeatedNegativeEvaluations:0,rejectsWithCertifiedBackjumpTarget:0,evaluationsByDepth:{},repeatedByDepth:{},negativeByDepth:{},assessments:[],collisions:[]},standaloneFrontier:{totalRejections:0,certificates:[],examples:[]},deepestStandaloneFrontier:null,macroParticipantMeals:{firstDestruction:null,aggregatesByKindDepth:{},deepestFeasibleFrontier:null},macroPendingPrerequisiteCapacityCertificates:[],macroPendingPrerequisiteCapacityCertificateOverflow:0,deferredArrivalFirstRepair:null,setupCandidateOutcomeCertificates:[],setupCandidateOutcomeCertificateOverflow:0,setupGeometryMatchingOutcomeCertificates:[],setupGeometryMatchingOutcomeCertificateOverflow:0}:null;
