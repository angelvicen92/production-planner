...
test("scores every complete order before applying bestK",()=>{const p=scenario();p.budget.bestK=1;p.auxiliaryPolicy={participantPresencePreference:"HIGH"};const prior=scheduled(p,"a",680,{id:"prior-after",duration:20,spaceId:"outside",requiredResourceIds:[],start:680,end:700}),all=constructSaturatedResourceWindowBlockCandidates(p,block(p),[prior],200);assert.deepEqual(all.candidates.map(candidate=>candidate.order),[["a","b"],["b","a"]]);assert.ok(scoreAuxiliaryTaskSequence(p,all.candidates[1]!.tasks,[prior])<scoreAuxiliaryTaskSequence(p,all.candidates[0]!.tasks,[prior]));const result=placeAuxiliaryTasks(p,[prior],200);assert.ok(result.tasks);assert.deepEqual(result.saturatedSelectedOrders[block(p,[prior]).key],["b","a"])});
