...
import { jointGroupMembers } from "../jointTasks";
...
const inputs: Record<string, () => PlannerNextProblem> = {
  ...
  setupGrouping: setupGroupingScenario,
  setupPreparation: setupPreparationScenario,
  jointAuxiliaryTasks: jointAuxiliaryTasksScenario,
};
function logicalResult(problem: PlannerNextProblem) {
  ...
  return {
    ...
    secondaryContinuityViolationCount: m.secondaryContinuityViolationCount, setupViolationCount: m.setupViolationCount,
    setupFamilySequenceBySpaceId:m.setupFamilySequenceBySpaceId,setupBlockCountBySpaceAndFamily:m.setupBlockCountBySpaceAndFamily,setupSwitchCountBySpaceId:m.setupSwitchCountBySpaceId,
    ...
    violationCount: m.dependencyViolationCount + m.overlapViolationCount + m.transitionViolationCount + m.availabilityViolationCount + m.blockViolationCount + m.resourceAvailabilityViolationCount + m.resourceOverlapViolationCount + m.resourceTransitionViolationCount + m.secondaryContinuityViolationCount + m.setupViolationCount + m.setupPreparationViolationCount + m.jointGroupViolationCount,
...
const setupResult=scenarios.setupGrouping;
const preparationProblem=setupPreparationScenario(), preparationPlan=planMainFlowAndFeeders(preparationProblem), preparationMetrics=preparationPlan.metrics;
const setupTasksOrdered=preparationPlan.scheduledTasks.filter(t=>t.spaceId==="setup-room").sort((a,b)=>a.start-b.start||a.id.localeCompare(b.id));
const occupations=[...setupTasksOrdered,...preparationPlan.scheduledSetupPreparations].sort((a,b)=>a.start-b.start||a.id.localeCompare(b.id));
scenarios.setupPreparation={...scenarios.setupPreparation,scheduledSetupPreparations:preparationPlan.scheduledSetupPreparations,setupPreparationCount:preparationMetrics.setupPreparationCount,setupPreparationMinutesBySpaceId:preparationMetrics.setupPreparationMinutesBySpaceId,setupPreparationCountBySpaceAndFamily:preparationMetrics.setupPreparationCountBySpaceAndFamily,setupPreparationSequenceBySpaceId:preparationMetrics.setupPreparationSequenceBySpaceId,taskFamilySequence:preparationMetrics.setupFamilySequenceBySpaceId["setup-room"],taskSpanMinutes:(setupTasksOrdered.at(-1)?.end??0)-(setupTasksOrdered[0]?.start??0),occupiedSpanMinutes:(occupations.at(-1)?.end??0)-(occupations[0]?.start??0),setupRoomStart:occupations[0]?.start,setupRoomEnd:occupations.at(-1)?.end,gap:preparationMetrics.secondarySpaceGapMinutesById["setup-room"],blockCount:preparationMetrics.secondarySpaceBlockCountById["setup-room"],setupSwitches:preparationMetrics.setupSwitchCountBySpaceId["setup-room"],violationCounts:{setupPreparation:preparationMetrics.setupPreparationViolationCount,setup:preparationMetrics.setupViolationCount}};
const setupAccepted=setupResult.complete&&setupResult.hardValid&&setupResult.plannedTaskCount===24&&setupResult.secondarySpaceEndById["setup-room"]-setupResult.secondarySpaceStartById["setup-room"]===90;
const preparationAccepted=preparationPlan.complete&&preparationMetrics.hardValid&&preparationPlan.scheduledSetupPreparations.length===3&&preparationMetrics.setupPreparationMinutesBySpaceId["setup-room"]===30&&scenarios.setupPreparation.occupiedSpanMinutes===120&&scenarios.setupPreparation.gap===0&&scenarios.setupPreparation.blockCount===1&&preparationMetrics.setupPreparationViolationCount===0;
const jointProblem=jointAuxiliaryTasksScenario(), jointPlan=planMainFlowAndFeeders(jointProblem), jointMembers=jointPlan.scheduledTasks.filter(t=>t.jointGroupId==="shared-operation-1").sort((a,b)=>a.id.localeCompare(b.id));
const jointMetrics=jointPlan.metrics;
scenarios.jointAuxiliaryTasks={...scenarios.jointAuxiliaryTasks,members:jointMembers.map(({id,participantId,start,end,duration,spaceId,requiredResourceIds,jointGroupId})=>({id,participantId,start,end,duration,spaceId,resourceIds:requiredResourceIds,jointGroupId})),candidateCount:jointMetrics.jointGroupCandidateCountWhenSelectedById["shared-operation-1"],workItemSelectionOrder:jointMetrics.auxiliaryWorkItemSelectionOrder,resourcePresence:jointMetrics.resourcePresenceMinutesById["joint-resource"],resourceInternalGap:jointMetrics.resourceInternalGapMinutesById["joint-resource"],jointGroupCount:jointMetrics.jointGroupCount,jointScheduledTaskCount:jointMetrics.jointScheduledTaskCount,jointGroupViolationCount:jointMetrics.jointGroupViolationCount,branches:jointMetrics.branchesExplored,fingerprint:jointMetrics.planFingerprint};
const jointAccepted=jointPlan.complete&&jointMetrics.hardValid&&jointMetrics.plannedTaskCount===26&&jointMembers.length===2&&jointMembers[0]?.start===jointMembers[1]?.start&&jointMembers[0]?.end===jointMembers[1]?.end&&jointMembers.every(t=>t.spaceId==="joint-room")&&jointMetrics.resourcePresenceMinutesById["joint-resource"]===20&&jointMetrics.resourceInternalGapMinutesById["joint-resource"]===0&&jointMetrics.jointGroupViolationCount===0&&jointMetrics.auxiliaryWorkItemSelectionOrder.filter(x=>x==="joint:shared-operation-1").length===1;
const evidenceProblem=jointAuxiliaryTasksScenario(), evidenceMembers=jointGroupMembers(evidenceProblem.tasks,"shared-operation-1");
const shortBudget={remaining:2}, shortAssessment=assessFutureFeasibility(evidenceProblem,[],evidenceMembers,shortBudget,()=>({count:0,exhausted:false}));
evidenceProblem.budget.bestK=3;const bestKBudget={remaining:1000}, bestKAssessment=assessFutureFeasibility(evidenceProblem,[],[...evidenceMembers].reverse(),bestKBudget,()=>({count:0,exhausted:false}));
const jointFutureFeasibilityEvidence={shortBudget:{exhausted:shortAssessment.exhausted,feasible:shortAssessment.feasible,branchesConsumed:shortAssessment.branchesConsumed,budgetRemaining:shortBudget.remaining},bestK:{exhausted:bestKAssessment.exhausted,feasible:bestKAssessment.feasible,branchesConsumed:bestKAssessment.branchesConsumed,budgetRemaining:bestKBudget.remaining,assessment:bestKAssessment.assessments[0]}};
const jointFutureFeasibilityBudgetAccepted=shortAssessment.exhausted&&!shortAssessment.feasible&&shortAssessment.branchesConsumed===2&&shortBudget.remaining===0&&!bestKAssessment.exhausted&&bestKAssessment.feasible&&bestKAssessment.assessments[0]?.key==="joint:shared-operation-1"&&bestKAssessment.assessments[0]?.kind==="joint"&&bestKAssessment.assessments[0]?.alternativeCount===3&&bestKAssessment.branchesConsumed>=3;
const accepted = jointFutureFeasibilityBudgetAccepted && jointAccepted && preparationAccepted && setupAccepted && historicalOk && historical.bestK === 5 && bounded.bestK === 1 && bestK === 1 && bounded.complete && bounded.hardValid && bounded.plannedTaskCount === 22 && bounded.mainFlowGapMinutes === 0 && bounded.secondaryContinuityViolationCount === 0 && bounded.violationCount === 0 && bounded.logicalMetrics.futureInfeasibleCandidatesPruned >= 1 && bounded.logicalMetrics.futureTopRankedCandidatesPruned >= 1 && bounded.logicalMetrics.futureBlockerCountByWorkItemKey["task:scarce-window-task"] >= 1 && bounded.logicalMetrics.acceptedPathMinimumFutureAlternativeCount >= 1 && topAssessment?.feasible === false && topAssessment.blockingWorkItemKeys.includes("task:scarce-window-task") && secondAssessment?.feasible === true && scenarios.boundedFutureFeasibility.boundedEvidence.acceptedBlockIsViableAlternative && boundedBlockConstruction.withinBound && boundedBlockConstruction.deterministic && boundedBlockConstruction.maximumPartialStatesPerStart <= 1 && probe1.consumed < search.consumed && probeK.consumed <= search.consumed && search.candidates.length >= 2 && countersOk && branchHistoryInvariance.regionStructurallyIndependent && branchHistoryInvariance.projectionsEqual && branchHistoryInvariance.originalWorkItemOrderEqual && branchHistoryInvariance.originalParticipantPresenceEqual && branchHistoryInvariance.originalAuxiliaryCandidateCountsEqual && branchHistoryInvariance.originalResourcePresenceEqual && branchHistoryInvariance.isolatedTasksPlanned && branchHistoryInvariance.isolatedFuturePrunes>=1 && branchHistoryInvariance.isolatedTopLocalPrunes>=1 && isolatedBlockersOnly && variant.complete && variant.hardValid && variant.plannedTaskCount===22 && variant.violationCount===0 && Object.values(scenarios).every((scenario: any) => scenario.runtimeMs < 2000 && scenario.deterministic);
process.stdout.write(JSON.stringify({ version: "planner-next-joint-auxiliary-tasks-v2", scenarios, jointFutureFeasibilityEvidence, setupEvidence: { familySequence: setupResult.setupFamilySequenceBySpaceId, blockCounts: setupResult.setupBlockCountBySpaceAndFamily, switches: setupResult.setupSwitchCountBySpaceId, continuity: setupResult.secondarySpaceGapMinutesById, workItemSelectionOrder: setupResult.workItemSelectionOrder }, boundedBlockConstruction, branchHistoryInvariance, acceptance: { accepted, jointFutureFeasibilityBudgetAccepted, jointAuxiliaryTasksAccepted:jointAccepted, setupPreparationAccepted:preparationAccepted, historicalRegressionIntact:historicalOk, frozenFingerprints: frozen } }, null, 2) + "\n");
