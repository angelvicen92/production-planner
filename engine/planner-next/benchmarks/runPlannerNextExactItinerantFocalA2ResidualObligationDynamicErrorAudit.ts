...
import { auditResidualObligationDynamicError } from "../residualObligationDynamicErrorAudit";
...
const audit = auditResidualObligationOutcomes(createProblem(), baseline.plan.scheduledTasks, experiment.plan.scheduledTasks, experiment.orderingEvidence!.acceptedPathDecisions);
const auditAgain = auditResidualObligationOutcomes(createProblem(), baselineAgain.plan.scheduledTasks, experimentAgain.plan.scheduledTasks, experimentAgain.orderingEvidence!.acceptedPathDecisions);
const auditReversed = auditResidualObligationOutcomes(createProblem(true), baselineReversed.plan.scheduledTasks, experimentReversed.plan.scheduledTasks, experimentReversed.orderingEvidence!.acceptedPathDecisions);
const dynamicAudit = auditResidualObligationDynamicError(createProblem(), experiment.plan.scheduledTasks, experiment.plan.scheduledSpaceMeals, audit, experiment.orderingEvidence!.acceptedPathDecisions);
const dynamicAuditAgain = auditResidualObligationDynamicError(createProblem(), experimentAgain.plan.scheduledTasks, experimentAgain.plan.scheduledSpaceMeals, auditAgain, experimentAgain.orderingEvidence!.acceptedPathDecisions);
const dynamicAuditReversed = auditResidualObligationDynamicError(createProblem(true), experimentReversed.plan.scheduledTasks, experimentReversed.plan.scheduledSpaceMeals, auditReversed, experimentReversed.orderingEvidence!.acceptedPathDecisions);
...
const artifact = { baseline, experiment, audit, dynamicAudit, deterministic: true, orderInvariant: true,
...
assert.equal(audit.aggregates.participantCount, 19);
assert.equal(audit.aggregates.improvedParticipantCount, 7); assert.equal(audit.aggregates.worsenedParticipantCount, 6);
assert.equal(audit.aggregates.unchangedParticipantCount, 6); assert.equal(audit.aggregates.grossImprovementMinutes, 510);
assert.equal(audit.aggregates.grossHarmMinutes, 375); assert.equal(audit.aggregates.netIdleImprovementMinutes, 135);
assert.equal(audit.aggregates.maximumIndividualHarm, 120); assert.deepEqual(audit.aggregates.participantsUnmapped, []);
assert.equal(audit.acceptedPathDecisions.length, 19); assert.equal(audit.auditFingerprint, auditAgain.auditFingerprint);
assert.equal(audit.auditFingerprint, auditReversed.auditFingerprint);
assert.equal(experiment.orderingEvidence!.acceptedPathDecisions.filter((d) => d.selectedTrace).length, 19);
assert.equal(dynamicAudit.dynamicErrorAuditFingerprint, dynamicAuditAgain.dynamicErrorAuditFingerprint);
assert.equal(dynamicAudit.dynamicErrorAuditFingerprint, dynamicAuditReversed.dynamicErrorAuditFingerprint);
assert.equal(dynamicAudit.dynamicErrorAuditFingerprint, "23fb148849da5795388cf145aca80a80a77fea3587c35d4792314f92c7f40ec8");
