...
const current = JSON.parse(readFileSync("planner-next-focal-a2-itinerant-unit-audit-v3.json", "utf8"));
...
test("current artifact retains the v2 preferred historical scenarios and frozen plans", () => {
  ...
  assert.equal(current.historicalEvidence.preferredPlan.fingerprint, "cff587b5eac3b77d6e81589791035aead34187b65ab248d9586e462294e0087b");
  assert.equal(current.historicalEvidence.preferredPlan.branches, 15599);
  assert.deepEqual(current.scenarios.focalA2BandPreferredAudit.preferred.bandPresence.preferredLexicographicTuple, [4, 330, 60]);
  const scheduled = current.historicalEvidence.preferredPlan.scheduledTasks;
  assert.equal(Math.max(...scheduled.map((task: any) => task.end)) - Math.min(...scheduled.map((task: any) => task.start)), 450);
  assert.equal(current.historicalEvidence.preferredPlan.mainFlowSpanMinutes, 360);
  assert.equal(current.historicalEvidence.currentOff.fingerprint, "76f52d292e810ab8506ba868d77036126f299bcf129462a62b6c3b49a13be4fc");
  assert.equal(current.historicalEvidence.currentOff.branches, 64558);
});
