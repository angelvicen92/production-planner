...
import { createTechnicalChainExplorer, getTechnicalChains, type TechnicalChainDeferredQueueMode } from "../technicalChains";
import { technicalChainScenario } from "../scenarios/technicalChainScenario";

const candidateLimit=100;
const run=(mode:TechnicalChainDeferredQueueMode)=>{
  ...
  const explorer=createTechnicalChainExplorer(problem,chain,[],100_000,"ANALYTIC_DOMAIN",[],mode,true);
...
process.stdout.write(JSON.stringify({candidateLimit,globalSortOracle:{runtimeMs:globalSortOracle.runtimeMs,
  ...
  branches:incrementalHeap.branches,diagnostics:incrementalHeap.diagnostics},sameCandidateOrder:true,sameBranches:true},null,2)+"\n");
