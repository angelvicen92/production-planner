...
function runTransportPolicyProbe(expansion: ExpandedCanonicalFullA2Template): RepresentabilityAnalysis["transportPolicyProbe"] {
  ...
  input.arrivalGroupingTarget = policy.arrival.targetGroupSize;
  input.departureGroupingTarget = policy.departure.targetGroupSize;
  input.arrivalMinGapMinutes = policy.arrival.minGapMinutes;
  input.departureMinGapMinutes = policy.departure.minGapMinutes;
  input.arrivalMaximumGroupSize = policy.arrival.maximumGroupSize;
  input.departureMaximumGroupSize = policy.departure.maximumGroupSize;
  input.vanCapacity = policy.arrival.maximumGroupSize;
  input.transportSettings = { arrivalTargetGroupSize: policy.arrival.targetGroupSize, departureTargetGroupSize: policy.departure.targetGroupSize, arrivalMinGapMinutes: policy.arrival.minGapMinutes, departureMinGapMinutes: policy.departure.minGapMinutes, arrivalMaximumGroupSize: policy.arrival.maximumGroupSize, departureMaximumGroupSize: policy.departure.maximumGroupSize, vanCapacity: policy.arrival.maximumGroupSize, groupingWeight: policy.arrival.groupingWeight, source: "engine-buildInput-optimizer-transport" };
  const snapshot = structuredClone(input);
  const preflight = preflightEngineInputForPlannerNext(input);
  const adapted = adaptEngineInputToPlannerNextProblem(input);
  ...
  const maximumPreserved = arrival?.maximumGroupSize === 3 && departure?.maximumGroupSize === 6;
  ...
  const separateTargetSemantic = [arrival, departure].every((direction) => direction !== undefined
    && direction.minimumGroupSize === 1 && "targetGroupSize" in direction);
  const invalidInputs = [
  ...
  return {
    sourceConfigurationPresent: policy.arrival.targetGroupSize === 3 && policy.arrival.maximumGroupSize === 3 && policy.arrival.minGapMinutes === 30 && policy.arrival.groupingWeight === 3 && policy.departure.targetGroupSize === 1 && policy.departure.maximumGroupSize === 6 && policy.departure.minGapMinutes === 20 && policy.departure.groupingWeight === 3,
    ...
    plannerNextContractPresent, targetPreserved, maximumPreserved, minGapPreserved, groupingWeightPreserved,
    separateTargetSemantic,
...
export function analyzeCanonicalFullA2Representability(
  ...
  const transportRepresentationProven = transportPolicyProbe.sourceConfigurationPresent
    ...
    && transportPolicyProbe.targetPreserved && transportPolicyProbe.maximumPreserved
    && transportPolicyProbe.minGapPreserved && transportPolicyProbe.groupingWeightPreserved
    && transportPolicyProbe.separateTargetSemantic && transportPolicyProbe.capabilityDetected
