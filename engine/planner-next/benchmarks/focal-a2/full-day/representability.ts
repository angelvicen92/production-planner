...
function runTransportPolicyProbe(expansion: ExpandedCanonicalFullA2Template): RepresentabilityAnalysis["transportPolicyProbe"] {
  ...
  input.transportSettings = { arrivalTargetGroupSize: policy.arrival.targetGroupSize, departureTargetGroupSize: policy.departure.targetGroupSize, arrivalMinGapMinutes: policy.arrival.minGapMinutes, departureMinGapMinutes: policy.departure.minGapMinutes, vanCapacity: policy.arrival.maximumGroupSize, groupingWeight: policy.arrival.groupingWeight, source: "engine-buildInput-optimizer-transport" };
  const snapshot = structuredClone(input);
  const preflight = preflightEngineInputForPlannerNext(input);
  const adapted = adaptEngineInputToPlannerNextProblem(input);
  ...
  const maximumPreserved = arrival?.maximumGroupSize === 3 && departure?.maximumGroupSize === 3;
  ...
  return {
    sourceConfigurationPresent: policy.arrival.targetGroupSize === 3 && policy.arrival.maximumGroupSize === 3 && policy.arrival.minGapMinutes === 30 && policy.arrival.groupingWeight === 3 && policy.departure.targetGroupSize === 1 && policy.departure.maximumGroupSize === 3 && policy.departure.minGapMinutes === 20 && policy.departure.groupingWeight === 3,
