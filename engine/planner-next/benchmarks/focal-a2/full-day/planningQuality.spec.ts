...
test("shared planning-quality evaluator computes the human P01 baseline without using the reference as planner input", () => {
  ...
  assert.deepEqual(p01.value, {
    firstStart: 675,
    lastEnd: 1035,
    productiveMinutes: 285,
    authorizedPauseMinutes: 75,
...
test("human P03 participant presence baseline is frozen from canonical obligations", () => {
  ...
  assert.deepEqual(p03.value.byParticipantId, {
    C01: 385, C02: 385, C03: 325, C04: 510, C05: 355, C06: 545, C07: 300,
    C08: 265, C09: 435, C10: 515, C11: 495, C12: 380, C13: 465, C14: 405,
    C15: 380, C16: 440, C17: 350, C18: 350, C19: 300,
  });
});
...
test("human P09 baseline proves anchored, joint, technical and Totales synchronization semantics", () => {
  ...
  assert.deepEqual(p09.value, {
    anchoredOperationCount: 3,
    anchoredViolationCount: 0,
    jointOperationCount: 2,
    jointViolationCount: 0,
    technicalChainCount: 1,
    technicalChainViolationCount: 0,
    synchronizedTotalesRoundCount: 9,
    totalesSynchronizationViolationCount: 0,
    residualTotales1RoundCount: 1,
    residualTotalesCoreoRoundCount: 0,
...
test("quality evaluator blocks KPIs whose required operational configuration is not yet available instead of guessing", () => {
  ...
  for (const id of [
    "P04_AVOIDABLE_PARTICIPANT_WAIT",
    "P05_CRITICAL_RESOURCE_PRESENCE",
    "P06_SPACE_CONTINUITY_UTILIZATION",
    "P07_BLOCKS_SETUPS",
    "P08_MOVES_ZONES",
    "P10_ROBUSTNESS_SLACK",
  ] as const) {
...
test("quality evaluator rejects partial or non-canonical planning identity sets", () => {
  assert.throws(
    () => evaluatePlanningQuality(expanded, reference.intervals.slice(1), { authorizedMainFlowBreaks: [{ start: 840, end: 915 }] }),
    /exact canonical task identity set/,
  );
});
