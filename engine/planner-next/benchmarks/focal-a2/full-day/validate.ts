...
const invariantChecks: ReadonlyArray<[string, (expansion: ExpandedCanonicalFullA2Template) => ValidationIssue[]]> = [
  ...
  ["EFFECTIVE_CONFIGURATION", (expansion) => {
    ...
    const transportValid = c.transportPolicy.arrival.minimumGroupSize === 3 && c.transportPolicy.arrival.maximumGroupSize === 6 && c.transportPolicy.arrival.minGapMinutes === 35 && c.transportPolicy.arrival.groupingWeight === 3 && c.transportPolicy.departure.minimumGroupSize === 3 && c.transportPolicy.departure.maximumGroupSize === 6 && c.transportPolicy.departure.minGapMinutes === 20 && c.transportPolicy.departure.groupingWeight === 3;
  ...
  ["TRANSPORT_RULE", (expansion) => expansion.rules.inTransport.minimumGroupSize === 3 && expansion.rules.inTransport.maximumGroupSize === 6 && expansion.rules.inTransport.minGapMinutes === 35 && expansion.rules.outTransport.minimumGroupSize === 3 && expansion.rules.outTransport.maximumGroupSize === 6 && expansion.rules.outTransport.minGapMinutes === 20 && expansion.tasks.filter((task) => task.type === "IN").every((task) => task.operationalKind === "transport_arrival" && task.transport?.direction === "arrival") && expansion.tasks.filter((task) => task.type === "OUT").every((task) => task.operationalKind === "transport_departure" && task.transport?.direction === "departure")
    ? []
