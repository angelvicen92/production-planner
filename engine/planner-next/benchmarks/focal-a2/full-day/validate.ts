...
const invariantChecks: ReadonlyArray<[string, (expansion: ExpandedCanonicalFullA2Template) => ValidationIssue[]]> = [
  ...
  ["EFFECTIVE_CONFIGURATION", (expansion) => {
    ...
    const transportValid = c.transportPolicy.arrival.minParticipantsPerGroup === 3 && c.transportPolicy.arrival.groupingTarget === 3 && c.transportPolicy.arrival.minGapMinutes === 35 && c.transportPolicy.arrival.vanCapacity === 6 && c.transportPolicy.arrival.groupingWeight === 3 && c.transportPolicy.departure.groupingTarget === 3 && !("minParticipantsPerGroup" in c.transportPolicy.departure) && c.transportPolicy.departure.minGapMinutes === 20 && c.transportPolicy.departure.vanCapacity === 6 && c.transportPolicy.departure.groupingWeight === 3;
  ...
  ["TRANSPORT_RULE", (expansion) => expansion.rules.inTransport.minParticipantsPerGroup === 3 && expansion.rules.inTransport.groupingTarget === 3 && expansion.rules.inTransport.minGapMinutes === 35 && expansion.rules.inTransport.vanCapacity === 6 && expansion.rules.outTransport.groupingTarget === 3 && !("minParticipantsPerGroup" in expansion.rules.outTransport) && expansion.rules.outTransport.minGapMinutes === 20 && expansion.rules.outTransport.vanCapacity === 6 && expansion.tasks.filter((task) => task.type === "IN").every((task) => task.operationalKind === "transport_arrival" && task.transport?.direction === "arrival") && expansion.tasks.filter((task) => task.type === "OUT").every((task) => task.operationalKind === "transport_departure" && task.transport?.direction === "departure")
    ? []
