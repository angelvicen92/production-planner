...
const invariantChecks: ReadonlyArray<[string, (expansion: ExpandedCanonicalFullA2Template) => ValidationIssue[]]> = [
  ...
  ["GLOBAL_COUNTS", (expansion) => {
    ...
    if (expansion.tasks.length !== 266) issues.push(issue("GLOBAL_COUNTS", "TOTAL_TASK_COUNT_MISMATCH", "tasks", `Expected 266 canonical obligations, got ${expansion.tasks.length}.`));
    return issues;
  }],
  ...
  ["COACH_TRANSITION_RULE", (expansion) => expansion.rules.coachTransition.minutes === 30 && expansion.rules.coachTransition.scope === "coach" && expansion.rules.mainFlow.maxBlocksPerCoach >= expansion.participants.length && expansion.rules.mainFlow.continuity === "REQUIRED"
    ? []
  ...
  ["EFFECTIVE_CONFIGURATION", (expansion) => {
    ...
    const transportValid = c.transportPolicy.arrival.targetGroupSize === 3 && c.transportPolicy.arrival.maximumGroupSize === 3 && c.transportPolicy.arrival.minGapMinutes === 30 && c.transportPolicy.arrival.groupingWeight === 3 && c.transportPolicy.departure.targetGroupSize === 1 && c.transportPolicy.departure.maximumGroupSize === 6 && c.transportPolicy.departure.minGapMinutes === 20 && c.transportPolicy.departure.groupingWeight === 3;
    const mealsValid = c.meals.effectiveWindow.start === "13:00" && c.meals.effectiveWindow.end === "16:30" && c.meals.operational.defaultDurationMinutes === 75 && c.meals.operational.coachDurationMinutes === 45 && c.meals.operational.realityDurationMinutes === 75 && c.meals.operational.flexible === true && c.meals.operational.fixedHumanCutIntervals.length === 0 && c.meals.operational.legacyItinerantMealBreakMinutesAuthoritative === false && c.meals.participant.sodexoDurationMinutes === 40 && c.meals.participant.maxSimultaneous === 10 && c.meals.participant.independentFromOperationalMeal === true;
  ...
  ["TECHNICAL_CHAIN", (expansion) => {
    ...
    const expectedPhases = [expected.slice(0, 3), expected.slice(3, 5), expected.slice(5, 7)];
    const chain = expansion.technicalChains[0];
    return expansion.technicalChains.length === 1 && chain?.id === "continuity.reality-c-eva-alfombra"
      ...
      && JSON.stringify(chain.phases) === JSON.stringify(expectedPhases)
