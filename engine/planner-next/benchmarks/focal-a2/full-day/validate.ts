...
const invariantChecks: ReadonlyArray<[string, (expansion: ExpandedCanonicalFullA2Template) => ValidationIssue[]]> = [
  ...
  ["EFFECTIVE_CONFIGURATION", (expansion) => {
    ...
    const bandConfigurationValid = Object.keys(c.requiresBand).length === 19
      && expansion.participants.every((id) => typeof c.requiresBand[id as keyof typeof c.requiresBand] === "boolean")
      && c.unresolvedCreationInputs.join(",") === "band_resource_availability,band_authorized_meal,band_presence_concentration_policy";
    return availabilityValid && transportValid && mealsValid && c.resourceTransitionMinutes === 5 && bandConfigurationValid ? [] : [issue("EFFECTIVE_CONFIGURATION", "EFFECTIVE_CONFIGURATION_DRIFT", "effectiveConfiguration", "Effective A2 source configuration changed, lost an explicit decision, or acquired a human fixed meal interval.")];
  }],
