...
test("the audit builder is serializable, explicit, and accepts expected false gates", () => {
  ...
  assert.equal(output.acceptance.focalCurrentRequiredComplete, false);
  assert.equal(output.acceptance.focalCurrentRequiredPlanPublished, false);
  assert.equal(output.acceptance.fullBandBenchmarkPassed, false);
  assert.equal(Object.keys(output.scenarios).length, 24);
});
