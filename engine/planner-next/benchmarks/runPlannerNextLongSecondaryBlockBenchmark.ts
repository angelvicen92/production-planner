...
function logical(factory:()=>PlannerNextProblem, units=false) {
  ...
  const secondary=result.scheduledTasks.filter((task)=>task.spaceId==="long-form-room").sort((a,b)=>a.start-b.start||a.id.localeCompare(b.id));
  return { complete:result.complete, hardValid:m.hardValid, plannedTaskCount:m.plannedTaskCount, mainFlowStart:m.mainFlowStart, mainFlowEnd:m.mainFlowEnd,
    ...
    auxiliaryCandidateCountWhenSelectedByTaskId:m.auxiliaryCandidateCountWhenSelectedByTaskId, auxiliaryWorkItemSelectionOrder:m.auxiliaryWorkItemSelectionOrder, secondaryBlockCandidateCountWhenSelectedBySpaceId:m.secondaryBlockCandidateCountWhenSelectedBySpaceId, secondarySpaceStartById:m.secondarySpaceStartById, secondarySpaceEndById:m.secondarySpaceEndById, secondarySpaceGapMinutesById:m.secondarySpaceGapMinutesById, secondarySpaceBlockCountById:m.secondarySpaceBlockCountById, secondaryBlockBranchesExplored:m.secondaryBlockBranchesExplored,
    ...
    secondaryBlockView: secondary.length ? { taskIds:secondary.map(({id})=>id), participantIds:secondary.map(({participantId})=>participantId), start:secondary[0]!.start, end:secondary.at(-1)!.end, gaps:secondary.slice(1).map((task,index)=>task.start-secondary[index]!.end), blockSpanMinutes:secondary.at(-1)!.end-secondary[0]!.start, flexibleTaskStarts:Object.fromEntries(result.scheduledTasks.filter((task)=>task.spaceId==="flexible-room").map((task)=>[task.id,task.start])) } : undefined,
...
const scenarios={
  ...
  auxiliaryOff:run(()=>auxiliaryScarcityScenario("OFF")), auxiliaryHigh:run(()=>auxiliaryScarcityScenario("HIGH")), itinerantUnits:run(itinerantUnitsScenario,true), longSecondaryBlock:run(longSecondaryBlockScenario),
};
...
const itinerant=scenarios.itinerantUnits, long=scenarios.longSecondaryBlock;
const accepted=frozen && itinerant.planFingerprint==="eed9c26041aba86398c0c898940f5dfd9cf0d5571209cadaafd8ce49ac55e9e3" && scenarios.adversarial.backtracks>=1 && scenarios.adversarialZeroBacktracks.searchStopReason==="BACKTRACK_BUDGET_EXHAUSTED"
 ...
 && long.complete && long.hardValid && long.plannedTaskCount===22 && long.secondarySpaceGapMinutesById["long-form-room"]===0 && long.secondarySpaceBlockCountById["long-form-room"]===1 && long.auxiliaryWorkItemSelectionOrder[0]==="space:long-form-room"
 && Object.values(scenarios).every((x)=>x.deterministic&&x.runtimeMs<2000&&(x.complete?x.violationCount===0:true));
process.stdout.write(JSON.stringify({version:"planner-next-long-secondary-block-v1",scenarios,acceptance:{accepted,frozenFingerprints:frozen}},null,2)+"\n");
