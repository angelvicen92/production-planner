...
const problem=():PlannerNextProblem=>{const value=structuredClone(canonical.problem);for(const task of value.tasks){
  if([conflictA.id,conflictB.id,"C02.prueba_vocal_lucia"].includes(task.id))task.dependencies=[];
  if(task.id===scopeTask.id)task.dependencies=["C02.prueba_vocal_lucia"];
}value.spaces=value.spaces.map(space=>({id:space.id,availability:space.availability}));
value.resources=value.resources.map(resource=>({id:resource.id,availability:resource.availability,presencePreference:"OFF"}));
value.anchoredAccompaniments=[];value.technicalChains=[];value.roundSynchronizations=[];value.participantMeals=[];
value.resourceMeals=[];value.itinerantUnitMeals=[];value.operationalMealPolicies=[];delete value.transportPolicy;
value.participantTransitionMinutes=0;value.coachRouteTransitions=[];
return value;};
...
const conflictA=canonical.problem.tasks.find(task=>task.id==="C01.croma")!;
const conflictB=canonical.problem.tasks.find(task=>task.id==="C01.giratuto")!;
const scopeTask=canonical.problem.tasks.find(task=>task.id==="C02.ensayo_estudio_7")!;
const validationProblem=():PlannerNextProblem=>({day:canonical.problem.day,spaces:canonical.problem.spaces,resources:canonical.problem.resources,
...
export async function runA2Assist6Evidence(){
  ...
  const service=new AssistedPlanningService(storage,rpc as any,{buildInput:async()=>({} as any),buildConfigRevision:()=>({configurationFingerprint:"c".repeat(64)} as any),validateManual:createManualDeltaValidationHarness(validationProblem(),identity)});
  ...
  const requiredProblem=validationProblem();requiredProblem.tasks=requiredProblem.tasks.map(task=>({...task,requiredResourceIds:[...new Set([...(task.requiredResourceIds??[]),"cam-2"])]}));
  ...
  return {benchmark:"A2-ASSIST-6",sourceObligationCount,validationMode:hardValidation.reportJson.mode,hardCount:hardValidation.hardCount,requiredCount:requiredViolations.length,productWritesBeforeAccept:beforeAccept,dailyTasksWritesAtAccept:dailyWrites,acceptedExceptionCount:exceptions.length,proposalExists:Boolean(proposal.proposal),acceptedFollowupStageId:s3.id,inheritedAcceptedHardViolationCount:inherited.length,newHardViolationCount:projected.length-inherited.length,inheritedAcceptedRequiredViolationCount:inheritedRequired.length,newRequiredViolationCount:requiredViolations.length-inheritedRequired.length,exceptionSurvivesValidateReset:true,affectedTaskReconfirmationDemonstrated:validation.reportJson.newHardCount>=1};
}
