...
export async function runA2Assist6Evidence(){
  ...
  const requiredStage={id:99,parentStageId:null,snapshotJson:requiredSnapshot};
  const requiredExceptions=requiredViolations.map((item,index)=>({id:100+index,severity:"REQUIRED",violationKey:item.violationKey,affectedTaskIdsJson:item.affectedTaskIds}));
  const inheritedRequired=requiredExceptions.filter(item=>requiredViolations.some(violation=>violation.severity===item.severity&&violation.violationKey===item.violationKey)&&affectedTasksUnchanged(requiredStage.snapshotJson,requiredSnapshot,item.affectedTaskIdsJson));
  ...
  assert.equal(requiredExceptions.filter(item=>affectedTasksUnchanged(requiredStage.snapshotJson,movedRequired,item.affectedTaskIdsJson)).length,0);
