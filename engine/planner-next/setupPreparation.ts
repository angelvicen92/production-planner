import type { PlannerNextProblem, ScheduledSetupPreparation, ScheduledTask, SetupPolicy, Window } from "./contracts";
...
export function spaceOccupations(tasks:ScheduledTask[], preparations:ScheduledSetupPreparation[], spaceId:string):TemporalOccupation[]{return [...tasks.filter(x=>x.spaceId===spaceId),...preparations.filter(x=>x.spaceId===spaceId)].sort((a,b)=>a.start-b.start||a.end-b.end||a.id.localeCompare(b.id));}
