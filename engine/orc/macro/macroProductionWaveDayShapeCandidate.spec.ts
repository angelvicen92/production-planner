...
test("preflight captura outside work day antes del prefilter global",()=>{ const st=planState(); st.workDay={start:"09:00",end:"09:40"}; const r=buildMacroProductionWaveDayShapeCandidates({operationalState:st as any}); assert.equal(r.summary.candidateCount,0); assert.equal(r.summary.candidatePreflightRejectedCount,1); assert.ok(r.summary.rejectedCandidateDetails.some((d:any)=>d.reason==="outside-work-day" && d.proposedStart && d.proposedEnd && d.allowedWindow)); });
