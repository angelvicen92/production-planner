...
test("rejects locked/done prerequisites",()=>{
 assert.equal(buildProductionWaveDependencyBundleCandidates({operationalState:state(true)}).summary.rejectionReasons[0],"dependency_closure_incomplete");
 assert.equal(buildProductionWaveDependencyBundleCandidates({operationalState:state(false,"done")}).summary.rejectionReasons[0],"dependency_closure_incomplete");
});
...
test("rejects protected transitive prerequisite with explainable closure details",()=>{
 ...
 assert.deepEqual(r.summary.rejectionReasons,["dependency_closure_incomplete"]);
