...
export interface InitialConstructionAnchorTemporalCandidateFrontierSource { kind: InitialConstructionAnchorTemporalCandidateSourceKind | string; taskId: number | null; boundary: "start" | "end" | "candidate-start" | "candidate-end"; time: string; readOnly: true }
export interface InitialConstructionAnchorTemporalCandidate { windowIndex: number; candidateRankWithinWindow: number; sourceKinds: InitialConstructionAnchorTemporalCandidateSourceKind[]; frontierSources: InitialConstructionAnchorTemporalCandidateFrontierSource[]; startPlanned: string; endPlanned: string; fingerprint: string; readOnly: true }
...
type FrontierSourceDraft={kind:InitialConstructionAnchorTemporalCandidateSourceKind;taskId:number|null;boundary:"start"|"end"|"candidate-start"|"candidate-end";time:string};
function add(map:Map<number,FrontierSourceDraft[]>, start:number|null, source:InitialConstructionAnchorTemporalCandidateSourceKind, taskId:number|null=null, boundary:"start"|"end"|"candidate-start"|"candidate-end"="candidate-start"){ if(start==null||!Number.isFinite(start)) return; const list=map.get(start)??[]; list.push({kind:source,taskId:taskId==null?null:Number(taskId),boundary,time:hh(start)}); map.set(start,list); }
export function generateInitialConstructionAnchorTemporalCandidates(args:{input:EngineInput; anchorTask:TaskLike; provisionalWindow:{start:string;end:string}; provisionalAssignments?:readonly CandidateAssignment[]; originOperationalState?:OperationalState; maxCandidates?:number; windowIndex?:number; dependencyTemporalBounds?: InitialConstructionDependencyTemporalBounds | null}): InitialConstructionAnchorTemporalCandidate[]{
  ...
  for(const b of args.dependencyTemporalBounds?.prerequisiteFinishBounds??[]) add(starts,b.minutes,"assigned-prerequisite-end",Number(b.taskId));
  for(const b of args.dependencyTemporalBounds?.dependentStartBounds??[]) add(starts,b.minutes-d,"assigned-dependent-start",Number(b.taskId));
  for(const a of [...(args.provisionalAssignments??[]), ...((args.originOperationalState?.planning??[]).map((p:any)=>({taskId:p.taskId,startPlanned:p.startPlanned,endPlanned:p.endPlanned,spaceId:p.spaceId??null,resourceIds:p.assignedResourceIds??[]})) as CandidateAssignment[])].filter(a=>affects(a,args.anchorTask,args)).sort((a,b)=>Number(a.taskId)-Number(b.taskId)||String(a.startPlanned).localeCompare(String(b.startPlanned)))){ const s=min(a.startPlanned), e=min(a.endPlanned); add(starts,s==null?null:s-d,"provisional-assignment-start",Number(a.taskId)); add(starts,e,"provisional-assignment-end",Number(a.taskId)); }
  const candidates=[...starts.entries()].filter(([s])=>s>=ws&&s+d<=we&&(depEarliest==null||s>=depEarliest)&&(depLatest==null||s+d<=depLatest)).sort((a,b)=>{ const kinds=new Set(a[1].map(x=>x.kind)), kindsB=new Set(b[1].map(x=>x.kind)); const ha=kinds.has("historical-end-aligned")?0:1, hb=kindsB.has("historical-end-aligned")?0:1; return ha-hb||a[0]-b[0]; }).slice(0,args.maxCandidates??DEFAULT_INITIAL_CONSTRUCTION_ANCHOR_EXPLORATION_BUDGET.maxTemporalCandidatesPerAnchor).map(([s,sources],i)=>{ const sourceKinds=[...new Set(sources.map(x=>x.kind))].sort() as InitialConstructionAnchorTemporalCandidateSourceKind[]; const frontierSources=[...new Map(sources.map((x)=>[stableStringify({kind:x.kind,taskId:x.taskId,boundary:x.boundary,time:x.time}),{kind:x.kind,taskId:x.taskId,boundary:x.boundary,time:x.time,readOnly:true as const}])).values()].sort((a,b)=>String(a.kind).localeCompare(String(b.kind))||Number(a.taskId??-1)-Number(b.taskId??-1)||String(a.time).localeCompare(String(b.time))); const c={windowIndex:args.windowIndex??0,candidateRankWithinWindow:i,sourceKinds,frontierSources,startPlanned:hh(s),endPlanned:hh(s+d),fingerprint:"",readOnly:true as const}; c.fingerprint=hash({...c,fingerprint:undefined}); return c; });
