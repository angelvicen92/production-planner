...
export function generateInitialConstructionAnchorTemporalCandidates(args:{input:EngineInput; anchorTask:TaskLike; provisionalWindow:{start:string;end:string}; provisionalAssignments?:readonly CandidateAssignment[]; originOperationalState?:OperationalState; maxCandidates?:number; windowIndex?:number; dependencyTemporalBounds?: InitialConstructionDependencyTemporalBounds | null}): InitialConstructionAnchorTemporalCandidate[]{
  ...
  const candidates=[...starts.entries()].filter(([s])=>s>=ws&&s+d<=we&&(depEarliest==null||s>=depEarliest)&&(depLatest==null||s+d<=depLatest)).sort((a,b)=>{ const ha=a[1].has("historical-end-aligned")?0:1, hb=b[1].has("historical-end-aligned")?0:1; return ha-hb||a[0]-b[0]; }).slice(0,args.maxCandidates??DEFAULT_INITIAL_CONSTRUCTION_ANCHOR_EXPLORATION_BUDGET.maxTemporalCandidatesPerAnchor).map(([s,sources],i)=>{ const sourceKinds=[...sources].sort() as InitialConstructionAnchorTemporalCandidateSourceKind[]; const frontierSources=sourceKinds.map((kind)=>({kind,taskId: kind==="assigned-prerequisite-end" ? (args.dependencyTemporalBounds?.prerequisiteFinishBounds?.find((b)=>b.minutes===s)?.taskId ?? null) : kind==="assigned-dependent-start" ? (args.dependencyTemporalBounds?.dependentStartBounds?.find((b)=>b.minutes-d===s)?.taskId ?? null) : null,boundary:"candidate-start" as const,time:hh(s),readOnly:true as const})).sort((a,b)=>String(a.kind).localeCompare(String(b.kind))||Number(a.taskId??0)-Number(b.taskId??0)); const c={windowIndex:args.windowIndex??0,candidateRankWithinWindow:i,sourceKinds,frontierSources,startPlanned:hh(s),endPlanned:hh(s+d),fingerprint:"",readOnly:true as const}; c.fingerprint=hash({...c,fingerprint:undefined}); return c; });
