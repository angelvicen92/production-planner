...
import { buildCriticalResourceIdleCompressionCandidates, type CriticalResourceIdleCompressionSummary } from "./criticalResourceIdleCompressionCandidateBuilder";
...
const emptyMainZoneContinuitySummary = (): MainZoneContinuitySummary => ({ mainZoneConfigured: false, mainZoneId: null, mainZoneSource: null, configured: false, rawMainZoneId: null, targetKind: "unknown", mainSpaceIds: [], mainZoneIds: [], dominantMainSpaceId: null, mainEntryCount: 0, mainProductiveEntryCount: 0, resolutionWarnings: ["main_zone_not_configured"], mainZoneResolutionContractVersion: ORC_MAIN_ZONE_RESOLUTION_CONTRACT_VERSION_ID228, mainZoneGapCount: 0, largestMainZoneGapMinutes: 0, mainZoneGapCandidatesAvailable: false, mainZoneContinuityContractVersion: "ORC-MAIN-ZONE-CONTINUITY-ID226", gaps: [], readOnly: true });
const emptyMainZoneGapResourceBlockSwapSummary = (skippedReason: string | null = "not_executed"): MainZoneGapResourceBlockSwapSummary => ({ executed: false, generatedCandidateCount: 0, candidateIds: [], selectedAsBest: false, selectedAsCommit: false, movedMainZoneTaskIds: [], movedBlockingTaskIds: [], sharedResourceIds: [], gapMinutesBefore: null, expectedGapMinutesAfter: null, candidateStateCount: 0, simulatedStateCount: 0, validSimulationCount: 0, invalidSimulationCount: 0, prefilterDiscardReasons: {}, prefilterDiscardDetails: [], skippedReason, candidateGenerationReason: skippedReason, candidateGenerationBlockers: skippedReason ? [skippedReason] : [], readOnly: true });
...
const emptyCriticalResourceIdleCompressionSummary = (reason: string | null = "not_executed"): CriticalResourceIdleCompressionSummary => ({ executed: false, reason, targetResourceIds: [], targetGapCount: 0, generatedCandidateCount: 0, candidateIds: [], prefilterAcceptedCount: 0, prefilterDiscardedCount: 0, candidateStateCount: 0, simulatedStateCount: 0, validSimulationCount: 0, invalidSimulationCount: 0, selectedAsCommit: false, selectedCandidateId: null, selectedSimulatedStateId: null, movedTaskIds: [], targetResourceId: null, targetResourceIdleBefore: null, targetResourceIdleAfter: null, targetResourceIdleReductionMinutes: 0, targetGapBeforeMinutes: null, targetGapAfterMinutes: null, mainZoneContinuityPreserved: true, makespanBefore: null, makespanAfter: null, prefilterDiscardReasons: {}, prefilterDiscardDetails: [], candidateGenerationBlockers: reason ? [reason] : [], validationRejectReasons: [], warnings: [], readOnly: true });
...
export function buildCandidates(searchSpaces: SearchSpace[], options: CandidateBuilderOptions = {}): CandidateBuilderResult {
  ...
  const mainZoneGapResourceBlockSwap = shouldDelayMainFlow ? { candidates: [], evidence: [], summary: emptyMainZoneGapResourceBlockSwapSummary("baseline_overlap_repair_priority_initial_pass"), mainZoneContinuity: emptyMainZoneContinuitySummary() } : buildMainZoneGapResourceBlockSwapCandidates(options.operationalState ?? null, options.createdAt ?? null);
  const mainFlowGapClosure = shouldDelayMainFlow ? { candidates: [], evidence: [], summary: emptyMainFlowGapClosureSummary("baseline_overlap_repair_priority_initial_pass") } : buildMainFlowGapClosureCandidates(options.operationalState ?? null, sourceSearchSpaces, options.createdAt ?? null);
  const shouldDelayCriticalResourceIdleCompression = shouldDelayMainFlow || options.baselineSeedHardFeasibility?.hardFeasible === false;
  const criticalResourceIdleCompression = shouldDelayCriticalResourceIdleCompression ? { candidates: [], evidence: [], summary: emptyCriticalResourceIdleCompressionSummary(shouldDelayMainFlow ? "baseline_overlap_repair_priority_initial_pass" : "base_plan_hard_infeasible") } : buildCriticalResourceIdleCompressionCandidates(options.operationalState ?? null, options.createdAt ?? null);
  const generatedCandidates = [...result.candidates, ...baselineOverlapRepair.candidates, ...mainZoneGapResourceBlockSwap.candidates, ...criticalResourceIdleCompression.candidates, ...mainFlowGapClosure.candidates].map(cloneCandidate);
  const hardPrefilterResult = prefilterCandidatesByHardConstraints(generatedCandidates, options.operationalState ?? null, { createdAt: options.createdAt ?? null });
  ...
  for (const item of [...result.evidence, ...baselineOverlapRepair.evidence, ...mainZoneGapResourceBlockSwap.evidence, ...criticalResourceIdleCompression.evidence, ...mainFlowGapClosure.evidence]) {
  ...
  return {
    ...
    summary: {
      ...
      criticalResourceIdleCompression: { ...criticalResourceIdleCompression.summary, prefilterAcceptedCount: criticalResourceIdleCompression.candidates.filter((candidate) => hardPrefilterResult.candidates.some((accepted) => accepted.id === candidate.id)).length, prefilterDiscardedCount: criticalResourceIdleCompression.candidates.filter((candidate) => !hardPrefilterResult.candidates.some((accepted) => accepted.id === candidate.id)).length, prefilterDiscardReasons: Object.fromEntries(hardPrefilterResult.discardedCandidates.filter((d) => criticalResourceIdleCompression.candidates.some((c) => c.id === d.candidateId)).reduce((m, d) => m.set(d.reason, (m.get(d.reason) ?? 0) + 1), new Map<string, number>())), prefilterDiscardDetails: hardPrefilterResult.discardedCandidates.filter((d) => criticalResourceIdleCompression.candidates.some((c) => c.id === d.candidateId)).map((d) => ({ candidateId: d.candidateId, reason: d.reason, conflictingTaskIds: d.conflictingTaskIds ?? d.affectedTaskIds, spaceIds: d.spaceIds ?? [], timeWindow: d.timeWindow ?? null, relatedTimeWindow: d.relatedTimeWindow ?? null, roleLabels: d.roleLabels ?? [], spaceOccupancyModes: d.spaceOccupancyModes ?? [], diagnosticHint: d.diagnosticHint ?? null })) },
