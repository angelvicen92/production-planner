...
import { evaluateResourceIdleNetValue, ORC_RESOURCE_IDLE_NET_VALUE_CONTRACT_VERSION_ID234, RESOURCE_IDLE_NET_VALUE_REJECTION_REASON } from "../evaluation/resourceIdleNetValueEvaluator";

export const ORC_POST_CONTINUITY_RESOURCE_IDLE_PASS_VERSION_ID231 = "ORC-POST-CONTINUITY-RESOURCE-IDLE-PASS-ID231" as const;
export const ORC_RESOURCE_IDLE_COMPOSITE_SELECTION_POLICY_ID231 = "valid-committed-continuity-and-net-positive-resource-compactness-first-v1" as const;

export type CriticalResourceIdleCompressionExecutionPhase = "initial-pass" | "post-continuity-pass" | "not-executed";
export type CriticalResourceIdleCompressionPassSummary = CriticalResourceIdleCompressionSummary & Record<string, unknown> & {
  ...
  readonly netValue?: Record<string, unknown>;
...
export function runCriticalResourceIdleCompressionPass(args: { originalState: OperationalState; baseSimulation: SimulatedState | null; baseValidation: ValidationResult | null; basePlanningMaterialization?: Rec | null; mainZoneContinuity?: Rec | null; postRepairMainZoneContinuityPass?: Rec | null; criticalResourceIdleCompressionSummaryFromInitialPass?: Rec | null; createdAt?: string | null; budget?: { maxCandidates?: number } }): CriticalResourceIdleCompressionPassResult {
  ...
  const rawSelectedWithMaterialization = rawSelected ? withCompositeMaterialization(rawSelected, args.originalState, baseState, mat, selectedCandidate) : null;
  const candidateValidation = selectedSimId ? valid.find(v=>v.simulatedStateId===selectedSimId) ?? pipeline.validation.validationResults.find(v=>v.simulatedStateId===selectedSimId) ?? null : null;
  const candidateValue = selectedSimId ? pipeline.ranking.rankedOperationalValues.find(v=>v.simulatedStateId===selectedSimId) ?? pipeline.evaluation.operationalValues.find(v=>v.simulatedStateId===selectedSimId) ?? null : null;
  const baseValue = pipeline.ranking.rankedOperationalValues.find(v=>v.simulatedStateId===args.baseSimulation?.id) ?? null;
  const netEval = rawSelectedWithMaterialization ? evaluateResourceIdleNetValue({ originalState: args.originalState, baseState, candidateState: rawSelectedWithMaterialization.operationalStateSnapshot as OperationalState, baseSimulation: args.baseSimulation, candidateSimulation: rawSelectedWithMaterialization, baseValidation: args.baseValidation, candidateValidation, basePlanningMaterialization: mat, candidatePlanningMaterialization: rawSelectedWithMaterialization.planningMaterialization as Rec, selectedCandidateMetadata: selectedCandidate?.metadata as Rec | null, baseOverallScore: baseValue?.overallScore ?? null, candidateOverallScore: candidateValue?.overallScore ?? null }) : null;
  const commitOk = rawSelectedWithMaterialization != null && netEval?.netValue.acceptedByNetValueGate === true && reduction > 0 && (afterSpan == null || beforeSpan == null || afterSpan <= beforeSpan);
  const selectedSimulation = commitOk ? rawSelectedWithMaterialization : null;
