...
import { evaluateZoneTaskChangeLimit } from "./spaceTaskChangeLimit";
...
export function runORCActivePlanner(input: EngineInput, options: ORCActivePlannerOptions = {}): ORCActivePlannerResult {
  ...
  const zoneTaskChangeLimit = evaluateZoneTaskChangeLimit({ planning: [...(simulation?.operationalStateSnapshot?.planning ?? []), ...mealBreakBlocks.blocks.filter((b:any)=>b.materializable!==false)], spaces: simulation?.operationalStateSnapshot?.spaces ?? (input as any), maxTemplateChangesByZoneId });
  const spaceTaskChangeLimit = zoneTaskChangeLimit;
  const productionConceptGate = evaluateProductionConceptNonRegressionGate({ baseline: preMacroCompositeSummary.productionConceptAlignment, selected: compositeSummary.productionConceptAlignment, mealBreakPolicy: { mealBreakDurationMinutes: mealBreakBlocks.mealBreakDurationMinutes, mealBreakBlockCoverageValid: mealBreakBlocks.blockers.length === 0 || mealBreakBlocks.mealBreakDurationMinutes === 0 || !(mealBreakBlocks.evidence as any).window } });
  ...
  gates.spaceTaskChangeLimitRespected = !selectedChangesPlanning || shadow == null || !zoneTaskChangeLimit.exceeded;
