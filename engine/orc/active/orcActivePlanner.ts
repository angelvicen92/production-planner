...
import { evaluateSpaceTaskChangeLimit } from "./spaceTaskChangeLimit";
...
export function runORCActivePlanner(input: EngineInput, options: ORCActivePlannerOptions = {}): ORCActivePlannerResult {
  ...
  const maxChangesBySpaceId = (input as any).maxTaskChangesBySpaceId ?? (input as any).maxTaskGroupChangesBySpaceId ?? (input as any).maxFormatChangesBySpaceId ?? (input as any).maxChangesPerSpace ?? {};
  const spaceTaskChangeLimitConfigured = Object.keys(maxChangesBySpaceId ?? {}).length > 0;
  const spaceTaskChangeLimit = evaluateSpaceTaskChangeLimit({ planning: [...(simulation?.operationalStateSnapshot?.planning ?? []), ...mealBreakBlocks.blocks], spaces: simulation?.operationalStateSnapshot?.spaces ?? null, maxChangesBySpaceId });
  ...
  gates.spaceTaskChangeLimitRespected = !selectedChangesPlanning || shadow == null || !spaceTaskChangeLimitConfigured || !spaceTaskChangeLimit.exceeded;
