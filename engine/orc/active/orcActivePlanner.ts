...
export function runORCActivePlanner(input: EngineInput, options: ORCActivePlannerOptions = {}): ORCActivePlannerResult {
  ...
  if (macroMainZoneBlockRelayoutPass.selectedSimulation && macroMainZoneBlockRelayoutPass.selectedValidation) {
    simulation = macroMainZoneBlockRelayoutPass.selectedSimulation;
    validation = macroMainZoneBlockRelayoutPass.selectedValidation;
    value = value;
    candidateState = null;
    candidate = macroMainZoneBlockRelayoutPass.candidates.find((c) => c.id === macroMainZoneBlockRelayoutPass.summary.selectedCandidateId) ?? candidate;
    selection = { ...selection, simulation, validation, candidateState, candidate, diagnostics: { ...selection.diagnostics, selectedSimulatedStateId: simulation.id, selectedFinalCandidateFamily: "macro-main-zone-block-relayout", selectedBecause: "valid-committed-macro-main-zone-relayout-first-v1" } as any };
  }
