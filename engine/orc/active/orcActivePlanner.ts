...
import { compactProductionConceptComparison, selectORCIncumbent, type ORCIncumbentCandidate } from "./selectORCIncumbent";
...
export function runORCActivePlanner(input: EngineInput, options: ORCActivePlannerOptions = {}): ORCActivePlannerResult {
  ...
  let incumbent: ORCIncumbentCandidate | null = simulation ? {
  ...
  const macroMainZoneBlockRelayoutPass = runMacroMainZoneBlockRelayoutPass({ originalState: shadow?.operationalState ?? (simulation?.operationalStateSnapshot as any), baseSimulation: simulation, baseValidation: validation, basePlanningMaterialization: preMacroCompositeSummary.planningMaterialization as any, productionWavePlanner: preMacroCompositeSummary.productionWavePlanner as any, productionConceptAlignment: preMacroCompositeSummary.productionConceptAlignment as any, createdAt: null });
  const postMacroSelectionInput = buildPostMacroORCSelectionInput({ shadow, macroMainZoneBlockRelayoutPass, preMacroSelection: selection, preMacroCompositeSummary: preMacroCompositeSummary as any });
  const postMacroSelection = selectBestORCSimulation(postMacroSelectionInput);
  ...
  const incumbentDecision = selectORCIncumbent({ incumbent, candidate: postMacroCandidate, mealBreakPolicy: { mealBreakDurationMinutes: postMacroMealBreakBlocks.mealBreakDurationMinutes, mealBreakBlockCoverageValid: postMacroMealBreakBlocks.blockers.length === 0 || postMacroMealBreakBlocks.mealBreakDurationMinutes === 0 || !(postMacroMealBreakBlocks.evidence as any).window } });
