...
function placeOneOptions(args:any, current:CandidateAssignment[], taskId:number, maxBranches:number){
  ...
  const options:any[]=[];
  for(const opt of attempt.selectable??[]){ const byId=new Map(current.map(a=>[a.taskId,a])); for(const a of opt.branch.assignments.map(norm)) byId.set(a.taskId,a); const combined=[...byId.values()].sort((a,b)=>a.taskId-b.taskId); if(new Set(combined.map(a=>a.taskId)).size!==combined.length) continue; if(validateCombined(args,combined,`repair-neighborhood:${taskId}`)==="VALID") options.push({ok:true,combined,attempt,closure,branchFingerprint:hash(opt.branch)}); }
  return options.length?options:[{ok:false,combined:current,attempt,closure,failedTaskId:taskId}];
}
function placeOne(args:any, current:CandidateAssignment[], taskId:number, maxBranches:number){ return placeOneOptions(args,current,taskId,maxBranches)[0]; }
export function runInitialConstructionRepairNeighborhoodSession(args:{ originInput:EngineInput; originOperationalState:OperationalState; canonicalContext:any; combinedPartialPlan:any; repairProblem:any; ejectionSet:any; blockedAnchorTaskId:number; localBudget?:any; stage1?:any }){
  ...
  const node={rootCandidateProfileFingerprint:args.ejectionSet?.candidateProfileFingerprint??null,cumulativeEjectedTaskIds:[...(args.ejectionSet?.ejectedTaskIds??[])].map(Number),cumulativeRepairDependencyClosureTaskIds:[...removed].map(Number).sort((a:number,b:number)=>a-b),repairNeighborhoodTaskIds:neighborhood,repairNeighborhoodTopologicalTaskIds:(args.canonicalContext?.topologicalTaskIds??neighborhood).filter((id:number)=>neighborhood.includes(id)),baseAssignmentsFingerprint:initialConstructionAssignmentFingerprint(before),failedTaskId:null,failurePhase:null,expansionDepth:0,parentNodeFingerprint:null,fingerprint:"",readOnly:true}; (node as any).fingerprint=hash({...node,fingerprint:undefined});
  const first=placeOne(args,current,Number(args.blockedAnchorTaskId),maxBranches); attempts++; validations++;
  if(!first.ok){ const audit=auditInitialConstructionRepairNeighborhoodPreservation({before,after:before,repairNeighborhoodTaskIds:neighborhood,protectedTaskIds:args.repairProblem?.protectedTaskIds??[],productiveTaskIds:(args.originInput.tasks??[]).map((t:any)=>Number(t.id))}); return deepFreeze({version:"INITIAL-CONSTRUCTION-REPAIR-NEIGHBORHOOD-SESSION-V1",executed:true,accepted:false,combinedPartialPlan:args.combinedPartialPlan,anchorAssigned:false,allNeighborhoodReinserted:false,acceptedCycleCount:0,anchorAttemptCount:attempts,validationCount:validations,audit,failurePhase:"BLOCKED_ANCHOR_RECONSTRUCTION",failedTaskId:Number(args.blockedAnchorTaskId),failedRepairCandidateProfiles:args.repairProblem?.repairCandidateProfiles??[],failedRepairCandidateProfilesFingerprint:args.repairProblem?.repairCandidateProfilesFingerprint??null,failedRepairableCandidateProfileCount:args.repairProblem?.repairableCandidateProfileCount??0,failedCandidateEjectionSetCount:args.repairProblem?.candidateEjectionSets?.length??0,searchNodes:[node],searchNodeCount:1,expansionAttemptCount:1,expansionAcceptedCount:0,maxExpansionDepthObserved:0,stopReason:"ANCHOR_RECONSTRUCTION_FAILED",sessionFingerprint:hash({audit:audit.fingerprint,set:args.ejectionSet?.fingerprint}),readOnly:true}) as any; }
  current=first.combined; cycles++;
  const remainingOrder=(args.canonicalContext?.topologicalTaskIds??neighborhood).filter((id:number)=>neighborhood.includes(id));
  const visited=new Set<string>(); let backtrackCount=0; let failed:any=null;
  const dfs=(plan:CandidateAssignment[]):CandidateAssignment[]|null=>{
    const next=remainingOrder.find((id:number)=>!plan.some(a=>a.taskId===id)); if(next==null) return plan;
    const key=hash({next,fp:initialConstructionAssignmentFingerprint(plan)}); if(visited.has(key)) return null; visited.add(key);
    for(const opt of placeOneOptions(args,plan,Number(next),maxBranches)){ attempts++; validations++; if(!opt.ok){ failed={taskId:next,opt}; continue; } const r=dfs(opt.combined); if(r) return r; backtrackCount++; }
    return null;
  };
  const solved=dfs(current);
  if(!solved){ const audit=auditInitialConstructionRepairNeighborhoodPreservation({before,after:before,repairNeighborhoodTaskIds:neighborhood,protectedTaskIds:args.repairProblem?.protectedTaskIds??[],productiveTaskIds:(args.originInput.tasks??[]).map((t:any)=>Number(t.id))}); return deepFreeze({version:"INITIAL-CONSTRUCTION-REPAIR-NEIGHBORHOOD-SESSION-V1",executed:true,accepted:false,combinedPartialPlan:args.combinedPartialPlan,anchorAssigned:true,allNeighborhoodReinserted:false,acceptedCycleCount:cycles,anchorAttemptCount:attempts,validationCount:validations,audit,failurePhase:"NEIGHBORHOOD_TASK_REINSERTION",failedTaskId:failed?.taskId??null,failedRepairCandidateProfiles:args.repairProblem?.repairCandidateProfiles??[],failedRepairCandidateProfilesFingerprint:args.repairProblem?.repairCandidateProfilesFingerprint??null,failedRepairableCandidateProfileCount:args.repairProblem?.repairableCandidateProfileCount??0,failedCandidateEjectionSetCount:args.repairProblem?.candidateEjectionSets?.length??0,searchNodes:[node],searchNodeCount:1,expansionAttemptCount:failed?1:0,expansionAcceptedCount:0,maxExpansionDepthObserved:0,backtrackCount,stopReason:"NEIGHBORHOOD_REINSERTION_FAILED",sessionFingerprint:hash({audit:audit.fingerprint,set:args.ejectionSet?.fingerprint}),readOnly:true}) as any; } current=solved; cycles=remainingOrder.length;
