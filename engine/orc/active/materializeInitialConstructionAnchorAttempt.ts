...
function candidateClass(p:InitialConstructionRepairCandidateProfile):"repairable"|"immutable"|"static"|"incomplete" {
  if (p.repairable) return "repairable";
  if (!p.evidenceComplete) return "incomplete";
  if (p.immutableConflictTaskIds.length || p.protectedIntervalConflictIds.length) return "immutable";
  return "static";
}
function rejectedTemporalCandidateEvidenceFromProfile(p:InitialConstructionRepairCandidateProfile){
  const payload:any={...p,anchorTaskId:p.blockedAnchorTaskId,classification:candidateClass(p),repairableObserved:p.repairable,staticObserved:p.staticUnrepairableReasonCodes.length>0||p.staticWindowConflictCount>0,immutableObserved:p.immutableConflictTaskIds.length>0||p.protectedIntervalConflictIds.length>0,incompleteObserved:!p.evidenceComplete,readOnly:true};
  payload.fingerprint=createHash("sha256").update(stableStringify({...payload,fingerprint:undefined})).digest("hex");
  return deepFreeze(payload) as any;
}
function selectRepresentativeRejectedTemporalCandidate(candidates:any[]):any|null{
  return [...candidates].sort((a,b)=>{
    const score=(x:any)=>x.repairable?0:(x.repairableConflictTaskIds?.length&&x.windowConflictCausalLinks?.some((l:any)=>(l.reversibleSourceTaskIds??[]).length)?1:x.immutableObserved?2:x.staticObserved&&x.evidenceComplete?3:!x.evidenceComplete?4:5);
    return score(a)-score(b)||Number(a.candidateRank)-Number(b.candidateRank)||String(a.fingerprint).localeCompare(String(b.fingerprint));
  })[0]??null;
}
function boundedRejectedTemporalCandidateEvidence(profiles:InitialConstructionRepairCandidateProfile[], limit=SAMPLE_LIMIT){
  const all=profiles.map(rejectedTemporalCandidateEvidenceFromProfile); const selected=new Map<string,any>();
  const add=(x:any)=>{ if(x&&!selected.has(x.fingerprint)&&selected.size<limit) selected.set(x.fingerprint,x); };
  all.filter(x=>x.repairable).sort((a,b)=>a.candidateRank-b.candidateRank||a.fingerprint.localeCompare(b.fingerprint)).forEach(add);
  for(const cls of ["repairable","static","immutable","incomplete"]){ add(all.filter(x=>x.classification===cls).sort((a,b)=>a.candidateRank-b.candidateRank||a.fingerprint.localeCompare(b.fingerprint))[0]); }
  const byCausal=new Set<string>(); for(const x of all.filter(x=>x.staticObserved).sort((a,b)=>a.candidateRank-b.candidateRank||a.fingerprint.localeCompare(b.fingerprint))){ const key=createHash("sha256").update(stableStringify({reasons:x.staticUnrepairableReasonCodes,links:x.windowConflictCausalLinks,details:x.taskWindowConflictDetails})).digest("hex"); if(!byCausal.has(key)){byCausal.add(key); add(x);} }
  add(selectRepresentativeRejectedTemporalCandidate(all));
  return {all, samples:[...selected.values()].sort((a,b)=>a.candidateRank-b.candidateRank||a.fingerprint.localeCompare(b.fingerprint)), selected:selectRepresentativeRejectedTemporalCandidate(all)};
}
...
export function materializeInitialConstructionAnchorAttempt(args: { originInput: EngineInput; originOperationalState: OperationalState; stage: any; anchor?: any | null; baseProvisionalAssignments?: readonly CandidateAssignment[]; provisionallySatisfiedTaskIds?: readonly number[]; closureTaskIds?: readonly number[]; maxBranches?: number; reasoningBudget?: ReasoningBudgetProfile | null; createdAt?: string | null; requireFutureFeasibility?: (branch: InitialConstructionBranch) => any | null; canonicalContext?: InitialConstructionCanonicalContext | null }) {
  ...
  const rejectedTemporalCandidateEvidence = boundedRejectedTemporalCandidateEvidence(repairCandidateProfiles);
  const selectedFailureCandidate = rejectedTemporalCandidateEvidence.selected;
  ...
  return deepFreeze({ version: "MATERIALIZE-INITIAL-CONSTRUCTION-ANCHOR-ATTEMPT-V1", anchorTaskId, built, branches: built.branches, attempts, selectable, selected, repairCandidateProfiles, repairCandidateProfileCount: repairCandidateProfiles.length, repairableCandidateProfileCount: repairCandidateProfiles.filter((p)=>p.repairable).length, unrepairableCandidateProfileCount: repairCandidateProfiles.filter((p)=>!p.repairable).length, candidateProfilesWithDependencyBoundBlockers: repairCandidateProfiles.filter((p)=>p.dependencyBoundSourceTaskIds.length>0).length, repairCandidateProfilesFingerprint, diagnostics: { ...diagnostics, ...dependencyEvidence, repairCandidateProfileCount: repairCandidateProfiles.length, repairableCandidateProfileCount: repairCandidateProfiles.filter((p)=>p.repairable).length, unrepairableCandidateProfileCount: repairCandidateProfiles.filter((p)=>!p.repairable).length, candidateProfilesWithDependencyBoundBlockers: repairCandidateProfiles.filter((p)=>p.dependencyBoundSourceTaskIds.length>0).length, repairCandidateProfilesFingerprint, rejectedTemporalCandidateEvidence: rejectedTemporalCandidateEvidence.samples, rejectedTemporalCandidateEvidenceCount: rejectedTemporalCandidateEvidence.all.length, repairableRejectedTemporalCandidateCount: rejectedTemporalCandidateEvidence.all.filter((x:any)=>x.repairable).length, staticRejectedTemporalCandidateCount: rejectedTemporalCandidateEvidence.all.filter((x:any)=>x.staticObserved).length, immutableRejectedTemporalCandidateCount: rejectedTemporalCandidateEvidence.all.filter((x:any)=>x.immutableObserved).length, incompleteRejectedTemporalCandidateCount: rejectedTemporalCandidateEvidence.all.filter((x:any)=>x.incompleteObserved).length, shiftableWindowConflictCount: rejectedTemporalCandidateEvidence.all.reduce((n:number,x:any)=>n+Number(x.shiftableWindowConflictCount??0),0), staticWindowConflictCount: rejectedTemporalCandidateEvidence.all.reduce((n:number,x:any)=>n+Number(x.staticWindowConflictCount??0),0), completeTaskWindowConflictWithoutExplanationCount: rejectedTemporalCandidateEvidence.all.filter((x:any)=>x.evidenceComplete&&(x.reasonCodes??[]).includes("TASK_WINDOW_CONFLICT")&&!(x.repairableConflictTaskIds??[]).length&&!(x.staticUnrepairableReasonCodes??[]).length&&!(x.immutableConflictTaskIds??[]).length&&!(x.windowConflictCausalLinks??[]).length).length, selectedFailureCandidate, selectedFrontierFailureCandidateSamples: selectedFailureCandidate?[selectedFailureCandidate]:[] }, ...dependencyEvidence, hardValidBranchCount, attemptedBranchCount: built.branches.length, branchCount: diagnostics.branchCount, candidateBranchCount: diagnostics.candidateBranchCount, transformationsExecuted, simulationsExecuted, validationsExecuted, readOnly: true }) as any;
}
