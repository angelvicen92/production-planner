...
test("Opportunity Detection Engine detects one opportunity", () => {
  ...
  const enriched = { ...analysis, resourcePressure: { totalResourceCount: 2, resourceIds: [10, 20], assignedResourceIds: [10], overloadedResourceIds: [10], plannedTaskIdsByResourceId: { 10: [2, 1] } } };
...
test("Opportunity Detection Engine detects multiple opportunity classes deterministically ordered", () => {
  const analysis = {
    ...emptyAnalysis(),
    resourcePressure: { totalResourceCount: 2, resourceIds: [10, 20], assignedResourceIds: [20, 10], overloadedResourceIds: [20], plannedTaskIdsByResourceId: { 10: [3], 20: [2, 1] } },
...
test("Opportunity Detection Engine avoids duplicates from multiple bottlenecks in the same category", () => {
  const analysis = { ...emptyAnalysis(), resourcePressure: { totalResourceCount: 2, resourceIds: [10, 20], assignedResourceIds: [20, 10], overloadedResourceIds: [20, 10], plannedTaskIdsByResourceId: { 10: [3], 20: [2, 1] } } };
  const result = detectOpportunities(analysis, analyzeCriticalBottlenecks(analysis));
  assert.equal(result.opportunities.length, 1);
  assert.equal(result.opportunities[0].kind, "RESOURCE_PRESSURE");
...
test("Opportunity Detection Engine does not mutate its input", () => {
  const analysis = { ...emptyAnalysis(), resourcePressure: { totalResourceCount: 1, resourceIds: [3], assignedResourceIds: [1], overloadedResourceIds: [1], plannedTaskIdsByResourceId: { 1: [2, 1] } } };
