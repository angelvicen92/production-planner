...
function aggregatePriorities(reports: ImprovementOpportunityReport[]): ActiveOptimizationPriority[] {
  ...
  for (const report of reports) {
    for (const opportunity of report.opportunities) {
      ...
      if ((opportunity.objectiveJustification.includes("baseline_seed_hard_infeasible_blocks_candidate_optimization") || opportunity.objectiveJustification.includes("baseline_seed_unavailable_blocks_candidate_optimization")) && (opportunity.metric === "candidatesGenerated" || opportunity.metric === "candidatesSimulated" || opportunity.metric === "candidatesConsolidated" || opportunity.metric === "simulations")) continue;
