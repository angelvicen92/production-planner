...
import { runMacroMainZoneBlockRelayoutPass } from "../active/runMacroMainZoneBlockRelayoutPass";
...
export function runORCShadowMode(
  ...
  const macroBaseSimulation = postContinuityResourceIdlePass.summary.selectedAsCommit && postContinuityResourceIdlePass.selectedSimulation ? postContinuityResourceIdlePass.selectedSimulation : resourceIdleBaseSimulation;
  const macroBaseValidation = macroBaseSimulation?.id === postContinuityResourceIdlePass.selectedSimulation?.id ? postContinuityResourceIdlePass.selectedValidation : resourceIdleBaseValidation;
  const macroBaseCompositeSummary = buildFinalORCCompositeSummary({ originalState: operationalState, repairedState: selectedRepairSimulationForPostRepair?.operationalStateSnapshot ?? null, selectedSimulation: macroBaseSimulation, initialMainZoneContinuity: candidateResult.summary.mainZoneContinuity as unknown as Record<string, unknown>, mainZoneGapResourceBlockSwap: mainZoneGapResourceBlockSwapSummary as Record<string, unknown>, postRepairMainZoneContinuityPass: postRepairSummary as unknown as Record<string, unknown>, criticalResourceIdleCompression: criticalResourceIdleCompressionSummary as Record<string, unknown>, planningMaterialization: macroBaseSimulation?.planningMaterialization as unknown as Record<string, unknown> } as any);
  const macroMainZoneBlockRelayoutPass = runMacroMainZoneBlockRelayoutPass({ originalState: operationalState, baseSimulation: macroBaseSimulation, baseValidation: macroBaseValidation, basePlanningMaterialization: macroBaseCompositeSummary.planningMaterialization as Record<string, unknown>, productionWavePlanner: macroBaseCompositeSummary.productionWavePlanner as unknown as Record<string, unknown>, productionConceptAlignment: macroBaseCompositeSummary.productionConceptAlignment as unknown as Record<string, unknown>, createdAt });

  const decisionFeedbackLoop = buildDecisionFeedbackFromDecisions({ opportunities, operationalValues: [...rankingResult.rankedOperationalValues, ...(lateDecisionPipelineResult?.ranking.rankedOperationalValues ?? []), ...(postRepairContinuityPass.pipeline?.ranking.rankedOperationalValues ?? []), ...(postContinuityResourceIdlePass.pipeline?.ranking.rankedOperationalValues ?? []), ...(macroMainZoneBlockRelayoutPass.pipeline?.ranking.rankedOperationalValues ?? [])], commitDecisions: commitResult.commitDecisions });
  ...
  const finalSelectedSimulation = macroMainZoneBlockRelayoutPass.summary.selectedAsCommit && macroMainZoneBlockRelayoutPass.selectedSimulation ? macroMainZoneBlockRelayoutPass.selectedSimulation : (postContinuityResourceIdlePass.summary.selectedAsCommit && postContinuityResourceIdlePass.selectedSimulation ? postContinuityResourceIdlePass.selectedSimulation : (postRepairSummary.selectedAsCommit && postRepairSummary.selectedSimulatedStateId ? postRepairContinuityPass.pipeline?.simulation.simulatedStates.find((sim) => sim.id === postRepairSummary.selectedSimulatedStateId) ?? null : (selectedCommitSimulation ? [...simulationResult.simulatedStates, ...(lateDecisionPipelineResult?.simulation.simulatedStates ?? [])].find((sim) => sim.id === selectedCommitSimulation) ?? null : null)));
  const compositeSummary = buildFinalORCCompositeSummary({ originalState: operationalState, repairedState: selectedRepairSimulationForPostRepair?.operationalStateSnapshot ?? null, selectedSimulation: finalSelectedSimulation, initialMainZoneContinuity: candidateResult.summary.mainZoneContinuity as unknown as Record<string, unknown>, mainZoneGapResourceBlockSwap: mainZoneGapResourceBlockSwapSummary as Record<string, unknown>, postRepairMainZoneContinuityPass: postRepairSummary as unknown as Record<string, unknown>, criticalResourceIdleCompression: criticalResourceIdleCompressionSummary as Record<string, unknown>, macroMainZoneBlockRelayout: macroMainZoneBlockRelayoutPass.summary as Record<string, unknown>, planningMaterialization: finalSelectedSimulation?.planningMaterialization as unknown as Record<string, unknown> } as any);
  ...
  const evidence = [
    ...
    ...macroMainZoneBlockRelayoutPass.evidence,
  ...
  const preliminaryResult = {
    ...
    candidates: [...decisionInput.candidates, ...lateDecisionInputCandidates, ...postRepairContinuityPass.decisionInputCandidates, ...postContinuityResourceIdlePass.decisionInputCandidates, ...macroMainZoneBlockRelayoutPass.candidates],
    candidateStates: [...transformationResult.candidateStates, ...(lateDecisionPipelineResult?.transformation.candidateStates ?? []), ...(postRepairContinuityPass.pipeline?.transformation.candidateStates ?? []), ...(postContinuityResourceIdlePass.pipeline?.transformation.candidateStates ?? []), ...(macroMainZoneBlockRelayoutPass.pipeline?.transformation.candidateStates ?? [])],
    simulatedStates: [...simulationResult.simulatedStates, ...(lateDecisionPipelineResult?.simulation.simulatedStates ?? []), ...(postRepairContinuityPass.pipeline?.simulation.simulatedStates ?? []), ...(postContinuityResourceIdlePass.selectedSimulation ? [postContinuityResourceIdlePass.selectedSimulation] : (postContinuityResourceIdlePass.pipeline?.simulation.simulatedStates ?? [])), ...(macroMainZoneBlockRelayoutPass.selectedSimulation ? [macroMainZoneBlockRelayoutPass.selectedSimulation] : (macroMainZoneBlockRelayoutPass.pipeline?.simulation.simulatedStates ?? []))],
    validationResults: [...validationResult.validationResults, ...(lateDecisionPipelineResult?.validation.validationResults ?? []), ...(postRepairContinuityPass.pipeline?.validation.validationResults ?? []), ...(postContinuityResourceIdlePass.selectedValidation ? [postContinuityResourceIdlePass.selectedValidation] : (postContinuityResourceIdlePass.pipeline?.validation.validationResults ?? [])), ...(macroMainZoneBlockRelayoutPass.selectedValidation ? [macroMainZoneBlockRelayoutPass.selectedValidation] : (macroMainZoneBlockRelayoutPass.pipeline?.validation.validationResults ?? []))],
    operationalValues: [...rankingResult.rankedOperationalValues, ...(lateDecisionPipelineResult?.ranking.rankedOperationalValues ?? []), ...(postRepairContinuityPass.pipeline?.ranking.rankedOperationalValues ?? []), ...(postContinuityResourceIdlePass.pipeline?.ranking.rankedOperationalValues ?? []), ...(macroMainZoneBlockRelayoutPass.pipeline?.ranking.rankedOperationalValues ?? [])],
    commitDecisions: [...commitResult.commitDecisions, ...(lateDecisionPipelineResult?.commit.commitDecisions ?? []), ...(postRepairContinuityPass.pipeline?.commit.commitDecisions ?? []), ...(postContinuityResourceIdlePass.pipeline?.commit.commitDecisions ?? []), ...(macroMainZoneBlockRelayoutPass.pipeline?.commit.commitDecisions ?? [])],
    evidence,
    ...
    summary: {
      ...
      candidateCount: candidateResult.candidates.length + lateDecisionInputCandidates.length + postRepairContinuityPass.decisionInputCandidates.length + postContinuityResourceIdlePass.decisionInputCandidates.length + macroMainZoneBlockRelayoutPass.candidates.length,
      candidateStateCount: transformationResult.candidateStates.length + (lateDecisionPipelineResult?.transformation.candidateStates.length ?? 0) + (postRepairContinuityPass.pipeline?.transformation.candidateStates.length ?? 0) + (postContinuityResourceIdlePass.pipeline?.transformation.candidateStates.length ?? 0) + (macroMainZoneBlockRelayoutPass.pipeline?.transformation.candidateStates.length ?? 0),
      simulatedStateCount: simulationResult.simulatedStates.length + (lateDecisionPipelineResult?.simulation.simulatedStates.length ?? 0) + (postRepairContinuityPass.pipeline?.simulation.simulatedStates.length ?? 0) + (postContinuityResourceIdlePass.pipeline?.simulation.simulatedStates.length ?? 0) + (macroMainZoneBlockRelayoutPass.pipeline?.simulation.simulatedStates.length ?? 0),
      validCount: validationResult.summary.validCount + (lateDecisionPipelineResult?.validation.summary.validCount ?? 0) + (postRepairContinuityPass.pipeline?.validation.summary.validCount ?? 0) + (postContinuityResourceIdlePass.pipeline?.validation.summary.validCount ?? 0) + (macroMainZoneBlockRelayoutPass.pipeline?.validation.summary.validCount ?? 0),
      invalidCount: validationResult.summary.invalidCount + (lateDecisionPipelineResult?.validation.summary.invalidCount ?? 0) + (postRepairContinuityPass.pipeline?.validation.summary.invalidCount ?? 0) + (postContinuityResourceIdlePass.pipeline?.validation.summary.invalidCount ?? 0) + (macroMainZoneBlockRelayoutPass.pipeline?.validation.summary.invalidCount ?? 0),
      evaluatedCount: evaluatorResult.summary.evaluatedCount + (lateDecisionPipelineResult?.evaluation.summary.evaluatedCount ?? 0) + (postRepairContinuityPass.pipeline?.evaluation.summary.evaluatedCount ?? 0) + (postContinuityResourceIdlePass.pipeline?.evaluation.summary.evaluatedCount ?? 0) + (macroMainZoneBlockRelayoutPass.pipeline?.evaluation.summary.evaluatedCount ?? 0),
      ranking: rankingSummary,
      evaluation: evaluationSummary,
      sessionLearning: sessionLearningSummary,
      adaptivePriority: adaptivePrioritySummary,
      ...
      runtimeContract: { ...buildORCRuntimeContractID224(), postContinuityResourceIdleCompressionPassVersion: "ORC-POST-CONTINUITY-RESOURCE-IDLE-PASS-ID231", resourceIdleCompositeSelectionPolicy: "valid-committed-continuity-and-net-positive-resource-compactness-first-v1", resourceIdleNetValueContractVersion: "ORC-RESOURCE-IDLE-NET-VALUE-ID234", macroMainZoneBlockRelayoutContractVersion: "ORC-MACRO-MAIN-ZONE-BLOCK-RELAYOUT-ID238", macroMainZoneDependencyAwareRelayoutContractVersion: "ORC-MACRO-MAIN-ZONE-DEPENDENCY-AWARE-RELAYOUT-ID240", macroMainZoneGlobalNetValueContractVersion: "ORC-MACRO-MAIN-ZONE-GLOBAL-NET-VALUE-ID241", macroMainZoneMaterializationSourceContractVersion: "ORC-MACRO-MAIN-ZONE-MATERIALIZATION-SOURCE-ID241", macroMainZoneSuffixCompactionContractVersion: "ORC-MACRO-MAIN-ZONE-SUFFIX-COMPACTION-ID242", macroMainZoneDominanceGateContractVersion: "ORC-MACRO-MAIN-ZONE-DOMINANCE-GATE-ID243", macroMainZoneSuffixSummaryConsistencyContractVersion: "ORC-MACRO-MAIN-ZONE-SUFFIX-SUMMARY-CONSISTENCY-ID243" },
      baselineOverlapRepair: baselineOverlapRepairSummary,
      ...
      productionWavePlanner: compositeSummary.productionWavePlanner,
      macroPlannerBlueprintReady: compositeSummary.macroPlannerBlueprintReady,
      recommendedNextCapabilityFromBlueprint: compositeSummary.recommendedNextCapabilityFromBlueprint,
