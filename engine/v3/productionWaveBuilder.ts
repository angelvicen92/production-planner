...
export const generateProductionWaveCandidates = (input: EngineV3Input, baseline: EngineOutput): { candidates: EngineOutput[]; meta: ProductionWaveMeta } => {
  ...
  const anchors = (input.tasks ?? []).filter(t => Number(t.zoneId) === mainZone && plannedById.has(Number(t.id)) && Number(t.contestantId) > 0);
  ...
  if(!anchors.length) rejected.push("no_primary_stage_anchor_for_any_talent"); if(!moved.length&&anchors.length) rejected.push("anchored_waves_have_no_movable_tasks"); if(errors.length) rejected.push(...errors.slice(0,8).map(e=>`hard_invalid:${e}`)); if(!hard.hardValidationPassed) rejected.push(...hard.hardConstraintViolationCodes.slice(0,8).map(e=>`hard_invalid:${e}`));
