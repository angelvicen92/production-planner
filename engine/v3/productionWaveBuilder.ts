...
export const generateProductionWaveCandidates = (input: EngineV3Input, baseline: EngineOutput): { candidates: EngineOutput[]; meta: ProductionWaveMeta } => {
  ...
  const inspected = (input.tasks ?? []).filter(t => plannedById.has(Number(t.id)));
  const configuredAnchors = inspected.filter(t => Number(t.zoneId) === mainZone);
  ...
  const anchorDetectionReason = configuredAnchors.length ? "configured_main_zone" : fallbackAnchors.length ? "normalized_space_name_fallback" : "main_stage_anchor_detection_failed";
