...
BEGIN
  ...
  -- An exception is branch-local and survives unrelated edits. It becomes stale
  -- only when one of its affected task snapshots differs from the accepting Stage.
  UPDATE public.planning_accepted_exceptions exception SET status='STALE',resolved_at=now()
  FROM public.assisted_planning_stages stage
  WHERE exception.stage_id=stage.id AND exception.plan_id=p_plan_id AND exception.status='ACTIVE'
    AND EXISTS (SELECT 1 FROM jsonb_array_elements(exception.affected_task_ids_json) affected
      WHERE (SELECT task FROM jsonb_array_elements(stage.snapshot_json->'tasks') task WHERE task->>'taskId'=affected#>>'{}')
        IS DISTINCT FROM
...
DECLARE s public.assisted_planning_sessions%rowtype; v public.planning_stage_validations%rowtype; new_id bigint; next_ordinal integer; item jsonb;
BEGIN
 SELECT * INTO s FROM public.assisted_planning_sessions WHERE plan_id=p_plan_id AND status='ACTIVE' FOR UPDATE;
 IF NOT FOUND THEN RAISE EXCEPTION 'SESSION_NOT_FOUND'; END IF;
 IF s.draft_base_stage_id IS DISTINCT FROM p_expected_base THEN RAISE EXCEPTION 'STALE_BASE_STAGE'; END IF;
 ...
 IF v.hard_count>0 AND p_confirmation<>'HARD_EXCEPTIONS' THEN RAISE EXCEPTION 'HARD_CONFIRMATION_REQUIRED'; END IF;
 IF v.hard_count=0 AND v.required_count>0 AND p_confirmation<>'REQUIRED_DEVIATIONS' THEN RAISE EXCEPTION 'REQUIRED_CONFIRMATION_REQUIRED'; END IF;
 PERFORM public.assisted_apply_snapshot(p_plan_id,s.draft_snapshot_json);
