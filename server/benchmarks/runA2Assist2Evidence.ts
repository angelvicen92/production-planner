...
export async function runA2Assist2Evidence(){
  ...
  const reordered=reorderTasks(session.draftSnapshotJson,[2,1]);
  await service.patchDraft(planId,session.draftFingerprint,baseStageId,reordered.changes.map(task=>({taskId:task.taskId,startPlanned:task.startPlanned,endPlanned:task.endPlanned})));counters.manualEditCount++;
