...
function projected(c:ConfigurabilityCapability,input:EngineInput,plan:any,general:any,revision?:number,fingerprint?:string,optimizer?:any,optimizerState?:any):EffectiveConfigurationValue {
  ...
  switch(c.capabilityId){
    ...
    case "OPTIMIZATION": return {...common,value:optimizer??{editingMode:input.optimizerSnapshotEditingMode},baseline:optimizerState?.baseline_snapshot??undefined,canRestoreInherited:input.optimizerSnapshotSource==="DAY_OVERRIDE"&&!!optimizerState?.baseline_snapshot,overrideMetadata:{createdBy:optimizerState?.override_by??null,createdAt:optimizerState?.override_at??null},availability:"AVAILABLE",source:input.optimizerSnapshotSource??"UNKNOWN",fingerprint:input.optimizerSnapshotFingerprint,validationStatus:(input.optimizerCompatibilityWarnings?.length??0)>0?"INCOMPATIBLE":"VALID"};
...
export async function buildEffectiveConfigurationView(storage:IStorage,planId:number,inputBuilder:typeof buildEngineInput=buildEngineInput):Promise<EffectiveConfigurationView>{
  ...
  const [input,session,revisions,generalResult,optimizer,optimizerStateResult]=await Promise.all([inputBuilder(planId,storage),storage.getActiveAssistedPlanningSession(planId),typeof (storage as any).listPlanConfigRevisions==="function"?storage.listPlanConfigRevisions(planId):Promise.resolve([]),hasProvenance?import("./supabase").then(({supabaseAdmin})=>supabaseAdmin.from("program_settings").select("default_work_start,default_work_end,meal_start,meal_end,meal_mode").eq("id",1).single()):Promise.resolve({data:{}} as any),typeof (storage as any).getPlanOptimizerSnapshot==="function"?storage.getPlanOptimizerSnapshot(planId):Promise.resolve(undefined),hasProvenance?import("./supabase").then(({supabaseAdmin})=>supabaseAdmin.from("plan_optimizer_snapshots").select("baseline_snapshot,override_by,override_at").eq("plan_id",planId).single()):Promise.resolve({data:{}} as any)]);
  const revision=revisions.at(-1); const rawPlan=plan as any; const general=generalResult.data??{};
  const values=configurabilityRegistry.map(c=>projected(c,input,rawPlan,general,Number(rawPlan.current_config_revision_id??revision?.id) || undefined,revision?.fingerprint,optimizer,optimizerStateResult.data));
