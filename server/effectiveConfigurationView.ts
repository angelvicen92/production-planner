...
function projected(c:ConfigurabilityCapability,input:EngineInput,revision?:number):EffectiveConfigurationValue {
  ...
  switch(c.capabilityId){
    case "WORKDAY_WINDOW": return {...common,value:{start:input.workDay.start,end:input.workDay.end},availability:"AVAILABLE",source:"DAY_SNAPSHOT",validationStatus:"VALID"};
    case "GLOBAL_MEAL_BREAK": return {...common,value:{mode:input.mealMode??"global_hard_break",window:input.meal},availability:"AVAILABLE",source:"DAY_SNAPSHOT",validationStatus:"VALID"};
    ...
    case "OPTIMIZATION": return {...common,value:{editingMode:input.optimizerSnapshotEditingMode},availability:"AVAILABLE",source:"DAY_SNAPSHOT",fingerprint:input.optimizerSnapshotFingerprint,validationStatus:(input.optimizerCompatibilityWarnings?.length??0)>0?"INCOMPATIBLE":"VALID"};
