...
const base = { workDay: { start: "09:00", end: "21:00" }, zoneAvailability: { start: null, end: null }, spaceAvailability: { start: null, end: null } };
const resolve = (patch: Partial<typeof base> = {}) => resolveEffectiveSpaceAvailabilityHierarchy({ ...base, ...patch });
...
for (const [name, window, reason] of [
  ...
  ["partial", { start: "10:00" } as any, "MISSING_ZONE_WINDOW_ENDPOINT"],
...
] as const) test(`zone ${name}`, () => { const result = resolve({ zoneAvailability: window as any }); assert.equal(!result.valid && result.reason, reason); });
...
for (const [name, window, reason] of [
  ...
  ["partial", { start: "11:00" } as any, "MISSING_SPACE_WINDOW_ENDPOINT"],
...
] as const) test(`space ${name}`, () => { const result = resolve({ zoneAvailability: { start: "10:00", end: "20:00" }, spaceAvailability: window as any }); assert.equal(!result.valid && result.reason, reason); });
...
test("irrelevant names and parent ids do not influence deterministic immutable resolution", () => {
  const input: any = structuredClone(base); input.name = "anything"; input.parent_space_id = 99;
...
test("snapshot builder copies defaults, frozen zone relation, null inheritance and deterministic order", () => {
  ...
  (catalog.spaces[0] as any).zoneId = 1; assert.equal(built.spaces[1].zone_id, 2);
});
...
test("idempotent missing-row selection never overwrites existing snapshots", () => assert.deepEqual(missingSpatialSnapshots([{ zone_id: 1 }, { zone_id: 2 }], [1], "zone"), [{ zone_id: 2 }]));
