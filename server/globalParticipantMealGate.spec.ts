...
test("global gate prunes only a meal with an explicit zero-domain proof",()=>{
  const result=certifyGlobalParticipantMealGate([obligation("touched","p1"),obligation("zero","p2")],[],[meal("touched","p1",60)],[],{zeroDomainMealSourceIds:["zero"]});
  assert.equal(result.globalMealGate,"PRUNE");assert.deepEqual(result.zeroDomainMealSourceIds,["zero"]);assert.deepEqual(result.uncertifiedMealSourceIds,["zero"]);assert.equal(result.globallyCertifiedParticipantMealCount,1);
});
...
test("global gate reports general search exhaustion without inventing meal infeasibility",()=>{
  ...
  assert.equal(result.globalMealGate,"BUDGET_EXHAUSTED");assert.deepEqual(result.zeroDomainMealSourceIds,[]);assert.deepEqual(result.uncertifiedMealSourceIds,["pending"]);
});

test("global gate reports an uncertified witness as inconclusive without proof or exhaustion",()=>{
  const result=certifyGlobalParticipantMealGate([obligation("pending","p1")],[],[],[]);
  assert.equal(result.globalMealGate,"INCONCLUSIVE");assert.deepEqual(result.zeroDomainMealSourceIds,[]);
  assert.deepEqual(result.uncertifiedMealSourceIds,["pending"]);
});
