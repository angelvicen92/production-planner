...
OUTPUT="plan-27-orc-truthful-causal-branch-pruning-v1.json"
RUN_A="/tmp/plan-27-id320-run-a.json"
RUN_B="/tmp/plan-27-id320-run-b.json"
TMP_OUTPUT="/tmp/plan-27-id320-consolidated.json"
...
rm -f \
  ...
  "$RUN_B" \
  "$TMP_OUTPUT" \
  /tmp/plan-27-id319-run-a.json \
  /tmp/plan-27-id319-run-b.json \
  /tmp/plan-27-id319-consolidated.json
...
const out={id:320,deterministic,checks,runs:[a,b],baselineId318:base,readOnly:true};
