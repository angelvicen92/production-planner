#!/usr/bin/env bash
set -uo pipefail
artifact=planner-next-required-space-meal-v1.json
legacy=planner-next-space-meal-v2.json
failed=planner-next-required-space-meal-v1.failed.json
tmp1=$(mktemp); tmp2=$(mktemp)
trap 'rm -f "$tmp1" "$tmp2"' EXIT
fail(){ local reason=$1; node -e 'require("fs").writeFileSync(process.argv[1],JSON.stringify({version:"planner-next-required-space-meal-v1",accepted:false,reason:process.argv[2]},null,2)+"\n")' "$failed" "$reason"; echo "$reason" >&2; exit 1; }
...
const ac=a.acceptance,s=a.scenarios.requiredSpaceMeal,e=a.requiredSpaceMealFutureFeasibilityEvidence,historical=['jointFutureFeasibilityEvidence','setupEvidence','boundedBlockConstruction','branchHistoryInvariance','technicalChainConstructorEvidence','technicalChainFutureFeasibilityEvidence','spaceMealFutureFeasibilityEvidence'];
if(a.version!=='planner-next-required-space-meal-v1'||!ac.accepted||!ac.requiredSpaceMealAccepted||!ac.requiredSpaceMealFutureFeasibilityAccepted||Object.keys(a.scenarios).length!==19||historical.some(k=>a[k]===undefined))process.exit(3);
...
if(mode==='legacy'){const old=JSON.parse(fs.readFileSync(legacyPath));for(const [key,value] of Object.entries(old.scenarios))if(JSON.stringify(clean(value))!==JSON.stringify(clean(a.scenarios[key])))process.exit(8);for(const key of historical)if(JSON.stringify(old[key])!==JSON.stringify(a[key]))process.exit(9)}
NODE
...
NODE
  ...
  node -e 'const x=require("./planner-next-required-space-meal-v1.json");if(x.version!=="planner-next-required-space-meal-v1"||!x.acceptance.accepted)process.exit(1)' || fail PUBLISHED_ARTIFACT_INVALID
  rm -f "$legacy"
fi
rm -f "$failed"
