...
- **Algoritmo y pruebas causales:** una DFS leaf-first explora patrones y timelines canónicas; cada decisión materializa atómicamente `before → main → after + feeder`, conserva todos los starts feeder y no convierte `bestK` en poda. Fixtures sintéticos mínimos demuestran que una primera elección main por ID puede podarse por matching y que un start feeder tardío puede abandonarse en favor de otro anterior, completando ambos casos con `bestK = 1`.
- **Contabilidad corregida:** consumen el único `maxBranchExpansions`, antes de evaluarse, cada patrón, timeline, combinación y posición compuesta, candidato main, start feeder, matching completo, pareja main-posición, arista aumentante, comprobación futura y validación final de hoja. Los helpers distinguen explícitamente `BUDGET_EXHAUSTED` de ausencia de witness; `maxBacktracks` no limita esta ruta. La prueba de frontera obtiene `B = 86`: con 86 ramas completa canónicamente y con 85 devuelve agotamiento atómico y exactamente 85 ramas.
...
- **Benchmark Focal A2 corregido:** la contabilidad íntegra invalida la cifra anterior de 46.017 ramas. Dos ejecuciones deterministas e invariantes al orden agotan exactamente las 300.000 ramas disponibles durante enumeración contada de starts feeder, con 442 mains evaluados, 262.394 starts feeder, 1.584 matching checks, 1.577 podas, 73 podas sin alternativa, 1.577 backtracks, profundidad 6 y ninguna hoja completa. El resultado es diagnóstico `BRANCH_BUDGET_EXHAUSTED`, fingerprint exacto `null` y salida atómica; por tanto SPEC09-003 no se declara aceptada ni lista para merge y no se aumenta presupuesto, añade memoización o introduce otra heurística.
