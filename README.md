...
- **Tests y validación local:** los tests focales cubren jornada completa, formato/rango/orden, cruces de medianoche, rechazo de valores no string, pureza, congelación, copia/desacoplamiento del snapshot, construcción atómica de filas, recursos ad hoc e independencia de `is_available`. Se ejecutaron `npm ci`, `npm run check`, `npm run build`, `npx tsx --test server/resourceAvailabilityWindow.spec.ts`, `npm test` y `git diff --check`. La migración se revisó estáticamente; no se aplicó contra una base de datos local porque el repositorio no proporciona en esta validación un entorno DB configurado.
