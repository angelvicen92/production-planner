...
- **Corrección R1 y separación arquitectónica:** congelar la primera hoja core era incorrecto: sus 44 posiciones son una rama provisional, no locks. El runner interno exacto acepta ahora una continuación triestado en cada hoja core hard-valid; la API aislada usa la continuación por defecto `ACCEPT` y conserva su primera hoja, mientras la composición itinerante ejecuta la DFS standalone y devuelve `REJECT` para que el backtracking core normal pruebe la alternativa siguiente. No existe búsqueda conjunta main/standalone ni una heurística global nueva.
- **Presupuesto único y atomicidad:** un ledger pequeño comparte el límite de 300.000 y atribuye cada consumo a `CORE` o `STANDALONE`, sin reinicios ni devolución de ramas. La DFS standalone conserva enumeración completa, selección dinámica por número de starts, vencimiento, duración, recursos e ID, y scorer compartido sólo como ordenación. Una hoja standalone completa consume su rama y pasa por `validatePlan`; forma no soportada, inviabilidad o agotamiento publican tareas y comidas vacías.
- **Evidence y prueba causal:** se registran hojas core evaluadas/rechazadas, invocaciones standalone, blockers exclusivamente diagnósticos, ramas acumuladas, backtracks/profundidades por fase, fase de agotamiento, fingerprint core seleccionado y orden real de selección standalone. Un escenario sintético con `bestK = 1` demuestra que la primera hoja aislada bloquea una standalone, la continuación la rechaza, el core retrocede a otro start feeder y el planning completo queda hard-valid. La frontera integrada B/B−1 usa el mismo escenario y demuestra contabilidad exacta y fallo atómico.
- **Resultado Focal A2 R1:** la capacidad core aislada sigue `COMPLETE` con 44 tareas, 9 pendientes, 47.482 ramas y fingerprint `c85b9b2cfbbf9434135f08e2b293b0ab6c23e5ff070e880cbb5406022dd52785`. La ejecución integrada evalúa y rechaza 316 hojas core, invoca 316 búsquedas standalone y registra 316 bloqueos de `reality-operation-02`; consume exactamente 300.000 ramas —267.452 core y 32.548 standalone—, alcanza profundidad core 19 y standalone 0, y termina `BRANCH_BUDGET_EXHAUSTED` en fase `CORE`. No hay hoja completa: fingerprint core seleccionado y fingerprint completo permanecen nulos, se conservan 9 pendientes y no se publica parcial. Por tanto **no se declara alcanzado** el objetivo de 53 tareas, 12 operaciones y 375 minutos.
- **Protección y exclusiones:** el orden, matching, patrones, timelines, bloques compuestos, materialización anclada y reglas hard del núcleo no cambian; su benchmark aislado permanece congelado y la ruta histórica continúa separada. No hay hardcodes, detección de Focal A2, memoización, blocker learning, future feasibility nueva, aumento de presupuesto, dispatcher, routing, `PlanResult`, DB/RLS ni reutilización de código o commits de #586/#587.
- **Siguiente deuda basada en Evidence:** la continuación de hoja corrige el lock implícito y demuestra que el problema domina en la fase core, pero las primeras 316 hojas siguen bloqueando la misma obligación antes de cualquier selección standalone. Conforme al alcance R1 no se añade otra optimización; la siguiente unidad debe decidir, con esta Evidence, cómo alcanzar una hoja core compatible dentro del presupuesto sin introducir reglas específicas ni relajar restricciones hard.
