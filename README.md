...
- **Atomicidad de configuración general:** el endpoint valida toda la jerarquía propuesta, construye un único patch snake_case y realiza un solo `UPDATE ... RETURNING`; una validación fallida no escribe jornada, comida ni otros campos.
- **Tests y validación local:** el checker pasa `8/8`, el helper temporal/catálogo/inicialización `29/29`, los contratos compartidos `6/6` y la semántica HTTP `6/6`, program settings `4/4` y RLS/SQL `2/2`. TypeScript y production build pasan; `npm test` pasa `1911/1911` en 8 suites; `git diff --check`, la comparación byte a byte, los SHA-256 y la validación SQL estática pasan. La CI alojada cubre instalación, checker, TypeScript y build —no se le atribuye `npm test`— y debe concluir `success` sobre el head final antes del merge.
