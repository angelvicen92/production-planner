...
- **Objetivo y fuentes:** auditar, sin ampliar el motor, las 167 capacidades exigidas contra el inventario normalizado de `ENSAYO_A2_LV.pdf`, su desglose, las SPEC-06 a SPEC-10 y sus addenda, los contratos actuales, tests y Evidence.
- **Focal parcial frente al día completo:** el Focal conserva 53 tareas (19 main, 19 vocal, 9 operaciones standalone y 6 segmentos anclados), hard-validity y determinismo; no demuestra Plató 14/15, Totales, Alfombra Roja, setups, comidas individuales, operaciones técnicas, todos los locks ni toda SPEC-07.
- **Estados y resultado:** 66 capacidades son `CODE_SUPPORTED_NOT_REPRESENTATIVE_EVIDENCE`, 64 `PARTIALLY_SUPPORTED`, 15 `EXPLICITLY_UNSUPPORTED`, 16 `CONTRACT_GAP` y 6 `PRODUCT_PHASE_NOT_IMPLEMENTED`. Ninguna satisface todavía el umbral integral de `EVIDENCED_SUPPORTED`; por tanto `fullA2PlanningCoverage: false` y `fullA2ProductReadiness: false`. De 20 familias, 3 están parcialmente representadas y 17 no representadas; ninguna está cubierta end-to-end.
- **Blockers críticos:** contratos técnicos y de elegibilidad; locks de espacio/completos; capacidad/no exclusividad; setups/preparación; requisitos genéricos, cantidades y alternativas de recursos; comidas individuales; y las fases de producto 162–167.
- **Siguiente unidad:** capacidad 120, integración contractual de tareas técnicas sin participante, porque Planner Next ya posee un tipo y tests sintéticos y el gap bloquea Alfombra Roja y preparaciones técnicas.
- **Evidence y validación:** el catálogo, los 20 probes read-only, las matrices y la Evidence JSON determinista viven en `engine/planner-next/coverage`, `engine/planner-next/benchmarks/fixtures` y `docs/planner-next/SPEC10-012-focal-a2-capability-coverage.md`. La Evidence mide 326708 bytes y su SHA-256 es `b8f96fcd1e4cc3ec6c1aa18153da5c07dba82e34e5fac9e6a18e5722536461c3`.
- **Límites:** no se implementó ninguna capacidad nueva, publicación, shadow mode, UI, API, DB o RLS; tampoco se modificaron contratos ni algoritmos productivos.
