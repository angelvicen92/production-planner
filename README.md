...
- **Semántica HTTP y API contractual:** IDs o bodies inválidos devuelven `400`, entidades inexistentes o no accesibles devuelven `404`, fallos inesperados sanitizados devuelven `500`, y una lista válida vacía permanece `200 []`. Los namespaces `api.zones.availability` y `api.spaces.availability` tienen paths no intercambiados. Listados, inicialización y updates globales/diarios declaran respuestas Zod compartidas completas; runtime transforma filas Supabase snake_case a camelCase y valida la respuesta compartida, conservando IDs, parejas nullable, `source`, `zoneId` y timestamps.
- **Tests y validación local:** el checker pasa `8/8`, el helper temporal/catálogo/inicialización `29/29`, los contratos compartidos `6/6` y la semántica HTTP `5/5`. TypeScript y production build pasan; `npm test` pasa `1904/1904` en 8 suites; `git diff --check`, la comparación byte a byte, los SHA-256 y la validación SQL estática pasan. La CI alojada cubre instalación, checker, TypeScript y build —no se le atribuye `npm test`— y debe concluir `success` sobre el head final antes del merge.
