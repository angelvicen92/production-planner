...
### FOCAL-A2-010R2 — Bounded Main Backtracking

- **Objetivo:** sustituir el corte irreversible del flujo `main` por exploración reversible, manteniendo el scoring operativo únicamente como orden y `bestK` como frontera prioritaria, no como eliminación de candidatos hard-valid.
- **Implementación diagnóstica:** se eliminaron el barrido general de posiciones futuras, su sentinel numérico y la penalización de `alternativeCount`; la búsqueda constructiva conserva candidatos ordenados y contabiliza generación, profundidad y decisiones dentro de `maxBranchExpansions`.
- **Presupuesto y resultado:** la validación Focal A2 alcanzó exactamente las 300.000 ramas permitidas, con profundidad máxima 15 y ninguna hoja main completa. Se detiene atómicamente con `BRANCH_BUDGET_EXHAUSTED`, no publica un plan parcial y mantiene `hiddenFutureProbeCount = 0`.
- **Límite actual:** la estrategia depth-first agota demasiadas permutaciones de una decisión temprana antes de alcanzar las hojas. Conforme a la condición de parada de FOCAL-A2-010R2, no se aumentó `bestK`, `maxBacktracks` ni `maxBranchExpansions`, no se restauró el future probe y no se añadió ninguna heurística. El benchmark de aceptación de 53 tareas queda pendiente de una decisión de alcance sobre el orden constructivo primario/diferido; esto no declara implementación completa de SPEC-06.
