...
The subtree is obtained from the existing `InitialConstructionPartialPlanGraphIndex` parent/child structure rooted at the checkpoint parent, with suspended and archived entries resolved first by `partialPlanId` / `assignmentsFingerprint` and only unindexed recoverable entries using the bounded `resolveInitialConstructionCheckpointBranch()` fallback. Reopened children remain explicitly classified as `CHECKPOINT_REOPEN`. The same projection feeds transaction evidence and cursor observation, so unrelated nodes are counted as prefilter exclusions rather than prepare skips. Its raw global input is now partitioned exhaustively into local unique candidates, unrelated exclusions, local duplicates, failed-partial-plan exclusions, and invalid source entries; the failed plan itself and runtime `null` / `undefined` source candidates have explicit counters. This is an accounting-only correction: it does not alter candidate selection, source precedence, deterministic ordering, causal evaluation, cursor advancement, ledger behavior, backtracking, or plan search semantics.
