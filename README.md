...
- **Coaches compartidos, feeders y recursos:** una relación de coach sólo se materializa cuando el plan-resource relacionado está en la asignación efectiva de la main/vocal. Varios participantes pueden compartir legítimamente la misma identidad: se publica una sola entrada en `coaches`, las cuatro tareas usan el mismo `coachId`, las mains conservan `blockKey === coachId` y el coach se elimina de `resources/requiredResourceIds`. Siguen bloqueados su uso en auxiliary/technical, main/vocal sin relación o con otro coach, y cualquier doble canal incompatible. Cada participante con main o vocal exige exactamente un par y una dependencia main→vocal exclusiva.
...
- **Anchors tipados, continuidad y robustez:** `EngineInput.anchoredAccompaniments` usa el contrato oficial sin aliases. El preflight acepta continuidad sólo cuando la intersección de recursos genéricos proyectados de todas las fases es no vacía; elimina sólo el coach canalizado como `coachId` en main/vocal y conserva recursos adicionales propios de cada fase. No exige igualdad total ni acepta conjuntos vacíos. Entradas runtime null, primitivas, arrays ausentes/no-array o IDs inválidos retornan `UNSUPPORTED` sin lanzar. El array exterior se canonicaliza por ID, pero before/after preservan orden y cambian el fingerprint al invertirse.
