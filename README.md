...
- **Identidad y tiempo:** toda referencia usa exclusivamente el identity map reversible (`sourceId → namespace:sourceId → sourceId`) del preflight. Los helpers públicos estrictos convierten `HH:mm` a minutos desde `00:00` y viceversa sin truncado ni redondeo; el preflight mantiene el rechazo de formato, límites, intervalos o `timeGridMinutes` incompatibles antes de construir el problema.
- **Coaches, feeders y recursos:** una relación de coach sólo se materializa cuando el plan-resource relacionado está en la asignación efectiva de la main/vocal. Su ID reversible se usa como `coachId`, se publica en `coaches` con disponibilidad efectiva y, para main, también como `blockKey`; se elimina del canal `requiredResourceIds` para no duplicar exclusividad. Cada main exige exactamente un vocal del mismo participante y depende sólo de él. Un vocal con recursos adicionales, una relación sin consumo, un consumo sin relación o un uso cruzado coach/recurso queda `UNSUPPORTED`. Los demás recursos conservan asignación y disponibilidad efectivas; auxiliary usa únicamente la política neutral explícita `OFF`.
...
- **Anchors tipados y orden semántico:** `EngineInput.anchoredAccompaniments` usa el contrato oficial tipado, sin aliases `anchor`/`segments`. El preflight audita identidad, referencias, reutilización, participante, kinds y continuidad explícita de recursos. El array exterior se canonicaliza por ID, pero `beforeTaskIds` y `afterTaskIds` se preservan literalmente y su inversión cambia el fingerprint.
...
- **Determinismo, inmutabilidad y Evidence:** envelope, problema y descendientes quedan profundamente frozen. La serialización ordena sólo colecciones set-like y conserva secuencias operativas. El artefacto versionado es un único JSON parseable, sin preámbulo npm, logs, timestamps ni runtime. Dos generaciones directas fueron idénticas byte a byte: 4.801 bytes y SHA-256 `f367b8aa23f5d5dc1df24b4077505d36136b6614b4efd81ef50e3e6c5480096e`; publica ambos preflights, round-trips, contadores, inmutabilidad y fingerprints. La puerta vigente Focal A2 permanece sin cambios y se valida antes del cierre.
- **Validación correctiva:** pasan adaptador `18/18`, preflight afectado `199/199`, ejecución combinada `217/217`, Planner Next `515/515` y suite global `1.946/1.946`; TypeScript, migraciones, build y `git diff --check` también pasan. `./validate-focal-a2-010.sh current` conserva exit code 0, status aceptado, 33 escenarios, 53 tareas, 0 pendientes, hard validity, 3 operaciones/6 segmentos, máximo 300.000, sin human seed, PartialPlan ni fallback y determinismo; no se modificó su validator, benchmark ni presupuesto.
