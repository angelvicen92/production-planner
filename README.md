...
- **Gate y contrato sin parciales:** cada llamada ejecuta exactamente una vez el preflight vigente. Un resultado `UNSUPPORTED` retorna inmediatamente `problem: null`, `problemFingerprint: null` y conserva literalmente `reasonCodes`, issues, diagnostics, identity map y fingerprints del preflight, sin reparar, completar, filtrar blockers ni usar fallback. El fixture sintético explícito sí es `SUPPORTED` antes de adaptarse.
...
- **Mapping exacto:** `workDay` produce `day`; la comida global concreta produce `protectedMeal`; política, cuatro campos de presupuesto, transiciones y los cinco campos de main flow se proyectan literalmente desde `input.plannerNext`. Spaces requeridos proceden sólo de snapshots diarios efectivos; participants sólo de tareas participantes activas y su disponibilidad explícita; resources sólo de asignaciones directas/espacio/zone diarias efectivas o locks concretos, con disponibilidad efectiva, transición configurada y la semántica neutral documentada por Planner Next `presencePreference: OFF`.
- **Tareas, protección y locks:** `pending` e `interrupted` conservan identidad, kind, participante cuando aplica, duración, espacio, dependencias y recursos, pero descartan horarios planificados previos como seed. `done` e `in_progress` usan exclusivamente el intervalo real completo o el planificado completo cuando no existe endpoint real, convertido en la única task availability y con el mismo espacio/recursos; `cancelled` desaparece. Locks temporales fijan esa misma ventana y locks de recurso añaden la identidad concreta; space/full o dimensiones incompletas siguen bloqueadas por preflight. La relación configurada de coach no crea consumo: un coach asignado efectivamente permanece como `requiredResourceIds`, sin inventar además `coachId`.
- **Comidas, operaciones y límites honestos:** sólo se materializa la comida global exacta aceptada; ventanas flexibles y scopes de participante/space/zone/unidad siguen `UNSUPPORTED`. AnchoredAccompaniment sólo se proyecta desde el contrato explícito completo ya auditado, nunca desde Focal A2. Continúan sin soporte requisitos `byType`, cantidades distintas de uno, `anyOf`, cámaras genéricas, capacidades/concurrencias no exclusivas, setup sin mapping reversible, transporte, locks de espacio y cualquier contrato incompleto publicado por preflight.
- **Determinismo, inmutabilidad y fingerprint:** envelope, problem y todos sus descendientes quedan profundamente congelados. La serialización canónica ordena colecciones set-like e IDs/dependencias antes de SHA-256, omite nombres/display al no proyectarlos y conserva sensibilidad a configuración y restricciones hard; inversión, repetición, metadata visual e input/output frozen quedan cubiertos por tests.
- **Evidence y regresión:** `SPEC10-010-engine-input-adapter` incluye el fixture `SUPPORTED` y los tres escenarios productivos aún legítimamente `UNSUPPORTED`, con repetición, inversión, fingerprints, contadores de spaces/participants/resources/tasks/dependencies/protected tasks e inmutabilidad. Dos generaciones resultaron idénticas byte a byte: 4.314 bytes y SHA-256 `cb28e6de2d7e0a9721a74e74144bb62bc46d9d7ff0b085ac0fb9dd1cdd416923`. La puerta vigente `./validate-focal-a2-010.sh current` terminó con exit code 0, status aceptado, 33 escenarios, 53 tareas, 0 pendientes, hard validity, 3 `AnchoredAccompaniment`, 6 segmentos, máximo 300.000, sin human seed, PartialPlan ni fallback, determinista; digest publicado `b9681959f66f5093c5613bfbd9b491cfcee620023863b22ca9795df3b231d939`.
