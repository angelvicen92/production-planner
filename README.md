...
- **Issues, fingerprints y diagnostics:** la ausencia, deshabilitación o intersección vacía de un recurso requerido genera una única `MISSING_RESOURCE_AVAILABILITY`; contratos temporales mal formados generan una única `UNSUPPORTED_TIME_VALUE`. `sourceFingerprint` incorpora ID, `resourceItemId`, `typeId`, `isAvailable` y la presencia/valor de ambos extremos; `identityMapFingerprint` no cambia. Se publican `requiredPlanResourceCount`, `usableRequiredPlanResourceCount`, `unusableRequiredPlanResourceCount` y `protectedTaskResourceAvailabilityConflictCount`; `missingAvailabilityCounts.resources` representa sólo obligaciones sin ventana utilizable.
- **Evidence reproducible:** los tres escenarios reales permanecen honestamente `UNSUPPORTED` por roles, participantes, espacios, configuración, locks incompletos, requisitos genéricos, setup y demás contratos aún no representados, pero eliminan `MISSING_RESOURCE_AVAILABILITY`. Sus recursos requeridos (1, 2 y 2) resuelven `NULL/NULL` contra la jornada; inversión, repetición, congelación del output e inmutabilidad del input se verifican en el benchmark `SPEC10-007-engine-input-resource-availability`, manteniendo los fingerprints de identidad aceptados.
- **Validación:** tests focales cubren la proyección defensiva camelCase/snake_case, el resolvedor puro (jornada completa, intersecciones, precedencia de invalidez, determinismo y deep freeze) y el preflight (asignaciones, canceladas, locks, causas primarias, protección, fingerprints y diagnostics), además de las suites Planner Next/ORC/global y los benchmarks autoritativos.
