...
- **Objetivo y separación arquitectónica:** se añadió una composición interna que ejecuta primero `constructExactMainAndFeederCore`, conserva su resultado de 44 tareas y su comida sin replanificarlos, valida la forma de las nueve obligaciones restantes y sólo entonces inicia una DFS standalone exacta. La extensión no usa el beam de auxiliares, no convierte `bestK` en poda, no publica `PlanResult` y no introduce dispatcher, routing, fallback, DB, RLS ni integración de producción.
- **Búsqueda, presupuesto y atomicidad:** en cada estado se vuelven a enumerar, con `canPlaceTask`, todos los starts a pasos operativos de cinco minutos; cada evaluación consume previamente una rama del remanente global. La tarea se elige dinámicamente por menor número de posiciones, vencimiento efectivo, mayor duración, más recursos e ID estable; los starts se ordenan mediante el scorer de presencia ya compartido, conservando todas las alternativas. Una colocación abandonada cuenta como backtrack, la hoja consume su propia rama y pasa por `validatePlan`; cualquier forma no soportada, inviabilidad o agotamiento devuelve tareas y comidas vacías.
- **Forma soportada y Evidence:** sólo se aceptan pendientes `auxiliary` no anclados, sin setup, grupo conjunto, bloque secundario REQUIRED, comida secundaria ni dependencias todavía pendientes. Los estados distinguen `CORE_FAILED`, `UNSUPPORTED_STANDALONE_SHAPE`, `INFEASIBLE` y `BRANCH_BUDGET_EXHAUSTED`; la Evidence separa ramas del núcleo/standalone, checks, selecciones, podas por cero alternativas, backtracks, profundidad, hojas, IDs/starts, fingerprints y motivos sin modificar `PlanMetrics` ni contratos históricos.
- **Resultado Focal A2 observado:** el núcleo permanece idéntico en 44 tareas, 9 pendientes, 47.482 ramas y fingerprint `c85b9b2cfbbf9434135f08e2b293b0ab6c23e5ff070e880cbb5406022dd52785`. Sin embargo, la extensión demuestra `INFEASIBLE` en profundidad 0 tras 103 checks standalone (47.585 ramas totales): `reality-operation-02` no tiene ningún start hard-valid frente al núcleo consolidado. Por ello no se publica un resultado parcial, el fingerprint completo sigue nulo y **no se declara alcanzado** el objetivo de 53 tareas, 12 operaciones, 375 minutos, determinismo/invariancia completos ni hard validity de hoja.
- **Protección y exclusiones:** el algoritmo del núcleo, la ruta histórica, sus presupuestos y benchmarks no se modifican; tampoco se reutiliza código ni commits de los PR #586/#587. Las pruebas sintéticas cubren composición, recursos compartidos, selección dinámica, `bestK = 1`, forma no soportada, atomicidad, inviabilidad, frontera B/B−1, determinismo, invariancia, inmutabilidad, identidad del núcleo y validación hard.
- **Siguiente deuda basada en Evidence:** antes de routing o publicación debe resolverse documentalmente la incompatibilidad entre el núcleo aceptado y `reality-operation-02`; no corresponde aumentar presupuesto, añadir memoización o introducir una excepción de transición/resource específica, porque la poda ocurre antes de cualquier decisión standalone y no es un problema de coste de búsqueda.
