...
ID 321 completa la corrección causal de ID 320 haciendo explícita la separación prepare/commit y añadiendo una colección causal multifuente. Las alternativas se etiquetan al recopilarse desde `SUSPENDED_FRONTIER`, `GENERATED_GRAPH`, `CAUSAL_ARCHIVE` o `CHECKPOINT_REOPEN`; después se deduplican con una clave estructurada basada en `partialPlanId`, `assignmentsFingerprint`, `checkpointBranchFingerprint` y `checkpointBranchAssignmentsFingerprint`. La fuente sólo decide qué entrada se consume en el commit o actúa como desempate final: la selección conserva prioridad para utilidad causal, evaluación causal y comparador operativo, y las pruebas cubren el caso `B` rechazado por ledger → `C` preparado desde otra fuente.

Todas las alternativas causales pasan por un contrato focal de preparación que filtra candidates expandidos, fuera del checkpoint, ya inspeccionados por cursor, no-goods, hard-invalid/future-infeasible y duplicados del ledger antes de devolver `ACTIVATION_PREPARED`. La apertura del ledger ocurre antes de mutar `active`, frontera o contadores, pero el helper no declara backtrack ni transición consumidos; la sesión publica `ACTIVATION_COMMITTED` sólo después de retirar/consumir la fuente correcta, actualizar `activeCausalBranchAttemptId`, avanzar el cursor y consolidar `active`, backtrack y transición juntos. Un rechazo de ledger queda como skip interno y continúa inspeccionando la siguiente alternativa sin reentrar al bucle exterior con el dead end anterior; la reapertura de checkpoint queda reservada para cuando las fuentes existentes ya fueron inspeccionadas.
