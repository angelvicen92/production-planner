...
- **Objetivo y modelo:** `program_settings` incorpora el horario de jornada predeterminado; el horario se copia y queda independiente en cada plan. `zone` es el plató contenedor operativo de esta iteración. `zones` y `spaces` incorporan defaults temporales y las nuevas tablas `plan_zone_settings` y `plan_space_settings` conservan snapshots diarios; esta última congela además el `zone_id` autoritativo del espacio.
- **Herencia y validación:** la jerarquía es jornada → zone/plató → espacio. Una pareja nula en zone hereda la jornada y una pareja nula en espacio hereda la ventana efectiva de zone, sin materializar esa herencia. Toda pareja explícita es canónica, ordenada y debe quedar contenida por completo en su padre; nunca se intersecta ni corrige silenciosamente.
- **Migración y backfill:** la migración 067 añade defaults y constraints de pareja, formato y orden, crea ambas tablas con FKs, unicidad y cascade desde plan, y realiza backfill idempotente con parejas nulas para planes existentes sin modificar sus jornadas ni sobrescribir snapshots existentes. No añade RLS porque el acceso permanece exclusivamente server-side.
- **Creación, inicialización e independencia:** antes de insertar el plan se construye y valida de forma pura el lote completo usando un override diario explícito completo o los defaults de jornada. Tras crear el plan se copian todos los snapshots espaciales y el flujo ya existente de recursos; un fallo elimina el plan para evitar inicialización lógica parcial. La inicialización tardía sólo inserta filas ausentes de forma idempotente y nunca refresca ni sobrescribe filas. Cambios globales y diarios permanecen independientes.
- **Helper, storage y API:** el helper puro y determinista publica ventanas efectivas, modos de herencia y reason codes de contrato/contención, sin DB, nombres ni `parent_space_id`, con salida profundamente congelada. Storage y contratos Zod compartidos permiten leer, inicializar y actualizar defaults y snapshots mediante parejas completas; actualizar jornada o zone valida sus descendientes antes de persistir.
- **Tests y validación SQL:** existen specs puros de jerarquía, construcción de snapshots, determinismo, inmutabilidad, congelación, orden, rechazo de lotes inválidos, inicialización idempotente y contratos compartidos. La migración se valida estáticamente; no se declara aplicada sobre una DB real salvo evidencia externa explícita.
- **Límites:** sólo se snapshotan relaciones y ventanas temporales, no nombres, colores, prioridades, grouping ni setups. No existe todavía botón ni endpoint de “Copiar configuración predeterminada”, diff o actualización masiva; no se modifica UI, RLS, `EngineInput`, Planner Next, ORC, V3/V4 ni el preflight anterior, por lo que no se afirma que Planner Next reciba estas ventanas ni que desaparezca `MISSING_SPACE_REFERENCE`.
- **CI alojada:** la workflow alojada de este repositorio cubre instalación, TypeScript check y production build; no se le atribuyen los tests o benchmarks locales anteriores. Su estado corresponde al head publicado del PR y debe quedar verde antes del merge.
- **Alcance, límites y siguiente blocker:** no hay DB, migración, RLS, Supabase, UI, adaptador `EngineInput → PlannerNextProblem`, ejecución de Planner Next desde `EngineInput`, múltiples ventanas, fallback ni sustitución. La adaptación productiva continúa ausente; el siguiente blocker real sigue siendo demostrar y construir, en una unidad posterior, la adaptación exacta de los contratos restantes antes de cualquier ejecución productiva.
