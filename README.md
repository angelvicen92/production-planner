...
- **Algoritmo:** una DFS leaf-first explora los patrones compartidos y las timelines canónicas; cada decisión materializa atómicamente `before → main → after + feeder`, conserva todos los starts feeder (más tardío primero), y puede retroceder tanto de start como de main, sin convertir `bestK` en poda.
- **Factibilidad residual y presupuesto:** tras cada decisión, un matching bipartito determinista por caminos aumentantes exige sólo una cobertura relajada de mains y posiciones restantes. Patrones, timelines, mains, starts feeder, aristas, caminos y comprobaciones futuras consumen el único `maxBranchExpansions`; `maxBacktracks` no limita esta ruta.
- **Evidence y atomicidad:** el resultado propio del núcleo registra ramas, candidatos, matching, podas, backtracks, profundidad, leafs, patrón, timeline, IDs y fingerprint. Todo fallo devuelve tareas y comidas vacías; un éxito se valida con los validadores hard compartidos sobre un problema reducido e inmutable y devuelve canónicamente las tareas ajenas pendientes.
- **Benchmark Focal A2:** dos ejecuciones del runner no publicador completan hard-validly 44 tareas (19 main, 19 vocal y 6 segmentos de 3 operaciones ancladas), dejan 9 tareas standalone, preservan input y orden, y producen el fingerprint `06308843f3f955dfda0461b5e07350be74b08242c10943d0872c3b2bb3cfab14` con 46.017 ramas, 1.587 backtracks, profundidad 19, 1.606 matching checks y 1.587 podas. El núcleo exacto construye de forma completa y hard-valid el flujo principal, sus feeders y sus operaciones ancladas del escenario Focal A2 dentro del presupuesto.
- **Protección histórica:** `generateMainFlowPatterns` es la extracción mecánica única del generador antes privado; `COMPATIBILITY_PRESERVING` conserva objetos canónicos y fingerprint frente al baseline de `main`, ignorando exclusivamente `runtimeMs`. No se incorporó código experimental de los PR #586 ni #587.
