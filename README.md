...
- **Estimación y clave exacta:** cada start residual se compara por menor permanencia resultante, gap máximo, incremento idle, distancia a la obligación más cercana y start; se conserva solo esa señal individual. La agregación usa `projectedPresenceLowerBound = max(currentPresenceSpan, bestIndividualPresence...)`, `projectedMaximumGapLowerBound = max(bestIndividualMaximumGap...)` y sumas independientes de expansión de presencia, idle y gap. La clave ordena por `(emptyStaticDomainCount, projectedPresenceLowerBound, projectedMaximumGapLowerBound, sumIndependentIdleExpansion, sumIndependentPresenceExpansion, sumIndependentMinimumGap, participantSlack, firstObligation, candidateId)`. Las posiciones individuales pueden competir por recursos o ser incompatibles entre sí: la estimación no predice exactamente el plan, no prueba viabilidad u optimalidad, no elimina candidatos (incluidos dominios vacíos), no usa `bestK` como corte y no consume ledger.
- **Evidence causal:** el fixture de dos participantes, feeders directos y standalone tardía cambia el primer candidato, conserva las cinco tareas y hard validity, reduce idle, es determinista/invariante e inmutable, y demuestra backtracking desde un primer candidato que pierde su witness dinámico. La frontera exacta B/B−1 conserva salida atómica. La Evidence experimental queda separada del contrato baseline y registra orden base/contextual, claves, selección aceptada, 33 decisiones, 235 candidatos ranqueados (135 con obligaciones residuales y 100 sin ellas), 27 órdenes cambiados, 22 primeros candidatos cambiados, 11.163 evaluaciones estáticas, 2.186 starts y cero dominios vacíos; los diagnósticos están limitados determinísticamente.
