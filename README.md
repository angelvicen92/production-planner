...
- **Evidence de integración:** se añaden `MISSING_TRANSITION_CONFIGURATION` e `INVALID_TRANSITION_CONFIGURATION`, junto con `integrationConfigurationPresent` y `transitionConfigurationComplete`. Los 84 tests del preflight cubren contrato ausente, canales legacy ignorados, las dos políticas, budget, grid respecto del inicio de jornada, `preferredEnd`, espacio físico descrito, transiciones —incluido cero—, identidad, fingerprints canónicos, propiedades extra ignoradas, pureza y Evidence representativa congelada.
