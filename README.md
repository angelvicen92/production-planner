...
## SPEC10-012 — Auditoría ejecutable de cobertura del planning A2

- **Objetivo y método:** reemplaza la clasificación declarativa por 29 probes reales, frozen y read-only que ejecutan `preflightEngineInputForPlannerNext`, `adaptEngineInputToPlannerNextProblem`, búsqueda y `validatePlan`; cada estado se deriva de `CapabilityEvidenceRecord`, referencias exactas y resultados observados.
- **Fuentes y requisitos:** el manifest registra ambos PDFs p.1. Los 167 requisitos se clasifican individualmente: 81 `REQUIRED`, 80 `UNRESOLVED` y 6 `NOT_REQUIRED` de fase producto; no existe regla por rango de ID.
- **Correcciones:** done/in_progress, time lock válido, resource lock, coach availability y technical task dejan de figurar como gaps inexistentes; joint/technical chains separan soporte Planner Next de integración. El transporte separa tarea técnica de contrato especial.
- **Vocal y familias:** José Mª y Lucía tienen `representedTaskCount:null`; `A2_VOCAL_FLOW_TOTAL` conserva las 19 vocales sin duplicarlas. Reality se reconoce parcialmente representada. Conteos familiares: 7 `ENGINE_CAPABLE_NOT_A2_FIXTURE`, 2 `NOT_REPRESENTED`, 5 `PARTIALLY_REPRESENTED`, 7 `SOURCE_UNRESOLVED`.
- **Resultado calculado:** 6 `CODE_SUPPORTED_NOT_REPRESENTATIVE_EVIDENCE`, 31 `PARTIALLY_SUPPORTED`, 3 `EXPLICITLY_UNSUPPORTED`, 70 `CONTRACT_GAP`, 51 `SOURCE_AMBIGUOUS` y 6 `PRODUCT_PHASE_NOT_IMPLEMENTED`; `fullA2PlanningCoverage=false` y `fullA2ProductReadiness=false`.
- **Siguiente unidad calculada:** capacidad 141, tarea aplicable/elegibilidad explícita, seleccionada por el decision trace al afectar seis familias y disponer de soporte downstream parcial.
- **Evidence:** 371525 bytes; SHA-256 `3ff68ac95528eee806fd2f529f6e3df109482952340d1f4623fc5c9f76e8e854`. No se añadió capacidad ni se modificó código productivo.
