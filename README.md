...
ID 321 completa la corrección causal de ID 320 haciendo explícita la separación prepare/commit y delegando la ruta causal completa de la sesión en un orquestador multifuente. La sesión resuelve el checkpoint, recupera su cursor y entrega simultáneamente al orquestador candidatos de frontera suspendida, grafo generado y archivo causal antes de seleccionar; las alternativas se etiquetan al recopilarse desde `SUSPENDED_FRONTIER`, `GENERATED_GRAPH`, `CAUSAL_ARCHIVE` o `CHECKPOINT_REOPEN` y después se deduplican con una clave estructurada basada en `partialPlanId`, `assignmentsFingerprint`, `checkpointBranchFingerprint` y `checkpointBranchAssignmentsFingerprint`. La fuente sólo decide qué entrada se consume en el commit o actúa como desempate final: la selección conserva prioridad para utilidad causal, evaluación causal y comparador operativo, y el test integrado del orquestador usado por la sesión cubre `B` rechazado por ledger → `C` preparado desde `GENERATED_GRAPH`.

Todas las alternativas causales pasan por un contrato focal de preparación que filtra candidates expandidos, fuera del checkpoint, ya inspeccionados por cursor, no-goods, hard-invalid/future-infeasible y duplicados del ledger antes de devolver `ACTIVATION_PREPARED`. La apertura del ledger ocurre antes de mutar `active`, frontera o contadores, pero el helper no declara backtrack ni transición consumidos; la sesión publica `ACTIVATION_COMMITTED` sólo después de retirar/consumir la fuente correcta, actualizar `activeCausalBranchAttemptId`, avanzar el cursor y consolidar `active`, backtrack y transición juntos. Un rechazo de ledger queda como skip interno de la misma transacción y continúa inspeccionando la siguiente alternativa sin reentrar al bucle exterior con el dead end anterior; rechazo y commit se enlazan con `causalActivationTransactionId`, conflicto, checkpoint y decisión causal en muestras read-only. La ruta de Evidence causal completa ya no ejecuta el selector legacy de siblings ni el bloque legacy de preparación individual después de una transacción multifuente; si las fuentes existentes no preparan candidato, el cursor actualizado se persiste una sola vez por fingerprint y la reapertura debe pasar por la misma transacción con fuente explícita `CHECKPOINT_REOPEN`. El commit queda concentrado en un helper focal que no vuelve a abrir el ledger, valida candidato/intento/fuente prevalidada, retira sólo la entrada exacta de frontera o archivo cuando corresponde, aplica el archivo causal devuelto como estado de sesión, conserva nodos de grafo/reapertura y separa el avance de cursor por inspección del avance post-commit que marca la branch como activada.

El cursor causal es in-memory y determinista, con clave `(conflictFingerprint, checkpointFingerprint, causalDecisionId)`, pero ahora se limita al subtree real del checkpoint usando el índice estructurado de `PartialPlan`, `parentPartialPlanId`, `decisionLineage` y `decisionBranchFingerprint`. La Evidence distingue nodos globales inspeccionados, matches del subtree y nodos ajenos excluidos, además de branches observadas, inspeccionadas, activadas, agotadas, assignments hijos y candidatos temporales evaluados. Las muestras son acotadas y los contadores reales no dependen del tamaño de esas muestras; los invariants publicados cubren backtrack sin commit, commit sin intento abierto, violations de transición, active ya expandido, terminación prematura con frontera elegible, suma de commits por fuente, suma de skips por razón, clasificación explícita de fuentes, bypass legacy completo y cursor updates descartados.

Validar Plan 27 con `bash validate-id321-plan27.sh`; el script exige que exista `local_engine_scenarios/optiplan-plan-27-engine-scenario-v1.json`, ejecuta TypeScript, la batería focal ORC, dos benchmarks reales con el presupuesto de ID 318 y sólo entonces escribe `plan-27-orc-causal-activation-transaction-v1.json` de forma consolidada y atómica. Si el snapshot no está disponible, el script falla sin crear runs vacías ni sobrescribir un artefacto válido anterior; por tanto esta entrada no declara que Plan 27 haya pasado en entornos donde el snapshot no exista.

### ID 322 — Checkpoint-Local Candidate Projection and Deterministic Budget Completion v1

ID 322 addresses the ID 321 Plan 27 evidence where both runs reached 43 productive assignments but stopped on the 90s wall-clock guard with different logical work counts (190 vs 191 expansion work units). The new checkpoint-local projection builds a deterministic candidate view before causal collection: graph, suspended-frontier, archive and reopen sources are filtered by structured checkpoint subtree membership, then deduplicated by PartialPlan identity before causal evaluation, envelope construction, source prevalidation or ledger opening.

The subtree is obtained from the existing `InitialConstructionPartialPlanGraphIndex` parent/child structure rooted at the checkpoint parent, with suspended and archived entries resolved first by `partialPlanId` / `assignmentsFingerprint` and only unindexed recoverable entries using the bounded `resolveInitialConstructionCheckpointBranch()` fallback. Reopened children remain explicitly classified as `CHECKPOINT_REOPEN`. Exactly one physical projection is now built by the iterative session for each causal transaction attempt (including `CHECKPOINT_REOPEN`), initializes a missing cursor directly from its observed branches, child assignments and accounting fields, and is passed by object reference to the transaction for candidate collection and causal evaluation. The former `globalCursorCandidates` scan and per-node branch-resolution loop have been removed without a fallback. Evidence proves this contract with projection build/reuse/built-inside-transaction/unexpected-rebuild counters and a zero-valued legacy-global-cursor-scan counter; unrelated nodes are counted once as projection prefilter exclusions rather than prepare skips. Its raw global input is now partitioned exhaustively into local unique candidates, unrelated exclusions, local duplicates, failed-partial-plan exclusions, and invalid source entries; the failed plan itself and runtime `null` / `undefined` source candidates have explicit counters. This is an accounting-only correction: it does not alter candidate selection, source precedence, deterministic ordering, causal evaluation, cursor advancement, ledger behavior, backtracking, or plan search semantics.

Evidence now separates prefilter exclusions, duplicate local projections, failed-partial-plan exclusions, invalid source exclusions, projection accounted-source totals, conservation residuals, conservation violation counts, static/source skips and real ledger rejections. `causalActivationCausalEvaluationCount` only increases for local candidates that actually call `evaluateInitialConstructionCausalAlternative()`, while `causalActivationLedgerRejectedThenCommittedCount` is reserved for a real ledger rejection followed by a later opened and committed candidate in the same transaction. Transaction samples remain bounded to 10 inspected candidates and skipped samples must include a non-null reason.

Plan 27 validation is produced by `bash validate-id322-plan27.sh`, writing `plan-27-orc-checkpoint-local-candidate-projection-v1.json` only after both benchmark runs, determinism checks, projection counters, runtime margin, invariants and no-regression checks pass. The repository test suite now uses a portable deterministic Node runner instead of quoted shell globs, so `npm test` discovers sorted `.spec.ts` files under `engine` and `server` without relying on shell expansion or passing the Jest-only `--runInBand` flag through `tsx --test`. The optional V3 CP-SAT pilot now classifies missing Python as `python3_unavailable` (without misreporting `ortools_import_failed`) and applies the same bounded deterministic fallback for known runtime absences (`python3_unavailable`, `ortools_import_failed`, or `cp_sat_script_missing`) while preserving the original technical Evidence alongside `deterministic_pilot_fallback`. Until the Plan 27 snapshot exists in the executing environment and this artifact is produced for the current branch, this README entry documents the implemented validation path rather than claiming an accepted Plan 27 result.

### ID 323 — Sound Stage 2 Future Feasibility Gate & Single-Path Baseline Recovery v1

ID 323 fixes the `INITIAL_CONSTRUCTION` Stage 2 gate that could prevent the historical `constructionSearchStrategy = "single_path"` constructor from starting. The regression was not a zero-progress constructor run: Stage 2 could reject the only hard-valid branch before the iterative session, so the session reported `executed = false` with `stage2_hard_valid_partial_plan_required`.

The removed false proof was the suffix-capacity approximation: compute the latest end of provisional branch assignments for a contestant, set capacity to `availabilityEnd - latestAssignedEnd`, and prune when pending load exceeded that suffix. That suffix ignores valid capacity before the anchor, gaps in the whole availability span, prerequisite placement before the anchor, and the provisional nature of the branch.

Stage 2 now delegates to a pure preliminary Future Feasibility evaluator. It never returns `FEASIBLE`; it returns `INFEASIBLE` only for complete deterministic hard proofs, otherwise `UNKNOWN` with explicit risk or incomplete-Evidence reason codes. Contestant capacity uses the canonical constructive target, subtracts tasks already covered by protected/base/branch assignments, clips known occupying intervals to the official availability, merges overlapping or contiguous intervals deterministically, and compares pending mandatory target load against optimistic free minutes across the whole availability.

The hard proofs intentionally remain narrow: demonstrated contestant overload may emit `CONTESTANT_REMAINING_LOAD_EXCEEDS_AVAILABILITY`, and mandatory fixed resource inventory with no official capable available unit may emit `RESOURCE_WITHOUT_INVENTORY`. Alternative or incomplete resource contracts, incomplete durations, missing availability, and uncovered future dimensions stay `UNKNOWN`/risk instead of eliminating a hard-valid branch. The canonical constructive target is sourced from the existing task-universe classification (`strictProductiveWorkTaskIds`, transport-arrival targets, `canonicalConstructiveTargetTaskIds`, and fingerprint evidence), not from local names or Plan 27 IDs.

The public benchmark JSON now distinguishes Stage 2 gate rejection and a non-executed iterative session from an executed comparable run with zero progress. It exposes Stage 2 execution, selected branch, selected validation, Future Feasibility status, hard-valid branch counts, iterative-session execution/reason, `benchmarkComparisonEligible`, and `benchmarkOutcome`. The historical Plan 27 target remains 170/174 productive assignments, but this branch must not be described as recovered until `bash validate-id323-plan27.sh` produces `plan-27-orc-single-path-baseline-recovery-v1.json` locally from the real snapshot.


ID 323 local audit status is now explicit: Stage 2 and the `single_path` iterative session execute deterministically, Stage 2 selects a `VALID` branch whose preliminary Future Feasibility remains `UNKNOWN`, but the constructor reaches only 132 productive assignments, leaves 42 productive tasks pending, consumes 80 accepted cycles, and stops at `MAX_ACCEPTED_CYCLES`. The two current runs are deterministic, yet 56 of 80 accepted cycles add a single task and, after cycle 25, 51 of 55 cycles add a single task. That remains a regression versus the ID 295 baseline of 170/174 productive tasks in 67 accepted cycles, so ID 323 is not an accepted baseline recovery result.

### ID 324 — Goal-Coupled Critical-Chain Construction & Single-Path Productivity Recovery v1

ID 324 addresses the structural disconnection found after ID 323: a priority carrier may be an executable feeder that inherits the criticality of a later chain goal, while the previous `single_path` materializer still built only the carrier's local prerequisite closure. That allowed a PartialPlan to accept frontier prefixes without containing the critical goal that justified the priority decision.

The `single_path` path now resolves a goal-coupled anchor before materialization. The resolver separates the priority carrier, construction goal, execution frontier and pending goal-coupled closure, choosing `primarySupportedGoalTaskId`, then `goalTaskId`, then supported goals, and falling back to the self anchor only when no pending supported goal remains. The closure is derived from structured critical-chain topological order plus the canonical prerequisite graph, excludes already satisfied or protected work, preserves constructive support prerequisites, and remains deterministic under physical order inversion.

Stage 2 and the iterative `single_path` session now materialize and validate packages anchored on the construction goal rather than on a different carrier. The reasoning order is goal-first, but execution order remains topological: prerequisites may be placed before the goal, and the goal is not required to be the first assignment. Accepted branches must be hard-valid, have `VALID` Validation, avoid hard `INFEASIBLE` Future Feasibility in Stage 2, and include the construction goal. Prefix-only branches are rejected as `GOAL_COUPLED_CLOSURE_INCOMPLETE` and counted in immutable session Evidence.

The public benchmark retains ID 323 fields and adds Stage 2 carrier/goal/closure/fingerprint fields plus goal-coupled session counters and package-size aggregates. Retained alternatives are intentionally untouched; goal coupling is only active for `constructionSearchStrategy = "single_path"`. The validation artifact for this change is `plan-27-orc-goal-coupled-single-path-recovery-v1.json`, produced only by `bash validate-id324-plan27.sh` after local checks, focal tests, full tests, two real Plan 27 benchmarks and determinism/result gates pass. Without that local JSON, this README does not claim that the 170/174 baseline has been recovered.

### NEXT-001 — Isolated Main-Flow and Vocal-Feeder Vertical Slice
...
### REPO-002 — Legacy ORC Closure and Artifact Policy
...
### NEXT-009 — Branch-Local Auxiliary Ranking
...
### NEXT-012 — Synchronized Joint Auxiliary Tasks
Planner Next adds a real operational capability: `jointGroupId` links compatible auxiliary tasks without a new `Task.kind`.
...
PROBE stops as soon as its complete-candidate limit is reached, reducing real work rather than only truncating output.
Validation is crash-safe when a root, predecessor, or successor is absent and reports one violation per invalid chain.
...
`PROBE` now returns immediately when its candidate limit is reached, reducing real work rather than only truncating output.
`SEARCH` and the frozen operational plan remain unchanged, including fingerprint `deefc694a142b8d35ebd20cbda15f5fb9d786ef3c43fa8519d50fe5817cda3b9`.
...
`REQUIRED` is constructed as a composite block and positioned before its member tasks are ordered; the current main flow explicitly requires one homogeneous task duration.
Its deterministic structural enumeration is bounded by the existing `maxPatterns` budget, remains canonical for multiple REQUIRED resources, and never publishes a partial plan on exhaustion.
An assigned-space meal may cut the block only at an internal homogeneous task boundary, without changing productive duration or permitting an incompatible task inside it.
...
### FOCAL-A2-008R2 — Audited Itinerant Units Before Anchor Wrapping
The audit executes nine standalone operations; three anchor wraps remain pending.
Exact configurable compositions never duplicate a unit ID as a hard resource.
Each operation belongs only to its configured unit, producing corrected agenda Evidence.
The benchmark preserves the audited resource availability windows.
The behavioral control proves disjoint morning teams can work in parallel.
It also proves later afternoon recomposition, exclusivity, movement, and variable duration.
The v1 projection is withdrawn and v2 is protected solely as historical substrate.
Only `ANCHORED_OPERATION_RELATIVE_SEGMENTS_NOT_EXPRESSIBLE` and
...
Block candidates are scored incrementally with the ordinary auxiliary rules before `bestK` pruning.
...
The v3 benchmark validates a protected historical witness under neutral configuration without using it as a seed.
Real input mutations rebuild checks and acceptance alongside artifact, human-reference, and validator poison controls.
...
### SPEC09-001 — Contrato y resolución explícita de política de búsqueda

- **Objetivo:** introducir exclusivamente el contrato tipado y la resolución pura de políticas de búsqueda de SPEC-09, sin alterar la ejecución vigente de Planner Next.
- **Implementación:** `PlannerNextProblem` admite el campo opcional `searchPolicy`; se publican los contratos de selección, capacidades, requisitos, motivos y resolución, junto con una matriz centralizada y extensible. Los arrays de salida son canónicos, ordenados y sin duplicados.
- **Migración:** un caller histórico que omite la política obtiene temporalmente `COMPATIBILITY_PRESERVING`, con fuente `MIGRATION_DEFAULT` y advertencia `IMPLICIT_SEARCH_POLICY_DEPRECATED`; el resolvedor nunca muta el input ni sustituye una política explícita incompatible.
- **Compatibilidad:** la presencia real de al menos un contrato `anchoredAccompaniments` detecta únicamente `ANCHORED_ACCOMPANIMENT`. La matriz distingue el conjunto canónico de políticas soportadas de la política obligatoria cuando corresponde; para esta capacidad ambos contratos declaran exclusivamente `EXACT_CONSTRUCTIVE`. Una política incompatible se explica con `SEARCH_POLICY_CAPABILITY_UNSUPPORTED` sin excepción, fallback ni aumento de presupuesto.
- **Alcance y exclusiones:** no existe todavía dispatcher y el resolvedor no se invoca desde `planMainFlowAndFeeders`. No se ha integrado la ruta exacta del PR #587 ni su matching residual, feeder-prefix closure o leaf-first backtracking. No cambió ningún resultado operativo, tarea, horario, fingerprint, métrica, stop reason ni presupuesto.
- **Validación:** tests focales del contrato, suites de Planner Next y regresión AnchoredAccompaniment, `check`, `build`, suite global y dos ejecuciones deterministas del benchmark Focal A2 V4 ignorando únicamente `runtimeMs`.
- **Siguiente unidad pendiente:** SPEC09-002 debe extraer la ruta `COMPATIBILITY_PRESERVING` conservando exactamente su semántica histórica; esta iteración no realiza routing ni afirma que `EXACT_CONSTRUCTIVE` esté integrada.

### SPEC09-002 — Extracción aislada de la ruta COMPATIBILITY_PRESERVING

- **Objetivo:** crear exclusivamente la frontera física de la ruta histórica de Planner Next, sin modificar su algoritmo ni ningún resultado operativo.
- **Implementación:** `compatibilityPreservingSearch.ts` contiene la única implementación autoritativa previamente alojada en `planMainFlowAndFeeders.ts`, ahora publicada como `planCompatibilityPreserving`. La fachada histórica conserva `planMainFlowAndFeeders`, `GreedyFeederClosureResult`, `diagnoseGreedyFeederClosure` y `tryGreedyFeederClosure`, y se limita a delegar en la ruta extraída. No se duplicaron reglas hard.
- **Compatibilidad y exclusiones:** no existe dispatcher ni resolución de política en runtime; `searchPolicy` todavía no selecciona comportamiento y `EXACT_CONSTRUCTIVE` todavía no está implementada como ruta. No se reutilizó código de los PR #586 o #587, no se recuperaron sus mecanismos experimentales y no cambió ninguna tarea, horario, métrica, stop reason, presupuesto o fingerprint.
- **Validación:** equivalencia canónica, ignorando exclusivamente `runtimeMs`, cubierta en un escenario simple y en Focal A2 completo; suites focales y globales, `check` y `build`; benchmark Focal A2 V4 comparado dos veces con el baseline real de `main`, preservando objetos completos canonizados y fingerprints.
- **Resultado operativo:** producción permanece sin cambios, con una única implementación histórica y la API pública existente intacta; no se modificaron DB, RLS, endpoints, Evidence ni benchmarks.
- **Siguiente unidad pendiente:** SPEC09-003 debe introducir, en una iteración separada y con evidencia propia, el routing explícito que use la resolución contractual y una implementación real de `EXACT_CONSTRUCTIVE`; hasta entonces todas las políticas declaradas siguen ejecutando la ruta de compatibilidad.

### SPEC09-003 — Núcleo constructivo exacto de flujo principal y feeders

- **Objetivo:** implementar un núcleo interno, determinista y no publicador que construya el flujo `main`, un feeder `vocal` estructurado por main, los segmentos de `AnchoredAccompaniment` anclados a main y la comida configurada del flujo principal.
- **Algoritmo y pruebas causales:** una DFS leaf-first explora patrones y timelines canónicas; cada decisión materializa atómicamente `before → main → after + feeder` y no convierte `bestK` en poda. La exploración feeder es perezosa: ordena mains sólo por holgura, primera obligación e ID, y para cada main prueba starts del más tardío al más temprano, profundizando en cuanto encuentra uno válido y conservando los anteriores para backtracking. Fixtures sintéticos demuestran tanto el main diferido como el start diferido con `bestK = 1`, y un control adicional prueba que el primer start válido no fuerza la enumeración de los anteriores.
- **Contabilidad corregida:** consumen el único `maxBranchExpansions`, antes de evaluarse, cada patrón, timeline, combinación y posición compuesta, candidato main, start feeder, matching completo, pareja main-posición, arista aumentante, comprobación futura y validación final de hoja. Los checks feeder se separan entre construcción y matching, y su suma coincide con `feederCandidatesEvaluated`; los helpers distinguen `BUDGET_EXHAUSTED` de ausencia de witness. La prueba dinámica de frontera obtiene actualmente `B = 60`: con B completa canónicamente y con B−1 devuelve agotamiento atómico y exactamente B−1 ramas.
- **Backtracking y atomicidad:** un backtrack cuenta una única decisión constructiva ya tomada que se abandona porque el matching demuestra falta de cobertura o porque su búsqueda hija —incluida una hoja hard-invalid— no completa; no cuentan candidatos nunca materializados, starts inválidos, agotamientos ni retornos simples. Todo fallo publica tareas y comidas vacías y una hoja sólo se valida, con los validadores compartidos sobre el problema reducido, después de consumir su rama.
- **Benchmark Focal A2 tras exploración perezosa:** frente al baseline R1 de 300.000 ramas, 262.394 checks feeder, profundidad 6 y cero hojas, dos ejecuciones deterministas e invariantes al orden completan ahora dentro del mismo presupuesto con 288.755 ramas, 251.871 checks feeder (5.879 constructivos y 245.992 de matching), 488 mains, 1.558 matching checks, 1.539 podas, 77 podas sin alternativa, 1.539 backtracks, profundidad 19 y una hoja completa. El resultado contiene las 44 tareas esperadas, deja 9 pendientes, es hard-valid y conserva el fingerprint `c85b9b2cfbbf9434135f08e2b293b0ab6c23e5ff070e880cbb5406022dd52785`. La mejora reduce 10.523 checks feeder, alcanza la hoja y no elimina alternativas ni aumenta presupuesto.
- **Protección histórica:** `generateMainFlowPatterns` sigue siendo la extracción mecánica única del generador antes privado; `COMPATIBILITY_PRESERVING` conserva objetos canónicos y fingerprint frente al baseline de `main`, ignorando exclusivamente `runtimeMs`. No queda trabajo de búsqueda conocido sin contabilizar en el núcleo exacto y no se incorporó código experimental de los PR #586 ni #587.
- **Exclusiones:** no hay dispatcher, routing, `PlanResult`, publicación final, fallback, auxiliares standalone, operaciones técnicas standalone, comidas ni setups secundarios, DB, RLS, endpoints o integración de producción. `planMainFlowAndFeeders.ts`, `searchPolicy.ts`, presupuestos y Evidence histórica permanecen intactos.
- **Siguiente deuda:** SPEC09-004 debe decidir documentalmente cómo completar las obligaciones standalone y validar el planning completo antes de cualquier dispatcher o exposición pública de `EXACT_CONSTRUCTIVE`; este núcleo por sí solo no resuelve ni publica el planning Focal A2 completo.

### SPEC09-004 — Matching residual estructural ligero

- **Objetivo y evidencia de partida:** reducir el coste del núcleo `EXACT_CONSTRUCTIVE` sin cambiar planificación, orden constructivo, reglas hard, presupuesto, backtracking ni ruta histórica. SPEC09-003 completaba 44 tareas y dejaba 9 pendientes, pero consumía 288.755 ramas y 251.871 checks feeder; 245.992 de esos checks ocurrían dentro del matching residual y repetían anticipadamente decisiones que la DFS exacta vuelve a demostrar.
- **Implementación y frontera de responsabilidad:** el matching relajado conserva el consumo de ramas, la compatibilidad de `blockKey` y bloques compuestos REQUIRED, `materializeAnchoredOperation`, las aristas canónicas, el matching máximo determinista y la propagación de agotamiento. Ya no busca el feeder, calcula deadlines, enumera starts ni llama a `canPlaceTask` para feeders futuros. Por tanto sólo prueba cobertura estructural main-posiciones frente al estado construido; la construcción exacta sigue siendo la única autoridad que coloca cada feeder, descarta starts inviables y valida la hoja completa.
- **Seguridad y pruebas:** fixtures causales prueban que un feeder futuro imposible atraviesa el matching sin poda falsa y termina después como `INFEASIBLE` atómico, mientras una incompatibilidad estructural real continúa podándose y una cobertura real se conserva. Permanecen verdes los controles de backtracking entre mains y starts, frontera dinámica B/B−1, determinismo, invariancia al orden, inmutabilidad, `AnchoredAccompaniment` y hard validity. `matchingFeederStartChecks` queda protegido en cero y `feederCandidatesEvaluated` coincide con `constructiveFeederStartChecks`.
- **Resultado Focal A2:** dos ejecuciones completas, deterministas e invariantes al orden preservan exactamente 44 tareas construidas, 9 pendientes, 3 operaciones ancladas y el fingerprint `c85b9b2cfbbf9434135f08e2b293b0ab6c23e5ff070e880cbb5406022dd52785`. Las ramas bajan de 288.755 a 47.482 (reducción de 83,56 %), con 5.879 checks constructivos, 0 checks feeder de matching, 488 mains evaluados, 1.558 matchings, 1.539 podas, 1.539 backtracks, profundidad 19 y una hoja completa, muy por debajo del umbral de aceptación de 216.566.
- **Validación y protección histórica:** `npm ci`, `check`, `build`, tests focales, suite completa de Planner Next y `npm test`, además de dos ejecuciones de ambos benchmarks. El benchmark histórico Focal A2 SPEC08 Foundation V4 permanece canónicamente idéntico al baseline de `main` al ignorar exclusivamente `runtimeMs`: tareas, horarios, métricas, ramas, stop reasons y fingerprints no cambian.
- **Alcance y exclusiones:** no se incorporan capacidades nuevas: no hay standalone, técnicas, auxiliares no ancladas, dispatcher, routing, fallback, publicación de `PlanResult`, integración, DB, RLS, frontend, endpoints, memoización, cache, heurísticas ni aumento de presupuesto. Las 9 obligaciones standalone siguen pospuestas porque esta unidad sólo elimina trabajo redundante del matching; el margen obtenido debe medirse antes de seleccionar una siguiente unidad acotada, no interpretarse como evidencia suficiente para añadir automáticamente todas las tareas restantes.
- **Siguiente decisión basada en Evidence:** evaluar por separado qué subconjunto mínimo de obligaciones pendientes puede construirse y validarse dentro de las 252.518 ramas de margen observadas, con benchmark causal propio y sin asumir de antemano el alcance de SPEC09-005.

### SPEC09-005 — Construcción exacta de operaciones itinerantes standalone

- **Evidencia R1 y corrección R2:** R1 demostró que una continuación sólo en hoja era demasiado tardía: completó y rechazó 316 hojas core a profundidad 19, todas con el mismo blocker, hasta consumir 300.000 ramas. R2 añade una continuación parcial inmediatamente después del matching residual y antes de profundizar; si las tareas recién añadidas eliminan el último start hard-valid de una standalone impactada, se rechaza sólo esa decisión y el loop exacto prueba su siguiente alternativa.
- **Impacto y seguridad monotónica:** el filtro puro considera participante, espacio, coach o recurso compartido entre la nueva ocupación y la standalone; no decide validez ni consume presupuesto. Cada witness real se comprueba con `canPlaceTask`, de forma canónica y con consumo `STANDALONE`, deteniéndose en el primero. Añadir ocupaciones puede conservar o eliminar starts, nunca crear uno frente al mismo calendario y comidas: cero witnesses es una poda necesaria y segura; conservar uno no lo reserva ni garantiza la hoja.
- **Presupuesto y Evidence causal:** el ledger único mantiene `branchesExplored = coreBranches + standaloneBranches` y desglosa ramas forward y de DFS final. Se registran checks, starts, witnesses, podas, blockers, profundidades y la última decisión causante únicamente como diagnóstico, sin aprender comparadores ni realizar backjumping. Tests con `bestK = 1` protegen poda antes de hoja, alternativa hermana, decisiones no impactantes, conflictos acumulados, witness no reservado, backtracking standalone, B/B−1, atomicidad, determinismo, invariancia e inmutabilidad.
- **Resultado Focal A2 R2:** completa 53 tareas —19 main, 19 vocal, 9 standalone y 6 segmentos anclados—, 3 operaciones ancladas, 12 itinerantes, 375 minutos productivos, cero pendientes, cero operaciones parciales y hard validity. Consume 85.557 ramas: 48.032 core y 37.525 standalone, divididas en 2.709 forward y 34.816 de DFS de hoja; realiza 18 forward checks, 2.709 starts, 30 witnesses y 6 podas a profundidad 9, todas antes de la hoja. Sólo evalúa una hoja core, no rechaza ninguna en leaf, invoca una DFS standalone y completa con 1.545 backtracks core y 181 standalone.
- **Fingerprints aceptados:** la capacidad core aislada permanece en 47.482 ramas y fingerprint `c85b9b2cfbbf9434135f08e2b293b0ab6c23e5ff070e880cbb5406022dd52785`; el core seleccionado integrado queda en `0948b758c96f17ec546c331ce6d8b42464dbdbe95970d0640ae5fbea95fdbae9` y el plan completo en `825314fa954d2511c5cef2bcd2d6f988efe560131c6039dc99d4ba4c80b662b4`. Dos ejecuciones y la entrada invertida son idénticas, hard-valid e inmutables.
- **Protección y exclusiones:** no cambian el orden aislado del núcleo, matching, patrones, timelines, bloques, materialización anclada, validadores, presupuestos ni ruta histórica. No hay hardcodes, detección por nombres, aprendizaje de blockers, memoización, scoring de permanencia, horarios humanos, dispatcher, routing, `PlanResult`, DB/RLS ni reutilización de #586/#587.
- **Siguiente deuda basada en Evidence:** la viabilidad causal queda resuelta dentro del presupuesto. La siguiente unidad debe abordar por separado calidad y permanencia —estancia, llegada/salida o alineación de actividades— mediante Evidence propia, sin mezclarla con este forward checking ni asumir que un witness provisional es una reserva.

### SPEC09-006 — Evidence de calidad de itinerarios y permanencia

- **Objetivo y definición:** incorpora un evaluador puro, genérico, determinista e independiente de la búsqueda para medir los itinerarios del plan completo aceptado. La permanencia es el intervalo entre el inicio de la primera obligación efectiva y el final de la última. Una ventana de disponibilidad sólo autoriza una tarea: no demuestra la llegada o salida real, por lo que no se contabilizan supuestas esperas anteriores o posteriores.
- **Contrato y semántica:** publica por participante tareas canónicamente ordenadas, primera y última obligación, permanencia, productive, idle, ratio, gaps explicables y su máximo, cambios de espacio y ventanas de disponibilidad usadas; el resumen agrega totales, máximos, conteos, rankings estables y fingerprint SHA-256. `productiveMinutes` es la longitud de la unión de intervalos ocupados —también ante solapamientos anómalos— e `idleMinutes = max(0, presenceSpanMinutes - productiveMinutes)`. Los ratios no participan en el fingerprint.
- **Evidence Focal A2:** los 19 participantes acumulan 3.565 minutos de permanencia, 900 productivos y 2.665 idle (`overallIdleRatio = 0.7475455820476858`). Los 19 tienen gaps internos: 28 gaps en total, 34 cambios de espacio, permanencia máxima de 440 minutos, idle individual máximo de 380 y gap máximo de 225. El fingerprint de calidad es `39163b08d0f377ddd1bbc5bcb5ad1332d9f0c18b699643e0453cdddf547fda12` en las dos ejecuciones y con colecciones invertidas.
- **Concentración del coste:** top 5 por idle y permanencia: `lina-isabel-garcia-salcedo` (380 idle/440 permanencia), `linet-varela` (380/440), `carmen-maria-saborido` (345/390), `nela-garcia` (260/320) y `eva-martin-fernandez` (210/255). Los cinco gaps mayores son Carmen main→operación 11 (225), Linet main→operación 10 (225), Lina vocal→main (215), Nela operación 07→vocal (170) y Lina main→operación 08 (165). La Evidence muestra separaciones intermedias como problema dominante, especialmente alrededor de operaciones itinerantes flexibles, sin convertir esa observación en heurística.
- **Comparación humana exclusivamente informativa:** para las 12 operaciones se registran intervalos generados y de referencia, diferencias firmadas y absolutas. La diferencia absoluta media de inicio es 38,75 minutos y la máxima 90; 5 operaciones quedan adelantadas, 6 retrasadas y 1 conserva el inicio. La referencia no participa en aceptación, fingerprint, seed, scoring ni lógica de producción, y no se considera óptima.
- **Protección operativa y exclusiones:** permanece el plan hard-valid de 53 tareas, cero pendientes, 12 operaciones itinerantes, 375 minutos itinerantes, 85.557 ramas, core seleccionado `0948b758c96f17ec546c331ce6d8b42464dbdbe95970d0640ae5fbea95fdbae9` y plan completo `825314fa954d2511c5cef2bcd2d6f988efe560131c6039dc99d4ba4c80b662b4`. No cambian búsqueda, comparadores, orden, presupuesto, scoring ni reglas hard; no hay pesos, hardcodes Focal A2, horarios humanos en producción, routing, DB/RLS o `PlanResult`, y no se afirma haber mejorado la permanencia.
- **Validación y siguiente recomendación:** tests unitarios cubren vacío, adyacencia, gaps, unión solapada, espacios, ventanas, exclusión técnica, inmutabilidad, órdenes invertidos, desempates y fingerprint; el benchmark protege hard validity, determinismo, invariancia y los fingerprints operativos. Para SPEC09-007, evaluar de forma aislada y sin alterar restricciones hard una política de orden de candidatos que reduzca las separaciones intermedias de los participantes concentrados en los cinco mayores gaps, comparándola contra esta Evidence antes de aceptar cualquier cambio.

### SPEC09-007 — Calibración experimental de la política de permanencia

- **Objetivo y punto de partida:** evalúa en la ruta exacta completa la capacidad ya existente `participantPresenceIncrement × presencePreferenceWeight`, sin crear heurísticas ni tocar búsqueda o scoring. La configuración oficial `focalA2RealityAuxiliaryPolicy.participantPresencePreference` continúa en `OFF`. El baseline protegido conserva 53 tareas, cero pendientes, hard validity, 12 operaciones/375 minutos, 85.557 ramas, 3.565 minutos de permanencia, 900 productivos, 2.665 idle, máximo idle 380, gap máximo 225 y 28 gaps.
- **Matriz reproducible:** cada una de `OFF`, `LOW`, `MEDIUM`, `HIGH` y `MAXIMUM` parte de una proyección nueva, declara `EXACT_CONSTRUCTIVE`, aplica la política sólo a esa copia y ejecuta dos planes normales y uno con corpus, tareas, participantes, espacios, recursos, coaches, operaciones ancladas y ventanas invertidos. Las cinco completan 53 tareas, cero pendientes, 12 operaciones, 375 minutos y hard validity; todas consumen 85.557 ramas (48.032 core + 37.525 standalone; 2.709 forward + 34.816 leaf), son deterministas, invariantes, inmutables y no usan fallback. No hubo políticas incompletas ni agotadas; la atomicidad de fallo queda comprobada por el informe aunque no se observó un fallo.
- **Evidence `OFF`:** presencia 3.565, productive 900, idle 2.665 (`0.7475455820476858`), máximo de permanencia 440, máximo idle 380, gap máximo 225, 28 gaps y 34 cambios de espacio. Conserva core `0948b758c96f17ec546c331ce6d8b42464dbdbe95970d0640ae5fbea95fdbae9`, plan `825314fa954d2511c5cef2bcd2d6f988efe560131c6039dc99d4ba4c80b662b4` y calidad `39163b08d0f377ddd1bbc5bcb5ad1332d9f0c18b699643e0453cdddf547fda12`.
- **Evidence de políticas activas:** los cuatro niveles producen exactamente el mismo primer plan: presencia 3.515, productive 900, idle 2.615 (`0.7439544807965861`), máximo de permanencia 440, máximo idle 380, gap máximo 225, 28 gaps y 34 cambios de espacio. Comparten core `0948b758c96f17ec546c331ce6d8b42464dbdbe95970d0640ae5fbea95fdbae9`, plan `fded1fd188ba3daa833f68ce74533e6db43fd6e801d64f7f4cebea42aa5224d6` y calidad `a64f641fcde8d470808a1b3e2eda986b5a99390600dd5c70ab189d37fc16189f`.
- **Deltas contra `OFF`:** la convención explícita es que un delta negativo en idle/gap significa mejora y uno positivo empeoramiento. Cada política activa obtiene permanencia −50, idle −50, ratio −0,0035911012510997375, máximo idle ±0, máximo gap ±0, gaps ±0, cambios de espacio ±0 y ramas/core/standalone ±0. Mejoran `nela-garcia` (−35 idle/permanencia) y `pere-portero` (−15); no empeora ningún participante y los otros 17 quedan sin cambio.
- **Gaps modificados:** desaparecen Nela operación 07→vocal de 170 y Pere operación 04→vocal de 105, sustituidos por gaps de 135 y 90 respectivamente. En Gisela se intercambian las separaciones vocal→operación 05 y operación 05→main (60/5 pasa a 5/60), por lo que cambian las identidades temporales de esos gaps sin cambiar su total ni su máximo. No se ocultan como “eliminación neta”: el informe publica cuatro gaps retirados y cuatro nuevos con participante, tareas y duración.
- **Clases, elegibilidad y comparador:** aparecen dos clases reproducibles: `{OFF}` y `{LOW, MEDIUM, HIGH, MAXIMUM}`. Las cinco políticas son elegibles bajo el límite de 300.000 ramas. El comparador externo y lexicográfico (idle total, máximo idle, máximo gap, gaps, cambios de espacio, ramas y nombre estable) elige técnicamente `HIGH`, porque los cuatro niveles activos empatan en toda Evidence anterior y `HIGH` es el primer nombre lexicográfico. Sólo es el mejor primer plan entre estas configuraciones existentes según la Evidence declarada; no es un óptimo global ni un scoring de producción.
- **Alcance y ausencia de cambio productivo:** el benchmark experimental es adicional y el benchmark de calidad `OFF` permanece intacto. No cambian la configuración oficial, defaults, `scoreAuxiliaryTask`, `participantPresenceIncrement`, comparadores, DFS, forward checking, presupuestos, reglas hard, fingerprints aceptados, motor, DB/RLS, UI, routing ni `PlanResult`; tampoco se usa el horario humano como seed. Por tanto no se afirma que el plan oficial haya mejorado.
- **Recomendación única para SPEC09-008 (caso C):** tratar todos los niveles positivos como una sola clase conductual, no calibrar sus pesos y evaluar si el coste local resulta insuficiente. Aunque la clase activa reduce 50 minutos de idle sin regresión en máximos o presupuesto, este experimento no distingue un peso positivo de otro; la siguiente unidad debe preservar esa conclusión y no activar aún configuración ni modificar el algoritmo dentro de esta iteración.

### SPEC09-008 — Adopción de la clase positiva mínima en el baseline exacto

- **Objetivo y Evidence de partida:** se adopta exclusivamente para los benchmarks de la ruta interna completa `EXACT_CONSTRUCTIVE` de Focal A2 la clase positiva demostrada por SPEC09-007. El experimento preservado identifica dos clases conductuales en el escenario representativo actual: `{OFF}` y `{LOW, MEDIUM, HIGH, MAXIMUM}`. Todos los niveles positivos producen el mismo plan, calidad, ramas y Evidence; esta equivalencia no se generaliza a escenarios futuros ni demuestra un óptimo global.
- **Configuración explícita y aislada:** una factory nueva proyecta siempre un problema nuevo, declara `EXACT_CONSTRUCTIVE`, copia la política auxiliar y fija `participantPresencePreference = LOW` sin mutar el corpus ni constantes compartidas. La configuración histórica neutral `focalA2RealityAuxiliaryPolicy` permanece en `OFF`; no cambian el default oficial, el default del motor, la ruta histórica, scoring, búsqueda ni motor, y no existe fallback.
- **Por qué `LOW`:** es el representante canónico de intervención mínima suficiente de la clase positiva observada. `MEDIUM`, `HIGH` y `MAXIMUM` no aportaron diferencia medible; el ganador técnico `HIGH` de SPEC09-007 procedía únicamente del desempate lexicográfico del nombre. Elegir `LOW` evita fingir una calibración de pesos inexistente, sin afirmar que sea globalmente óptimo.
- **Nuevo baseline exacto aceptado:** conserva `COMPLETE`, 53 tareas —19 main, 19 vocal, 9 standalone y 6 segmentos anclados—, cero pendientes, 3 operaciones ancladas, 12 operaciones itinerantes, 375 minutos, hard validity y 85.557 ramas. Mantiene el core fingerprint `0948b758c96f17ec546c331ce6d8b42464dbdbe95970d0640ae5fbea95fdbae9`; adopta el full fingerprint `fded1fd188ba3daa833f68ce74533e6db43fd6e801d64f7f4cebea42aa5224d6` y el quality fingerprint `a64f641fcde8d470808a1b3e2eda986b5a99390600dd5c70ab189d37fc16189f`. Determinismo, invariancia al orden e inmutabilidad quedan protegidos.
- **Resultado operativo:** frente a `OFF`, la presencia e idle totales bajan 50 minutos, de 3.565/2.665 a 3.515/2.615, con 900 productivos y ratio idle `0.7439544807965861`. Mejoran `nela-garcia` (−35 minutos) y `pere-portero` (−15); 17 participantes no cambian y ninguno empeora. No mejoran los máximos: permanencia 440, idle 380 y gap 225; permanecen 28 gaps y 34 cambios de espacio, sin incremento de ramas.
- **Protecciones y alcance:** el experimento SPEC09-007 continúa usando `OFF` como baseline experimental con full fingerprint `825314fa954d2511c5cef2bcd2d6f988efe560131c6039dc99d4ba4c80b662b4` y quality fingerprint `39163b08d0f377ddd1bbc5bcb5ad1332d9f0c18b699643e0453cdddf547fda12`. El core aislado y la ruta histórica SPEC08 permanecen idénticos. No hay cambios de DB/RLS, UI, contratos, routing, `PlanResult`, presupuestos, comparadores, heurísticas ni algoritmos.
- **Validación:** la configuración cuenta con pruebas de aislamiento `OFF`/`LOW`, copias nuevas, corpus y ventanas inmutables, orden invertido, política explícita, metadata sin cambio de defaults y ausencia de fallback. Los benchmarks exacto y de calidad exigen el nuevo baseline y ejecutan dos entradas normales y una invertida; además se preservan mediante doble ejecución el core aislado, el experimento SPEC09-007 y la ruta histórica.
- **Deuda restante:** la política local de permanencia aporta una mejora gratuita, pero solo elimina 50 de los 2.665 minutos idle iniciales; no reduce el máximo idle, no reduce el gap máximo y no reduce el número de gaps. La siguiente unidad deberá estudiar el contexto global responsable de los 2.615 minutos idle restantes y no debe seguir calibrando pesos positivos. La calidad no está resuelta y el razonamiento operativo no se da por terminado.

### SPEC09-009 — Experimento de ordenación con obligaciones residuales

- **Corrección SPEC09-009R1 antes del merge:** la primera Evidence experimental quedó invalidada al detectarse dos defectos del estimador: la caché usaba solo `depth:slot:mainTaskId` y podía reutilizar una clave después de backtracking aunque `placedTasks` fuese distinto; además, productive, idle y gaps se calculaban sobre intervalos crudos, contando dos veces solapamientos y creando gaps falsos entre intervalos contenidos. Los resultados publicados en esta sección sustituyen, y no acumulan, aquella Evidence provisional.
- **Problema y alcance (`DB Safe Merge`):** el orden exacto estable del core —menor `participantSlack`, menor `firstObligation`, ID— solo ve la operación main local. Se añade exclusivamente un callback interno, opcional y determinista para estudiar el contexto de una **obligación residual**: una tarea standalone validada por la forma exacta, pendiente, del mismo participante y no incluida en la operación materializada. La API pública, el forward checking causal, la DFS standalone, `canPlaceTask`, restricciones, presupuesto y política productiva no cambian; el experimento no quedó activado.
- **Dominio estático y cota del feeder:** por cada obligación residual se enumeran starts en rejilla de cinco minutos que caben simultáneamente en jornada y disponibilidades de tarea, participante, espacio, todos los recursos requeridos y coach cuando aplica. No se consultan ocupaciones, comidas, transiciones contra ocupaciones, dependencias, locks ni `canPlaceTask`; por ello disponibilidad no equivale a llegada y el dominio no prueba validez hard. El intervalo provisional reúne obligaciones ya colocadas, operación candidata y el feeder ideal más tardío, con `transition = max(participantTransitionMinutes, resourceTransitionMinutes)`, `idealFeederEnd = firstObligation - transition` e `idealFeederStart = idealFeederEnd - feeder.duration`. Es una cota inferior de calidad, nunca un start consolidado.
- **Estimación, unión y clave exacta:** los intervalos sin duración positiva se ignoran; los restantes se copian, ordenan por start/end y fusionan cuando `next.start <= current.end`, de modo que solapamientos, contención y adyacencias forman una única unión productiva. Span, productive, idle y gap máximo se calculan sobre esa unión; la distancia compara el residual solo contra la unión previa y vale cero al solapar o ser adyacente. El feeder ideal participa en la misma unión. Cada start residual se compara por menor permanencia resultante, gap máximo, incremento idle, distancia a la obligación más cercana y start. La agregación y la clave permanecen `(emptyStaticDomainCount, projectedPresenceLowerBound, projectedMaximumGapLowerBound, sumIndependentIdleExpansion, sumIndependentPresenceExpansion, sumIndependentMinimumGap, participantSlack, firstObligation, candidateId)`. Las posiciones individuales pueden competir por recursos o ser incompatibles: no prueban viabilidad u optimalidad, no eliminan candidatos, no cortan con `bestK` y no consumen ledger. Una referencia explícita inexistente de participante, espacio, recurso o coach produce dominio estático vacío sin excepción ni poda; solo `task.availability === undefined` significa ausencia de restricción propia.
- **Caché y Evidence causal corregidas:** cada instancia readonly de `ExactMainChoiceDescriptor` identifica exactamente un estado y se cachea solo dentro del orderer con `WeakMap` (`cacheScope = DESCRIPTOR_STATE`); comparaciones repetidas reutilizan la evaluación, mientras otro descriptor en la misma profundidad/slot/ID se evalúa de nuevo porque sus tareas colocadas pueden diferir. Los tests cubren estados de ramas en ambos órdenes, intervalo contenido, solapamiento parcial, adyacencia, feeder contenido y referencias incompletas. El fixture de dos participantes cambia el primer candidato, conserva cinco tareas y hard validity, reduce idle, es determinista/invariante e inmutable, recupera por backtracking y protege B/B−1. La Evidence corregida registra 33 decisiones, 235 candidatos (135 con residuales y 100 sin ellas), 27 órdenes cambiados, 22 primeros candidatos cambiados, 13.944 evaluaciones estáticas, 2.748 starts y cero dominios vacíos; los diagnósticos están limitados determinísticamente.
- **Resultado Focal A2 experimental:** `COMPLETE`, 53 tareas, cero pendientes, 12 operaciones/375 minutos, hard-valid, determinista, invariante e input inmutable. Consume 70.704 ramas (48.224 core y 22.480 standalone). Fingerprints: core seleccionado `44f10279aa01fa7628c01962e9fbdd819d69486ae11df4fe4851de946600f07f`, completo `38309867fb51dcb14515d152035b7076a4738cac04d3d8cea721ec7be0749fa8` y calidad `13a87e0d9b6983c18ca5a0162785058b67b10f8ea65d46644463f49063791c75`. Calidad: presencia 3.380, productive 900, idle 2.480, ratio `0.7337278106508875`, máximo de permanencia 480, máximo idle 435, gap máximo 315, 28 gaps y 34 cambios de espacio.
- **Deltas y tradeoffs:** presencia/idle −135, ratio −`0.010226670145698558`, máximo de permanencia +40, máximo idle +55, gap máximo +90, gaps y cambios de espacio ±0, ramas −14.853 (core +192; standalone −15.045). Mejoran Adrian −55, Daniel −30, Eva −15, Lina −45, Linet −305, Moisés −30 y Naomi −30; empeoran Carmen +90, Cristina +30, José Javier +120, Marta +30, Nela +15 y Pere +90; seis participantes quedan iguales. No se crean ni eliminan identidades de gaps. Se mueven 11 main, 11 vocal/feeders y 6 standalone.
- **Clasificación y recomendación:** **CASO B — mejora con tradeoff**: baja el idle total, pero empeoran máximo idle, gap máximo y participantes concretos. Recomendación única: **no activar; analizar equidad y participantes perjudicados** en una iteración posterior, sin añadir comparación de hojas ni otra corrección aquí.
- **Baselines y limitaciones preservados:** sin callback permanecen las 47.482 ramas y fingerprint aislado `c85b9b2cfbbf9434135f08e2b293b0ab6c23e5ff070e880cbb5406022dd52785`; el baseline exacto `LOW` mantiene 85.557 ramas, core seleccionado `0948b758c96f17ec546c331ce6d8b42464dbdbe95970d0640ae5fbea95fdbae9`, completo `fded1fd188ba3daa833f68ce74533e6db43fd6e801d64f7f4cebea42aa5224d6` y calidad `a64f641fcde8d470808a1b3e2eda986b5a99390600dd5c70ab189d37fc16189f`. Se preservan el experimento de políticas y la ruta histórica SPEC08. La señal es estática e independiente por tarea; no modela competencia entre obligaciones ni reemplaza ninguna autoridad hard.

### SPEC09-012 — Simplificación y selección de planes completos

- **Objetivo y alcance (`DB Safe Merge`):** las auditorías SPEC09-010/011 demostraron que aceptar la primera hoja completa no era suficiente; se eliminan sus módulos, tests, trazas, fingerprints y benchmarks después de cumplir esa función. No se convierten en arquitectura permanente: la solución compara exclusivamente planes completos hard-válidos del mismo core.
- **Selección:** `FIRST_HARD_VALID` continúa siendo el default histórico y conserva la primera hoja. La ruta experimental residual usa `BEST_DOMINATING_WITHIN_BUDGET`: mantiene un único incumbent, valida cada hoja completa y sólo lo sustituye si no empeora máximo idle, gap máximo, idle total, gaps ni cambios de espacio y mejora al menos una métrica. La calidad no ordena, poda, genera starts ni consume ramas.
- **Presupuesto y publicación:** ambas rutas comparten el ledger contractual de 300.000 ramas, sin reinicio ni incremento. Si el presupuesto termina después de observar un incumbent, se publica atómicamente ese plan completo hard-válido y Evidence declara el agotamiento, sin afirmar optimalidad; sin incumbent no se publica un parcial.
- **Resultado Focal A2:** la primera hoja residual permanece en 70.704 ramas, plan `38309867fb51dcb14515d152035b7076a4738cac04d3d8cea721ec7be0749fa8`, calidad `13a87e0d9b6983c18ca5a0162785058b67b10f8ea65d46644463f49063791c75` e idle/máximo idle/gap máximo `2480/435/315`. La selección observa 78 completitudes en 300.000 ramas (48.224 core + 251.776 standalone), reemplaza dos veces y conserva `b5b1fc1fe3b1813e425b26b22cbf7932604718f1b194eb00a8e909f0937f7357`, calidad `256244c1ccad494ca319d921dfcdc8c696b54a4b16506d42567f2e29abb5657b`, con métricas `2390/365/225`, 28 gaps y 34 cambios. Conserva 53 tareas, cero pendientes, el mismo core de 44 tareas, las mismas comidas y hard validity; repetición e input invertido son idénticos e inmutables. Resultado: `IMPROVES_FIRST_COMPLETE`.
- **Coste, limpieza y límites:** las nueve ejecuciones del benchmark tardaron aproximadamente 0,16–0,94 s cada una en este entorno; la selección terminó por presupuesto y representa el **mejor plan completo observado dentro del presupuesto**, no un óptimo global. Se eliminaron claramente más líneas de las añadidas, sin auditorías sustitutas, hardcodes Focal en el motor, pesos, fronteras, nueva heurística, cambio de orden standalone/core ni aumento de presupuesto. Los próximos pasos sólo se justifican si este plan mejora la operativa real.
- **Validación:** tests sintéticos protegen dominancia, empate y tradeoffs; default/selección explícita; múltiples reemplazos; orden, ledger y fronteras con/sin incumbent; atomicidad, hard validity, determinismo, invariancia e inmutabilidad. Se ejecutaron además suites Planner Next y general, compilación, build y dos pasadas de los benchmarks protegidos.

### SPEC09-013 — Aceptación operativa contra planning humano

- **Propósito y alcance (`DB Safe Merge`):** el benchmark de calidad reconstruye la referencia humana Focal A2 y compara directamente el candidato SPEC09-012 con el baseline aceptado y con esa referencia. Esta iteración sólo amplía Evidence de validación: no modifica el motor, sus selectores, contratos, fixtures, referencias, configuración ni presupuestos, y no activa el candidato.
- **Integridad y validación humana:** se materializan exclusivamente desde las referencias existentes los mismos 53 IDs únicos del problema, sin ausentes, adicionales, duplicados ni duraciones discordantes: 19 main, 19 vocal, 9 standalone y 6 segmentos de tres acompañamientos anclados completos; los anchors y segmentos aparecen exactamente una vez. Las 12 operaciones itinerantes suman 375 minutos. La comida documental `840–915` se materializa con el contrato canónico y `validatePlan` confirma `hardValid: true`, sin reason codes. No se detecta incoherencia entre fixture y referencia documental.
- **Tabla de calidad completa:** baseline: presencia/productivo/idle `3515/900/2615`, ratio idle `0.7439544807965861`, máximos de presencia/idle/gap `440/380/225`, 19 participantes con gaps, 28 gaps y 34 cambios de espacio. Candidato: `3290/900/2390`, ratio `0.7264437689969605`, máximos `425/365/225`, 19, 28 y 34. Humano: `3590/900/2690`, ratio `0.7493036211699164`, máximos `480/435/315`, 19, 28 y 34. En las cinco dimensiones sin pesos ni umbrales el resultado calculado es `CANDIDATE_DOMINATES_HUMAN_REFERENCE`; esto describe estas métricas y no afirma causalidad ni optimalidad global.
- **Participantes y tradeoffs frente al baseline:** 8 `IMPROVED`, 4 `WORSENED`, 1 `MIXED` y 6 `UNCHANGED`; las reducciones/aumentos brutos de presencia e idle son `570/345` minutos. Los cinco mayores perjuicios publicados son José Javier Cuenca (`+120` presencia, `+120` idle, `+120` gap máximo), Marta Fonrali (`+90/+90/+30`), Pere Portero (`+90/+90/+30`), Cristina Zuloaga (`+30/+30/+30`) y Nela García (`+15/+15/−15`, `MIXED`). El agregado favorable no oculta estos daños individuales.
- **Envolvente humana:** 17 participantes quedan completamente dentro; ninguno empieza antes del inicio humano y dos terminan después: Linet Varela `+15` minutos y Marta Fonrali `+45`. Los 19 spans candidatos son menores que sus envolventes documentales, pero esas dos extensiones finales son advertencias informativas, no fallos hard.
- **Diferencias temporales frente al humano:** main: 2 inicios iguales, 8 anteriores y 9 posteriores, diferencia absoluta media/máxima `45.78947368421053/225` minutos; vocal: `6/6/7` y `49.473684210526315/305`; standalone: `2/5/2` y `48.333333333333336/90`; segmentos anclados: `0/0/6` y `30/45`. Las 12 operaciones itinerantes candidatas tienen 2 inicios iguales, 5 anteriores y 5 posteriores, diferencia absoluta media/máxima `43.75/90` (baseline: `3/4/5`, `30/90`). Esta distancia es informativa y no alimenta semilla, orden, objetivo ni poda.
- **Flujo main y resultado operativo:** humano, baseline y candidato mantienen 19 main consecutivos, tramo matinal de 165 minutos, comida autorizada `840–915`, cero huecos no autorizados, tres cambios de bloque de coach y continuidad requerida. El candidato conserva 53 tareas, cero pendientes, hard validity, determinismo, invariancia e input inmutable; domina al baseline aceptado. La decisión derivada es `READY_FOR_HUMAN_REVIEW`, con `operationalWarnings: true` por participantes perjudicados y dos finales fuera de envolvente. La decisión final corresponde al humano y el benchmark confirma `candidateActivated: false`.
- **Limitaciones y recomendación:** la comparación cubre una instancia y un presupuesto finito de 300.000 ramas; la referencia humana es informativa y algunas disponibilidades del fixture son deliberadamente neutrales. La dominancia agregada no elimina los tradeoffs individuales ni demuestra causalidad. Recomendación única: someter el candidato a revisión humana explícita, atendiendo primero a José Javier, Marta, Pere, Cristina, Nela y a las extensiones de Linet/Marta, sin activarlo automáticamente.
- **Validación ejecutada:** `npm ci`, `npm run check`, `npm run build`, dos ejecuciones de los benchmarks de calidad y selección completa, suite Planner Next, suite general y benchmarks protegidos de core exacto, exact itinerant, políticas de presencia, orden residual y ruta histórica SPEC08.

### SPEC09-014 — Promoción de la política exacta aceptada

- **Ruta aceptada (`DB Safe Merge`):** `constructExactItinerantPlan` compone el orderer residual estático existente con `BEST_DOMINATING_WITHIN_BUDGET`, un único incumbent, comparación exclusiva de planes completos hard-valid y el presupuesto compartido. Si no hay tareas standalone conserva estructuralmente `FIRST_HARD_VALID`. No se añadieron heurísticas ni políticas, no se modificaron búsqueda, comparador, métricas o `COMPATIBILITY_PRESERVING`.
- **Compatibilidad y rollback:** `constructFirstHardValidExactItinerantPlan` congela la llamada histórica `runExactItinerantPlanSearch(problem, {})` para control, comparación y rollback. En Focal A2 mantiene 85.557 ramas, core `0948b758c96f17ec546c331ce6d8b42464dbdbe95970d0640ae5fbea95fdbae9`, plan `fded1fd188ba3daa833f68ce74533e6db43fd6e801d64f7f4cebea42aa5224d6` y calidad `a64f641fcde8d470808a1b3e2eda986b5a99390600dd5c70ab189d37fc16189f`.
- **Evidence aceptada:** la ruta pública devuelve 53 tareas, cero pendientes y hard validity en 300.000 ramas (48.224 core y 251.776 standalone), tras observar 78 completitudes y efectuar dos reemplazos. El primer plan es `38309867fb51dcb14515d152035b7076a4738cac04d3d8cea721ec7be0749fa8`; el seleccionado/completo es `b5b1fc1fe3b1813e425b26b22cbf7932604718f1b194eb00a8e909f0937f7357` y su calidad `256244c1ccad494ca319d921dfcdc8c696b54a4b16506d42567f2e29abb5657b`. Sus métricas presencia/productivo/idle son `3290/900/2390`, máximos de presencia/idle/gap `425/365/225`, con 28 gaps y 34 cambios; es el mejor incumbent dominante observado dentro del presupuesto, no una afirmación de optimalidad.
- **Tradeoffs y limpieza:** se preservan como advertencias los cuatro participantes perjudicados, el caso mixto y las dos extensiones respecto de la envolvente humana documentados en SPEC09-013; el rollback histórico permanece explícito. Se eliminó el benchmark experimental de selección completa y sus protecciones pasaron al test exacto y a los benchmarks autoritativo y de calidad.
- **Validación:** tests exactos cubren compatibilidad, activación con standalone, fallback sin standalone, atomicidad, hard validity, presupuesto con/sin incumbent, determinismo, invariancia e input inmutable. Pasaron typecheck, build, suites Planner Next/general, dos ejecuciones de ambos benchmarks Focal A2 y los benchmarks protegidos de core exacto, SPEC08, presencia, orden residual y compatibilidad.

### SPEC09-015 — Dispatcher explícito de políticas

- **Objetivo e implementación (`DB Safe Merge`):** `executePlannerNext` es el nuevo entrypoint autoritativo: resuelve la política antes de planificar y publica un envelope discriminado `POLICY_REJECTED`, `COMPATIBILITY_PRESERVING` o `EXACT_CONSTRUCTIVE`, manteniendo juntas la resolución y la Evidence propia del único resultado ejecutado.
- **Selección y rechazo:** la política explícita despacha exclusivamente al motor correspondiente; una incompatibilidad se rechaza antes de ejecutar motor. La ausencia de política conserva temporalmente `COMPATIBILITY_PRESERVING`, `MIGRATION_DEFAULT` y su warning de deprecación, sin promover implícitamente AnchoredAccompaniment a exacta. No hay fallback entre motores.
- **Compatibilidad y alcance:** `planMainFlowAndFeeders` permanece como entrypoint histórico de compatibilidad e ignora `searchPolicy`. Focal A2 exacto se ejecuta ya mediante el dispatcher y publica `policyResolution`; no cambian motores, algoritmos, contratos, UI, DB ni RLS, y no se adapta el resultado exacto a `PlanResult`.
- **Validación:** tests sintéticos cubren ambas políticas explícitas, migration default, rechazos explícito e implícito, capacidad exacta, separación de resultados, equivalencia legacy, determinismo, invariancia e input inmutable; se conservan las suites y benchmarks históricos y el benchmark exacto Focal A2.

### SPEC10-001 — Preflight de representabilidad de EngineInput

- **Objetivo y alcance (`DB Safe Merge`):** preflight puro, determinista y profundamente read-only de `EngineInput`; materializa identidades y audita contratos inspeccionables sin crear `PlannerNextProblem`, adaptar entidades, ejecutar Planner Next/ORC/V3/V4 ni producir planificación. Política, presupuesto, main flow, rejilla y operaciones ancladas runtime sólo cuentan como presentes cuando su contenido completo es válido; valores inválidos generan Evidence explícita y nunca defaults.
- **Frontera auditada:** distingue duración ausente/inválida, duplicado de dependencia/ciclo, definiciones/referencias de recursos, tiempos reales/planificados protegidos sin mezclarlos, cámaras por cantidad, ocupación shared/non-blocking, coaches, componentes, asignaciones, transporte, setups y todos los scopes estructurados de comidas/breaks. El fingerprint proyecta exclusivamente esos campos, reduce planning pending a `hasDiscardablePriorPlanning` y excluye nombres, bundles y señales soft no auditadas.
- **`real-main-stage-with-backlog` — `UNSUPPORTED`:** `MISSING_MAIN_FLOW_CONFIGURATION`, `MISSING_PARTICIPANT_AVAILABILITY`, `MISSING_RESOURCE_AVAILABILITY`, `MISSING_SEARCH_BUDGET_CONFIGURATION`, `MISSING_SEARCH_POLICY_CONFIGURATION`, `MISSING_SPACE_REFERENCE`, `MISSING_TASK_DURATION`, `UNSUPPORTED_RESOURCE_REQUIREMENT`, `UNSUPPORTED_SETUP_MAPPING`, `UNSUPPORTED_TASK_ROLE`, `UNSUPPORTED_TIME_GRID`. Diagnóstico: 3 planificables, 2 horarios descartables, 2 participantes, 1 espacio referenciado/0 descritos, 3 recursos definidos, 2 duraciones ausentes, 2 asignaciones, 1 comida concreta y setup explícito. Source `74fb3271d7a4441c2ea4b4c0c04db7193b4a9bf69d5a88bdf8a4850e260f0812`; identities `68a72d0ac8f1d2246d5a7a8132c0090b43d76bb9a2dcb59f9f4b1cdb7b5c3b89`.
- **`real-resource-lock-pressure` — `UNSUPPORTED`:** `MISSING_MAIN_FLOW_CONFIGURATION`, `MISSING_PARTICIPANT_AVAILABILITY`, `MISSING_RESOURCE_AVAILABILITY`, `MISSING_SEARCH_BUDGET_CONFIGURATION`, `MISSING_SEARCH_POLICY_CONFIGURATION`, `MISSING_SPACE_REFERENCE`, `MISSING_TASK_DURATION`, `UNREPRESENTABLE_RESOURCE_LOCK`, `UNREPRESENTABLE_SPACE_LOCK`, `UNSUPPORTED_SETUP_MAPPING`, `UNSUPPORTED_TASK_ROLE`, `UNSUPPORTED_TIME_GRID`. Diagnóstico: 3 planificables/horarios descartables/participantes/espacios referenciados/recursos definidos, 0 espacios descritos, 3 duraciones ausentes, 3 asignaciones, 1 comida concreta y setup explícito; el lock `full` conserva tiempo pero no espacio/recurso explícitos representables. Source `968d5e471bfe73c829ec5ac1a1c35c2f8e1a1f617310b26c0543fcd0755fb9c0`; identities `794472d65522cebf41bbc687d25dccb474e8579766fd01de4a45fda48941cc65`.
- **`real-protected-break-recovery` — `UNSUPPORTED`:** `MISSING_MAIN_FLOW_CONFIGURATION`, `MISSING_PARTICIPANT_AVAILABILITY`, `MISSING_RESOURCE_AVAILABILITY`, `MISSING_SEARCH_BUDGET_CONFIGURATION`, `MISSING_SEARCH_POLICY_CONFIGURATION`, `MISSING_SPACE_REFERENCE`, `MISSING_TASK_DURATION`, `UNSUPPORTED_RESOURCE_REQUIREMENT`, `UNSUPPORTED_SETUP_MAPPING`, `UNSUPPORTED_TASK_ROLE`, `UNSUPPORTED_TIME_GRID`. Diagnóstico: 3 planificables, 2 horarios descartables/participantes/espacios referenciados/duraciones ausentes/asignaciones, 0 espacios descritos, 3 recursos definidos, 1 break equivalente a la comida legacy y setup explícito. Source `852d269d9f0c873423001cc173d6cf0c9918e6fd0f9e2b64761198badf9fc56b`; identities `fd6782b3ba5a6104c13e70baeff7303baf48b9f1561eea76e37e4a33f58eb01a`.
- **Evidence y siguiente decisión:** 85 tests nominales y el benchmark congelan status, issues, reason codes, diagnostics, mapas, fingerprints, input y output; repetición e inversión integral —incluidas operaciones ancladas— son idénticas. La Evidence confirma que antes de adaptar deben evolucionar contratos autoritativos de rol, catálogos/disponibilidad, rejilla, flujo principal, política, presupuesto, cantidades, ocupación, setup y transporte.

### SPEC10-002 — Contrato tipado de configuración Planner Next

- **Objetivo y contrato (`DB Safe Merge`):** `EngineInput.plannerNext` formaliza una única configuración opcional y DB-agnostic con política `COMPATIBILITY_PRESERVING | EXACT_CONSTRUCTIVE`, presupuesto (`bestK`, `maxBacktracks`, `maxPatterns`, `maxBranchExpansions`), `timeGridMinutes`, transiciones de participante y recurso, y flujo principal (`spaceId` físico, `preferredEnd` `HH:mm`, continuidad requerida y límites de bloques/tareas). Si el objeto existe, todos sus campos son obligatorios en TypeScript; el preflight continúa defendiendo entradas runtime incompletas o inválidas.
- **Frontera y ausencia de defaults:** el preflight lee exclusivamente `EngineInput.plannerNext`, ignora canales top-level legacy y no infiere política, presupuesto, rejilla, transiciones ni espacio. Esta iteración no conecta Settings, DB ni `buildEngineInput`, no construye/adapta `PlannerNextProblem` y no ejecuta Planner Next ni modifica la ruta productiva.
- **Evidence de integración:** se añaden `MISSING_TRANSITION_CONFIGURATION` e `INVALID_TRANSITION_CONFIGURATION`, junto con `integrationConfigurationPresent` y `transitionConfigurationComplete`. Los 86 tests específicos, 335 tests Planner Next, 8 tests ORC representativos y 1686 tests globales pasan; cubren contrato ausente, canales legacy ignorados, las dos políticas, budget, grid respecto del inicio de jornada, `preferredEnd`, espacio físico descrito, transiciones —incluido cero—, identidad, fingerprints canónicos, propiedades extra ignoradas, pureza y Evidence representativa congelada. `plannerNext.mainFlow.spaceId` se canonicaliza mediante `String(...)` exclusivamente en la proyección privada de `identityMapFingerprint`: `2` y el runtime inválido `"2"` conservan el mismo identity map y su huella, mientras el tipo inválido cambia `sourceFingerprint`, mantiene `MISSING_MAIN_FLOW_CONFIGURATION` y nunca se acepta como soportado.
- **Escenarios representativos:** los tres permanecen `UNSUPPORTED` y añaden `MISSING_TRANSITION_CONFIGURATION` a sus reason codes previos. Sus fingerprints permanecen confirmados: `real-main-stage-with-backlog` source `74fb3271d7a4441c2ea4b4c0c04db7193b4a9bf69d5a88bdf8a4850e260f0812` / identities `68a72d0ac8f1d2246d5a7a8132c0090b43d76bb9a2dcb59f9f4b1cdb7b5c3b89`; `real-resource-lock-pressure` source `968d5e471bfe73c829ec5ac1a1c35c2f8e1a1f617310b26c0543fcd0755fb9c0` / identities `794472d65522cebf41bbc687d25dccb474e8579766fd01de4a45fda48941cc65`; `real-protected-break-recovery` source `852d269d9f0c873423001cc173d6cf0c9918e6fd0f9e2b64761198badf9fc56b` / identities `fd6782b3ba5a6104c13e70baeff7303baf48b9f1561eea76e37e4a33f58eb01a`.
- **Validación y siguiente decisión:** dos ejecuciones del benchmark SPEC10-002 fueron byte a byte idénticas y ambas produjeron SHA-256 `cd861174412aab59565ffab4f868f94f23e40d2ca32972fe96cf41fd46326058`. Focal A2 exacto terminó `COMPLETE`, con 53 tareas, 0 pendientes, hard validity, input inmutable, determinismo e invariancia de orden, política `EXACT_CONSTRUCTIVE`, configuración `LOW` sin fallback y 300000 ramas; la regresión de calidad mantuvo candidato `COMPLETE`/hard-valid, fingerprint `b5b1fc1fe3b1813e425b26b22cbf7932604718f1b194eb00a8e909f0937f7357`, quality fingerprint `256244c1ccad494ca319d921dfcdc8c696b54a4b16506d42567f2e29abb5657b`, presencia/productivo/idle `3290/900/2390`, 78 planes completos y 2 reemplazos, dominando baseline y referencia humana. La siguiente decisión, antes de cualquier adaptador, debe basarse en esta Evidence para definir cómo materializar explícitamente el contrato desde una fuente productiva autorizada, sin defaults ni fallback.

### SPEC10-003 — Clasificación explícita de tareas para Planner Next

- **Objetivo y contrato (`DB Safe Merge`):** `TaskInput.plannerNextKind` añade una clasificación opcional, tipada y DB-agnostic con exactamente `main`, `vocal`, `auxiliary` y `technical`. Es independiente de `operationalRole`: éste conserva la semántica operativa y no productiva existente, mientras el nuevo campo expresa sólo la categoría contractual de Planner Next. El preflight lee exclusivamente el campo explícito, sin inferirlo desde nombres, IDs, plantillas, espacios, zonas, flags, recursos, dependencias o escenarios.
- **Compatibilidad auditada:** toda tarea no cancelada exige un kind válido. `main`, `vocal` y `auxiliary` exigen un `contestantId` finito, entero y positivo; su ausencia o invalidez conserva el kind reconocido y emite `MISSING_PARTICIPANT_REFERENCE`. `technical` no acepta una relación de participante y la rechaza mediante `UNSUPPORTED_TASK_ROLE` sin eliminarla ni mutar el input. Las tareas `cancelled` quedan exentas y su kind no participa en issues, diagnostics, identidad ni fingerprints. El kind de una tarea activa sí participa en `sourceFingerprint`, pero nunca en `identityMap` o `identityMapFingerprint`.
- **Alcance y exclusiones:** no hay wiring con `buildInput`, fuente DB/Settings, defaults, clasificación de fixtures reales, adaptador, `PlannerNextProblem` ni ejecución de Planner Next; tampoco cambian ruta productiva, planificadores, ORC, V3, V4, UI, API, DB o RLS. Los coaches quedan expresamente pendientes de una unidad posterior basada en Evidence.
- **Evidence representativa:** los tres escenarios reales permanecen honestamente `UNSUPPORTED`, conservan los reason codes y diagnostics de SPEC10-002 —incluido `UNSUPPORTED_TASK_ROLE`— y mantienen source/identity fingerprints: `real-main-stage-with-backlog` `74fb3271d7a4441c2ea4b4c0c04db7193b4a9bf69d5a88bdf8a4850e260f0812` / `68a72d0ac8f1d2246d5a7a8132c0090b43d76bb9a2dcb59f9f4b1cdb7b5c3b89`; `real-resource-lock-pressure` `968d5e471bfe73c829ec5ac1a1c35c2f8e1a1f617310b26c0543fcd0755fb9c0` / `794472d65522cebf41bbc687d25dccb474e8579766fd01de4a45fda48941cc65`; `real-protected-break-recovery` `852d269d9f0c873423001cc173d6cf0c9918e6fd0f9e2b64761198badf9fc56b` / `fd6782b3ba5a6104c13e70baeff7303baf48b9f1561eea76e37e4a33f58eb01a`.
- **Validación y siguiente decisión:** la validación local completa terminó con exit code 0: preflight 139/139 tests (0 fallidos, 0 suites), Planner Next 388/388 (0 fallidos, 1 suite), escenarios ORC 8/8 (0 fallidos, 0 suites) y `npm test` 1739/1739 (0 fallidos, 8 suites); también pasaron instalación, TypeScript check, production build y `git diff --check`. Dos ejecuciones del benchmark SPEC10-003, de 39.288 bytes cada una, fueron idénticas byte a byte y produjeron ambas SHA-256 `176eb3f25c944eedf433b653d8c90cd0c5741fd4f8d2573f0332618d6f50b630`; los source/identity fingerprints permanecen `74fb3271d7a4441c2ea4b4c0c04db7193b4a9bf69d5a88bdf8a4850e260f0812` / `68a72d0ac8f1d2246d5a7a8132c0090b43d76bb9a2dcb59f9f4b1cdb7b5c3b89`, `968d5e471bfe73c829ec5ac1a1c35c2f8e1a1f617310b26c0543fcd0755fb9c0` / `794472d65522cebf41bbc687d25dccb474e8579766fd01de4a45fda48941cc65` y `852d269d9f0c873423001cc173d6cf0c9918e6fd0f9e2b64761198badf9fc56b` / `fd6782b3ba5a6104c13e70baeff7303baf48b9f1561eea76e37e4a33f58eb01a`. Focal A2 exacto terminó `COMPLETE`: 53 tareas totales/planificadas, 0 pendientes, hard-valid, determinista, invariante al orden e input inmutable, política `EXACT_CONSTRUCTIVE`, configuración `LOW`, sin fallback, 300.000 ramas y fingerprint final `b5b1fc1fe3b1813e425b26b22cbf7932604718f1b194eb00a8e909f0937f7357`. El benchmark de calidad confirmó candidato `COMPLETE` y hard-valid, decisión `READY_FOR_HUMAN_REVIEW`, fingerprints de plan/calidad `b5b1fc1fe3b1813e425b26b22cbf7932604718f1b194eb00a8e909f0937f7357` / `256244c1ccad494ca319d921dfcdc8c696b54a4b16506d42567f2e29abb5657b`, presencia/productivo/idle `3290/900/2390`, 78 planes completos y 2 reemplazos, con `CANDIDATE_DOMINATES_BASELINE` y `CANDIDATE_DOMINATES_HUMAN_REFERENCE`. Tests y benchmark confirman pureza y output profundamente frozen; búsquedas de aislamiento confirman ausencia de inferencia, defaults, adaptación, wiring productivo y ejecución de Planner Next/ORC/V3/V4 desde el preflight. La siguiente decisión debe definir, mediante Evidence separada, el contrato explícito de coach antes de diseñar adaptación o wiring productivo.

### SPEC10-004 — Preservación de la asignación fija concursante–vocal coach

- **Problema corregido y fuente autoritativa (`DB Safe Merge`):** `buildEngineInput` ya cargaba la asignación explícita de la app desde el campo persistido existente `contestants.vocalCoachPlanResourceItemId` —también tolera la forma de fila `vocal_coach_plan_resource_item_id`—, cuyo valor referencia exactamente `plan_resource_items.id`, pero antes la reducía al agregado global `coachResourceIds` y perdía la asociación individual. `EngineInput.vocalCoachPlanResourceItemIdByContestantId` conserva ahora `contestants.id -> plan_resource_items.id`; `coachResourceIds` permanece sólo como agregado legacy compatible, ordenado y derivado de los valores válidos.
- **Semántica y límites:** la asignación procede exclusivamente de la configuración humana, es fija y no intercambiable; dos o más concursantes pueden compartir el mismo coach sin colisión. No hay inferencia desde nombres, tipos, categorías, posiciones, tareas, plantillas, espacios ni el agregado legacy; tampoco se decide qué tareas requieren coach, se añade `plannerNextCoachId`, se crea catálogo o disponibilidad paralela, se adapta a `PlannerNextProblem`, ni se ejecuta Planner Next, ORC, V3 o V4. No cambian DB, schema, RLS, UI, rutas ni planificadores.
- **Proyección y auditoría:** el helper puro acepta filas congeladas, valida IDs numéricos finitos, enteros y positivos, conserva camel/snake case sin mutar y materializa claves por concursante en orden ascendente. El preflight distingue ausencia de mapping y `{}`, rechaza sin lanzar contenedores, claves o valores runtime inválidos, verifica el valor sólo contra `planResourceItems[].id` y conserva referencias reversibles `participant:<id>` y `plan-resource:<id>`, incluso para un recurso ausente. La relación participa en `sourceFingerprint`; intercambiar coaches cambia esa huella pero no el conjunto de identidades ni `identityMapFingerprint`. Los diagnostics evitan contar dos veces mapping y agregado derivado; input permanece inmutable y output profundamente frozen.
- **Evidence representativa:** los tres escenarios sin mapping permanecen `UNSUPPORTED`, con reason codes, diagnostics y fingerprints intactos: `real-main-stage-with-backlog` source `74fb3271d7a4441c2ea4b4c0c04db7193b4a9bf69d5a88bdf8a4850e260f0812` / identities `68a72d0ac8f1d2246d5a7a8132c0090b43d76bb9a2dcb59f9f4b1cdb7b5c3b89`; `real-resource-lock-pressure` source `968d5e471bfe73c829ec5ac1a1c35c2f8e1a1f617310b26c0543fcd0755fb9c0` / identities `794472d65522cebf41bbc687d25dccb474e8579766fd01de4a45fda48941cc65`; `real-protected-break-recovery` source `852d269d9f0c873423001cc173d6cf0c9918e6fd0f9e2b64761198badf9fc56b` / identities `fd6782b3ba5a6104c13e70baeff7303baf48b9f1561eea76e37e4a33f58eb01a`.
- **Validación y siguiente decisión:** el preflight valida independientemente ambos extremos: una clave no canónica nunca crea identidad `participant`, pero conserva una referencia positiva válida `plan-resource`, incluso si el recurso no existe; un conjunto separado de referencias positivas auditadas impide que `coachResourceIds` emita un segundo blocker legacy equivalente sin convertir esa relación en válida. Pasaron 14/14 tests focales de proyección, 171/171 del preflight, 420/420 de Planner Next, 8/8 de escenarios ORC y 1785/1785 globales, además de instalación, TypeScript check, production build y `git diff --check`. Dos benchmarks `SPEC10-004-engine-input-preflight` de 39.288 bytes fueron idénticos byte a byte, con SHA-256 `3415df8d5eba5bfd947720c8c3846cc493f3787bd904be529012dba4bd625863`. Focal A2 exacto permaneció `COMPLETE`, hard-valid, determinista, invariante al orden e inmutable: 53 tareas, 0 pendientes, 300.000 ramas y fingerprint `b5b1fc1fe3b1813e425b26b22cbf7932604718f1b194eb00a8e909f0937f7357`; calidad mantuvo decisión `READY_FOR_HUMAN_REVIEW`, fingerprint `256244c1ccad494ca319d921dfcdc8c696b54a4b16506d42567f2e29abb5657b`, presencia/productivo/idle `3290/900/2390`, 78 planes completos y 2 reemplazos, con `CANDIDATE_DOMINATES_BASELINE` y `CANDIDATE_DOMINATES_HUMAN_REFERENCE`. No se añade catálogo paralelo, adaptación ni ejecución; la siguiente decisión debe usar Evidence separada para identificar el requisito operativo efectivo que hace consumir coach a una tarea y, sólo después, proyectar exactamente la asignación fija sin sustituciones.

### SPEC10-005 — Resolución de recursos efectivos por tarea

- **Objetivo y contrato (`DB Safe Merge`):** un único resolvedor puro, reutilizable, determinista, read-only y profundamente frozen prepara para cada tarea activa la unión ordenada y deduplicada de los recursos concretos `plan_resource_items.id` asignados directamente, al espacio físico exacto y a la zona efectiva. Conserva por separado `directResourceIds`, `spaceResourceIds` y `zoneResourceIds`, además de `effectiveResourceIds`, por lo que una referencia repetida se consume una vez sin perder ninguna procedencia.
- **Espacio y zona estructurados:** el espacio se consulta exclusivamente por `task.spaceId`, sin recorrer `spaceParentById` ni heredar de padres. La zona se resuelve sólo desde `task.zoneId` y `zoneIdBySpaceId[task.spaceId]`: ausencia, tarea, mapping y coincidencia quedan explícitos; una contradicción produce `CONFLICT`, no aplica recursos de ninguna zona y el preflight publica el blocker `INCONSISTENT_SPACE_ZONE_REFERENCE` con ambos IDs, conservando los recursos directos y del espacio exacto. No se usan nombres, tipos, categorías, kinds, IDs conocidos ni fixtures para inferir relaciones.
- **Semántica operativa y exclusiones:** `pending`, `interrupted`, `in_progress` y `done` conservan exactamente sus tres niveles de obligación; el resolvedor no altera planificación, espacio ni asignaciones. `cancelled` queda completamente excluida: su asignación directa no afecta resolución, issues, diagnostics, identity map ni fingerprints, mientras los mapas globales siguen auditándose independientemente. Las referencias concretas positivas se conservan aunque falte inventario o `isAvailable` sea falso; el preflight mantiene `MISSING_RESOURCE_REFERENCE` y `MISSING_RESOURCE_AVAILABILITY`, sin inventar ventanas, eliminar ni sustituir recursos.
- **Recursos y coach sin excepciones:** cualquier recurso concreto explícito —incluido un vocal coach— se trata de forma genérica para cualquier tarea y ámbito. La relación de coach del concursante y `coachResourceIds` no crean consumo, no sustituyen una edición humana vigente y las reglas históricas de autocreación no son autoridad. `resourceRequirements.byType`, `byItem` y `anyOf` permanecen separados y bloqueados por el preflight cuando no son representables: no se seleccionan pools, cantidades, alternativas ni primeros recursos.
- **Integración limitada y fingerprints:** el preflight ejecuta el resolvedor una sola vez para auditar conflictos, pero la unión derivada no duplica issues, contadores ni identidades. La misma identidad en tres niveles conserva un único `plan-resource:<id>`; cambiar sólo procedencia cambia `sourceFingerprint` y conserva `identityMapFingerprint` si el conjunto de identidades es el mismo. No se construye `PlannerNextProblem`, no hay adaptación, disponibilidad temporal inventada, recomendación, ejecución ni wiring productivo; no cambian Planner Next search, ORC, V3, V4, UI, API, DB, schema o RLS.
- **Evidence representativa:** 33/33 tests nominales del resolvedor y 180/180 del preflight demuestran unión exacta, procedencia, normalización estricta, ausencia de herencia/inferencia/sustitución, tareas protegidas y canceladas, determinismo, invariancia, pureza e input inmutable. Planner Next terminó 462/462, escenarios ORC 8/8 y la suite global 1827/1827. Los tres escenarios reales permanecen `UNSUPPORTED`, con diagnostics y reason codes intactos: `real-main-stage-with-backlog` source/identity `74fb3271d7a4441c2ea4b4c0c04db7193b4a9bf69d5a88bdf8a4850e260f0812` / `68a72d0ac8f1d2246d5a7a8132c0090b43d76bb9a2dcb59f9f4b1cdb7b5c3b89`; `real-resource-lock-pressure` `968d5e471bfe73c829ec5ac1a1c35c2f8e1a1f617310b26c0543fcd0755fb9c0` / `794472d65522cebf41bbc687d25dccb474e8579766fd01de4a45fda48941cc65`; `real-protected-break-recovery` `852d269d9f0c873423001cc173d6cf0c9918e6fd0f9e2b64761198badf9fc56b` / `fd6782b3ba5a6104c13e70baeff7303baf48b9f1561eea76e37e4a33f58eb01a`.
- **Benchmark, regresión y siguiente decisión:** dos ejecuciones de `SPEC10-005-engine-input-preflight`, sin runtime publicado, fueron idénticas byte a byte: 39.288 bytes y SHA-256 `30be7cf6e32d49193ed9f899f804863a92c78404a54f1e19998c5fc3d5a87d2a` cada una. Focal A2 exacto siguió `COMPLETE`, hard-valid, determinista, invariante e inmutable: 53 tareas, 0 pendientes, política explícita `EXACT_CONSTRUCTIVE`, configuración `LOW`, sin fallback, 300.000 ramas y fingerprint `b5b1fc1fe3b1813e425b26b22cbf7932604718f1b194eb00a8e909f0937f7357`. Calidad conservó `READY_FOR_HUMAN_REVIEW`, fingerprint `256244c1ccad494ca319d921dfcdc8c696b54a4b16506d42567f2e29abb5657b`, presencia/productivo/idle `3290/900/2390`, 78 planes completos, 2 reemplazos y dominancia sobre baseline y referencia humana. La siguiente decisión debe usar esta Evidence para definir el adaptador explícito, sin asumir que la ruta productiva ya recibe estos recursos ni que requisitos genéricos o disponibilidad temporal están resueltos.

### SPEC10-006 — Snapshot persistente de disponibilidad de recursos

- **Objetivo e implementación (`DB Safe Merge`):** la migración `072_resource_availability_snapshots.sql` incorpora el default global `default_availability_start/end` a cada `resource_items` y la copia diaria `availability_start/end` a cada `plan_resource_items`. La pareja `NULL/NULL` representa toda la jornada dinámica del plan; no almacena `00:00–23:59` ni materializa `plan.work_start/work_end`. Una ventana explícita exige dos valores canónicos `HH:mm`, con inicio estrictamente anterior al fin y sin cruce de medianoche.
- **Snapshot e independencia:** la creación normal y la inicialización explícita de inventario validan todos los defaults con el helper puro antes de una única inserción y copian exactamente la pareja global. Desde ese momento el snapshot no se sincroniza retroactivamente: editar el global no modifica planes existentes y editar el plan no modifica el catálogo. Los recursos ad hoc guardan `NULL/NULL` cuando no se envía ventana y aceptan sólo parejas explícitas completas y válidas.
- **Lectura, escritura e interruptor:** las lecturas de catálogo y plan exponen ambos extremos. Las rutas existentes de creación/edición escriben siempre la pareja completa de manera atómica y permiten restablecer `NULL/NULL`; una pareja parcial o inválida se rechaza antes de persistir. `is_available` continúa siendo un interruptor independiente: cambiarlo no borra ni reemplaza la ventana almacenada.
- **Contrato público y runtime:** `api.plans.resourceItems.list` declara y conserva como obligatorios los campos nullable `availabilityStart/availabilityEnd`; `create` y `update` incluyen la pareja opcional y la validan atómicamente —presencia conjunta, formato canónico y orden— en el esquema Zod compartido y estricto. Las dos rutas runtime consumen directamente esos inputs autoritativos, sin una segunda definición Zod local; el helper server-side sigue normalizando las filas que llegan a persistencia.
- **Persistencia y aislamiento:** los constraints estables de la migración 066 y sus equivalentes Drizzle comprueban pareja, formato/rango y orden. El backfill es implícito: las filas existentes quedan nulas, sin `UPDATE`. No se consulta, migra ni reutiliza la tabla legacy `resource_availability`; tampoco cambian asignaciones, tareas, locks o RLS.
- **Atomicidad de configuración general:** el endpoint valida toda la jerarquía propuesta, construye un único patch snake_case y realiza un solo `UPDATE ... RETURNING`; una validación fallida no escribe jornada, comida ni otros campos.
- **Tests y validación local:** 14/14 tests del helper cubren jornada completa, formato/rango/orden, cruces de medianoche, rechazo de valores no string, pureza, congelación, copia/desacoplamiento, construcción atómica, ad hoc e independencia de `is_available`; 11/11 tests del contrato cubren list/create/update y reutilización runtime. Pasaron `npm ci`, `npm run check`, `npm run build`, ambos comandos focales, `npm test` (1841/1841, 8 suites) y `git diff --check`. La migración se revisó estáticamente; no se aplicó contra una DB real porque no hay entorno configurado. La CI alojada debe distinguirse de estas comprobaciones locales y confirmarse en el PR; no se atribuye a ella la ejecución de `npm test`.
- **Límites y siguiente decisión:** esta unidad persiste una sola ventana continua y no añade UI, múltiples ventanas, proyección a `EngineInput`, adaptador, integración con Planner Next ni ejecución productiva del planificador; por ello `MISSING_RESOURCE_AVAILABILITY` no desaparece. La siguiente decisión debe basarse en Evidence separada para proyectar la intersección entre ventana y jornada sin reinterpretar este snapshot.

## SPEC10-007 — Proyección efectiva de disponibilidad de recursos

- **Objetivo y problema corregido:** el preflight deja de tratar todo el inventario diario como carente de disponibilidad y audita únicamente las identidades concretas realmente consumidas. La autoridad exclusiva es el snapshot de `plan_resource_items` proyectado a `EngineInput`; no se consulta el catálogo global ni `resource_availability`.
- **Contrato temporal:** `availabilityStart: null` junto con `availabilityEnd: null` significa exactamente la jornada vigente. Una pareja `HH:mm` explícita se intersecta con `workDay`; una intersección vacía produce `EMPTY_WORKDAY_INTERSECTION`. `isAvailable=false` produce `RESOURCE_DISABLED`. La omisión, pareja parcial, mezcla null/string, formato y orden inválidos permanecen diferenciados mediante `MISSING_SNAPSHOT_WINDOW`, `PARTIAL_SNAPSHOT_WINDOW`, `MIXED_NULL_AND_STRING`, `INVALID_TIME_FORMAT` e `INVALID_TIME_ORDER` (y una jornada inválida mediante `INVALID_WORKDAY`).
- **Recursos requeridos y locks:** se reutiliza la resolución efectiva de SPEC10-005 para unir asignaciones directas, del espacio exacto y de la zona efectiva de tareas `pending`, `interrupted`, `in_progress` y `done`, sin doble conteo. Las tareas `cancelled`, coaches meramente referenciados, inventario o requisitos genéricos no consumen instancias. Los locks `resource`/`full` positivos de tareas existentes no canceladas sí incorporan su identidad, sin sustituciones ni selección desde `byType`/`anyOf`.
- **Tareas protegidas:** `in_progress` y `done` conservan el intervalo real completo o, si éste no existe, el planificado completo; nunca se mezclan extremos. Una ventana efectiva válida que no contiene el intervalo genera `PROTECTED_TASK_CONSTRAINT_NOT_REPRESENTABLE` con intervalo, procedencia y orígenes de asignación, sin desplazar ni modificar la tarea.
- **Issues, canonicalización, fingerprints y diagnostics:** la ausencia, deshabilitación o intersección vacía de un recurso requerido genera una única `MISSING_RESOURCE_AVAILABILITY`; contratos temporales mal formados generan una única `UNSUPPORTED_TIME_VALUE`. Extremo omitido y propiedad propia `undefined` se canonicalizan ambos como ausencia, mientras `null/null` continúa siendo jornada completa. `sourceFingerprint` incorpora ID, `resourceItemId`, `typeId`, `isAvailable` y el valor canonicalizado de ambos extremos; `identityMapFingerprint` no cambia. Se publican `requiredPlanResourceCount`, `usableRequiredPlanResourceCount`, `unusableRequiredPlanResourceCount` y `protectedTaskResourceAvailabilityConflictCount`; `missingAvailabilityCounts.resources` representa sólo obligaciones sin ventana utilizable.
- **Evidence exacta de escenarios:** los tres escenarios permanecen honestamente `UNSUPPORTED`, eliminan `MISSING_RESOURCE_AVAILABILITY` y congelan estructuralmente todos sus reason codes y diagnostics. `real-main-stage-with-backlog` publica source `f9585b9e8ed4144eb1e9d29cbd881b5b8eaaf3fcbe1a16f3ed3c0708b5f510f3` e identities `68a72d0ac8f1d2246d5a7a8132c0090b43d76bb9a2dcb59f9f4b1cdb7b5c3b89`; `real-resource-lock-pressure`, source `9920e258a361c548514d5f97fad2c3e597494472e4543a80fa74fd188d3dabdf` e identities `794472d65522cebf41bbc687d25dccb474e8579766fd01de4a45fda48941cc65`; `real-protected-break-recovery`, source `41bc95e7cadf5e0851e78c8b74732d8f9d3209c36c352b5df4b973f98dcfc898` e identities `fd6782b3ba5a6104c13e70baeff7303baf48b9f1561eea76e37e4a33f58eb01a`. Sus recursos requeridos/utilizables son exactamente `1/1`, `2/2` y `2/2`, con cero no utilizables; repetición, inversión, input inmutable y output frozen quedan comprobados.
- **Validación local exacta:** pasaron `npm ci`, TypeScript check y production build; buildInput `7/7`, resolvedor `8/8`, preflight `194/194`, Planner Next `484/484` (1 suite), escenarios ORC `8/8` y global `1870/1870` (8 suites). Dos ejecuciones del benchmark `SPEC10-007-engine-input-resource-availability` fueron idénticas: `37.071` bytes y SHA-256 `511870cf7d3b5a59398b3018c9a7cca7d84145b7f3361ef45a79f4bf4bc2a7be` cada una. Focal A2 exacto permaneció `COMPLETE`, 53 tareas, 0 pendientes, hard-valid, determinista, invariante e input inmutable, política `EXACT_CONSTRUCTIVE`, configuración `LOW`, sin fallback, 300.000 ramas y plan fingerprint `b5b1fc1fe3b1813e425f26b22cbf7932604718f1b194eb00a8e909f0937f7357`. Calidad conservó fingerprint `256244c1ccad494ca319d921dfcdc8c696b54a4b16506d42567f2e29abb5657b`, presencia/productivo/idle `3290/900/2390`, 78 planes completos, 2 reemplazos y dominancia sobre baseline y referencia humana.

## SPEC10-008 — Snapshot temporal de jornada, platós y espacios

- **Objetivo y persistencia:** `program_settings` incorpora `default_work_start/default_work_end`; cada plan conserva su pareja independiente. `zones` y `spaces` incorporan defaults temporales, mientras `plan_zone_settings` y `plan_space_settings` conservan snapshots diarios. El `zone_id` autoritativo del espacio queda congelado en el snapshot; `zone` es el plató contenedor operativo de esta iteración.
- **Jerarquía completa:** la única semántica es jornada → zone/plató → espacio. `null/null` hereda del nivel superior sin materializar la ventana; una pareja explícita debe ser canónica, estrictamente ordenada y estar completamente contenida. Antes de actualizar la jornada general, una zone general o un espacio general, se construye y valida en memoria la configuración resultante completa pertinente y se identifican los IDs que impiden el cambio; no hay escritura, intersección, recorte ni modificación automática de descendientes antes de superar la validación.
- **Migración, auditoría y backfill:** el corte operativo confirmado es `070_engine_v4_parallel_results.sql`; la auditoría Git renumera los archivos post-corte como `071_id258_zone_meal_break_minutes.sql`, `072_resource_availability_snapshots.sql` y la migración idempotente de esta SPEC `073_workday_space_availability_snapshots.sql` añade defaults y constraints, crea ambas tablas con FKs, unicidad y cascade desde plan, y completa planes existentes con parejas nulas mediante `ON CONFLICT DO NOTHING`, sin `UPDATE` de snapshots. La migración activa RLS server-only en ambos snapshots, revoca tablas y secuencias a `anon`/`authenticated` y conserva `service_role`; la autorización de usuario ocurre en rutas antes de `supabaseAdmin`. La revisión SQL fue estática; no se aplicó sobre una DB Supabase real y el merge no la aplica automáticamente.
- **Creación, compensación e independencia:** la creación preconstruye y valida el lote completo antes de insertar el plan. Los fallos posteriores ejecutan borrado compensatorio por cascade y comprueban su resultado, publicando causa original, fallo de cleanup y plan potencialmente huérfano si la compensación falla. No se afirma una transacción SQL multi-tabla. Cambios globales y diarios permanecen independientes.
- **Inicialización tardía:** el candidato final combina jornada del plan, snapshots diarios existentes como autoridad y defaults globales sólo para filas ausentes. Los espacios nuevos se validan contra la zone diaria existente o la candidata nueva de esa ejecución; nunca se sobrescriben ventanas, `source` ni `zone_id` existentes. Primero se insertan zones y, si falla el lote de spaces, se eliminan exclusivamente las zones creadas por esa ejecución y se comprueba la compensación.
- **Semántica HTTP y API contractual:** IDs o bodies inválidos devuelven `400`, entidades inexistentes o no accesibles devuelven `404`, conflictos espaciales tipados devuelven `400`, entidades inexistentes `404` y fallos inesperados sanitizados devuelven `500`, y una lista válida vacía permanece `200 []`. Los namespaces `api.zones.availability` y `api.spaces.availability` tienen paths no intercambiados. Listados, inicialización y updates globales/diarios declaran respuestas Zod compartidas completas; runtime transforma filas Supabase snake_case a camelCase y valida la respuesta compartida, conservando IDs, parejas nullable, `source`, `zoneId` y timestamps.
- **Atomicidad de configuración general:** el endpoint valida toda la jerarquía propuesta, construye un único patch snake_case y realiza un solo `UPDATE ... RETURNING`; una validación fallida no escribe jornada, comida ni otros campos.
- **Tests y validación local:** el checker pasa `8/8`, el helper temporal/catálogo/inicialización `30/30`, los contratos compartidos `7/7` y la semántica HTTP `6/6`, program settings `4/4` y RLS/SQL `2/2`. TypeScript y production build pasan; `npm test` pasa `1912/1912` en 8 suites; `git diff --check`, la comparación byte a byte, los SHA-256 y la validación SQL estática pasan. La CI alojada cubre instalación, checker, TypeScript y build —no se le atribuye `npm test`— y debe concluir `success` sobre el head final antes del merge.
- **Protección y despliegue:** `npm run check:migrations` valida el baseline histórico, rechaza colisiones y exige continuidad desde `071`; Baseline CI ejecuta este check además de instalación, TypeScript y build. Sobre un entorno cuyo último estado confirmado sea `070`, deben aplicarse en orden `071_id258_zone_meal_break_minutes.sql`, `072_resource_availability_snapshots.sql` y `073_workday_space_availability_snapshots.sql`; el merge no aplica Supabase automáticamente.
- **Límites:** sólo se snapshotan relaciones y ventanas temporales, no nombres, colores, prioridades, grouping ni setups. No se añade botón/endpoint “Copiar configuración predeterminada”, diff, actualización masiva, UI, `EngineInput`, Planner Next, ORC o V3/V4; no se afirma que desaparezca `MISSING_SPACE_REFERENCE`. SPEC10-008 sí añadió RLS server-only a `plan_zone_settings` y `plan_space_settings`.

## SPEC10-009 — Proyección efectiva de disponibilidad espacial

- **Objetivo y autoridad (`DB Safe Merge`):** `buildEngineInput` proyecta literalmente `plan_zone_settings` y `plan_space_settings`, ordenados por identidad operativa. La configuración efectiva diaria es la única autoridad temporal: la jornada procede de `plan.workStart/workEnd` y el `zoneId` del snapshot diario del espacio prevalece sobre la relación mutable del catálogo; no se consultan defaults globales ni otro día para reinterpretar el plan.
- **Fallo de carga explícito:** las operaciones storage declaran `Promise<any[]>`; la proyección acepta defensivamente `null`/`undefined` en fixtures o implementaciones incompletas, pero una excepción de storage/RLS durante la carga diaria sigue abortando `buildEngineInput`. No se usa el helper de fallback porque convertir un fallo de autoridad en snapshots vacíos ocultaría un error de persistencia como ausencia de dominio.
- **Jerarquía compartida y resolución:** la semántica pura canónica vive en `shared/spatialAvailabilityHierarchy.ts` y es consumida tanto por persistencia como por preflight. `null/null` hereda jornada → zone → espacio; parejas parciales, mezclas null/string, formato u orden inválidos y ventanas fuera del padre generan `UNSUPPORTED_TIME_VALUE`, sin recorte, ampliación o reparación silenciosa. Una zone diaria ausente deja la disponibilidad espacial explícitamente no resoluble.
- **Obligaciones, issues y protección:** sólo consumen espacio las referencias concretas de tareas no canceladas y los scopes/configuraciones espaciales ya explícitos; inventario sin uso y tareas canceladas no bloquean ni duplican obligaciones. Una identidad ausente produce una sola `MISSING_SPACE_REFERENCE`; una identidad existente cuya jerarquía no puede resolverse produce `MISSING_SPACE_AVAILABILITY`. Las contradicciones tarea-zone frente a la relación diaria reutilizan `INCONSISTENT_SPACE_ZONE_REFERENCE`. Tareas `done`/`in_progress` conservan el intervalo real completo o, en su defecto, el planificado completo; si queda fuera se explica mediante `PROTECTED_TASK_CONSTRAINT_NOT_REPRESENTABLE` sin modificar tarea, lock o ventana.
- **Fingerprints y diagnostics:** `sourceFingerprint` distingue propiedad ausente, `undefined`, `null` y string e incorpora identidad, relación diaria, extremos y source de zones/spaces; `identityMapFingerprint` conserva su contrato. Diagnostics publican espacios requeridos/utilizables/no utilizables, zones necesarias, conflictos protegidos y `missingAvailabilityCounts.spaces` desde la misma resolución que crea issues.
- **Corrección de inmutabilidad e identidad:** la revisión final detectó que `Object.freeze(Map)` no bloquea `set`, `delete` ni `clear`. Los mapas publicados se sustituyen por vistas cerradas con la API completa de lectura y sin mutadores ni acceso al `Map` privado; tests runtime comprueban intentos de mutación y contenido estable. IDs diarios duplicados de zone o space ya no aplican `last write wins`: la identidad queda inutilizable y produce un `DUPLICATE_ID` bloqueante con causa `DUPLICATE_ZONE_SNAPSHOT` o `DUPLICATE_SPACE_SNAPSHOT`, normalizada e invariante al orden.
- **Autoridad espacial canónica en Planner Next:** la integración resuelve `task.spaceId → planSpaceSettings.zoneId` desde el snapshot diario y usa exclusivamente esa zone para `zoneResourceAssignments`. `zoneIdBySpaceId` y `spaceIdsByZoneId` continúan en `EngineInput` como contratos legacy para V3/V4, pero ya no participan en issues, identidades, fingerprints ni recursos efectivos de Planner Next. Una contradicción conserva la tarea y genera una sola `INCONSISTENT_SPACE_ZONE_REFERENCE`; recursos directos y del espacio permanecen, sin elegir recursos de ninguna zone.
- **Protección temporal y locks:** un único resolvedor clasifica intervalos protegidos como `COMPLETE_REAL`, `PARTIAL_REAL`, `COMPLETE_PLANNED` o `MISSING`; cualquier endpoint real parcial impide caer al intervalo planificado en las auditorías general, espacial y de recursos. Locks `time`/`full` válidos, incluso sobre tareas pending/interrupted, se contrastan con la ventana espacial diaria y publican un único `UNREPRESENTABLE_TIME_LOCK` explicable si quedan fuera o la jerarquía no es resoluble.
- **Representabilidad e inmutabilidad final:** toda tarea activa (`pending`, `interrupted`, `in_progress` o `done`) sin `spaceId` entero positivo genera exactamente un `MISSING_SPACE_REFERENCE`, sin identidad ficticia, obligación espacial inventada ni inferencia; `cancelled` permanece excluida. La relación diaria usada por `buildEngineInput` comparte la misma vista cerrada runtime que los mapas efectivos: no expone `set`, `delete`, `clear` ni backing map, conserva lecturas/orden y excluye duplicados sin mutar el input.
- **Evidence representativa:** los tres escenarios continúan `UNSUPPORTED` por blockers legítimos y eliminan `MISSING_SPACE_AVAILABILITY`, con `missingAvailabilityCounts.spaces = 0`. Todas las referencias espaciales concretas quedan resueltas (`1/1`, `3/3`, `2/2`); `real-main-stage-with-backlog` y `real-protected-break-recovery` conservan ahora un `MISSING_SPACE_REFERENCE` legítimo por su tarea activa sin `spaceId`, mientras `real-resource-lock-pressure` no lo publica. Sources: `ee7f89bdabfaa57bec7720013da926abb5beeb9e2504353c48a3fca4d42f79a8`, `208712de676bfdd5d6edac56d9f50cc30b5dd076f1db13c9b8435649888b4614` y `d157c50703c595ba04dd4a80972a6b42cca90618bbf78b77095172a0464e1bb9`; identity fingerprints permanecen estables, y repetición, inversión, input inmutable y output frozen quedan congelados por tests y Evidence.
- **Evidence regenerada:** dos ejecuciones de `SPEC10-009-engine-input-spatial-availability` fueron idénticas byte a byte, con `35.338` bytes y SHA-256 `7b61f4715ba62e960bb443ece24c9626093d6b8e77168d8f4560e3743691971a`; los tests focales adicionales congelan autoridad diaria frente al catálogo legacy, issue única, partial-real y locks temporales.
- **Focal A2 vigente:** `validate-focal-a2-009r3.sh` queda como Evidence histórica V3 superseded y no es la puerta actual. `./validate-focal-a2-010.sh current` terminó con exit code `0`, status `FOCAL_A2_SPEC08_MAIN_ANCHORED_ACCOMPANIMENT_ACCEPTED`, 33 escenarios y aceptación `true`; el escenario activo fue completo y hard-valid, con 53 tareas, 0 pendientes, 3 `AnchoredAccompaniment`, 6 segmentos, 28.432 ramas de un máximo de 300.000, sin human schedule seed, determinista, invariante, input inmutable, sin PartialPlan ni fallback. La suite Planner Next posterior a publicación pasó `497/497`; el artefacto publicado por el validator tuvo SHA-256 `48093adeaec158e4b26e6e5493b52dafdc5b414dcd61a2e8ce2c9bbffd77a31f` (la única diferencia respecto del artefacto versionado era `runtimeMs`, que no se incorpora al PR).
- **Límites y siguiente blocker:** no hay migración, SQL remoto, UI, cambios de API, creación de snapshots, adaptación `EngineInput → PlannerNextProblem`, ejecución productiva, publicación, Planner Next search, ORC, V3 o V4. Permanecen configuración de integración, roles/duraciones, disponibilidad de participantes y demás blockers publicados; el siguiente corte debe abordar el adaptador explícito sólo después de Evidence separada, sin ejecutar aún desde planes productivos.

## SPEC10-010 — Adaptador read-only de EngineInput a PlannerNextProblem

- **Objetivo y clasificación (`DB Safe Merge`):** se publica `adaptEngineInputToPlannerNextProblem(input)` como una unión discriminada `SUPPORTED | UNSUPPORTED`. La unidad termina al construir y validar estructuralmente el problema; no importa ni llama `executePlannerNext`, no conecta `buildEngineInput`, no publica planes y no escribe en DB.
- **Doble frontera sin parciales:** cada llamada ejecuta exactamente una vez el preflight de `EngineInput`; tras construir el problema ejecuta exactamente una vez el preflight canónico de Planner Next. Sólo publica `SUPPORTED` si ambos aceptan y el segundo devuelve `[]`. Una divergencia retorna `problem: null`, `problemFingerprint: null` y la issue determinista `ADAPTED_PROBLEM_NOT_REPRESENTABLE`, con los reason codes del dominio en details. El fixture explícito con dos pares main/vocal y una técnica es aceptado por ambas fronteras; los tres escenarios reales conservan sus rechazos originales.
- **Identidad, tiempo y grid:** toda referencia usa exclusivamente el identity map reversible (`sourceId → namespace:sourceId → sourceId`). Los helpers estrictos convierten `HH:mm` a minutos desde `00:00` y viceversa sin pérdida. La capacidad vigente de Planner Next es una rejilla fija de cinco minutos: sólo `timeGridMinutes === 5`, con tiempos, ventanas y duraciones alineados, puede ser `SUPPORTED`; grids 10/15 u otros valores se rechazan con requested/supported grid explícitos. Una rejilla configurable exige ampliar el contrato y todos los generadores en otra iteración.
- **Coaches compartidos, feeders y proyección hard única:** preflight y adaptador reutilizan un único resolvedor read-only que une asignaciones efectivas y todos los resource locks antes de separar canales. Una relación de coach sólo se materializa cuando el plan-resource relacionado está en la asignación efectiva de la main/vocal. Varios participantes pueden compartir legítimamente la misma identidad: se publica una sola entrada en `coaches`, las cuatro tareas usan el mismo `coachId`, las mains conservan `blockKey === coachId` y `coaches ∩ resources = ∅`. Un lock sobre el coach propio es redundante y queda auditado por ese mismo canal, sin duplicarlo; un lock/asignación sobre coach ajeno desde main, vocal, auxiliary o technical bloquea con `UNSUPPORTED_COACH_RESOURCE_MAPPING`. Los demás locks se deduplican y permanecen como recursos genéricos exactos. Cada participante con main o vocal exige exactamente un par y una dependencia main→vocal exclusiva.
- **Tareas y obligación temporal única:** `pending` e `interrupted` descartan horarios anteriores como seed; `done` e `in_progress` derivan duración y posición del intervalo protegido canónico; `cancelled` desaparece. El resolvedor puro reúne intervalo protegido, todos los time/full locks y fixed window: fuentes idénticas se deduplican, fuentes contradictorias o parciales bloquean y una pending/interrupted sólo acepta intervalo cuya duración sea exactamente la de la tarea. Resource locks se unen y deduplican; no existe last-write-wins.
- **Anchors tipados, continuidad y robustez:** `EngineInput.anchoredAccompaniments` usa el contrato oficial sin aliases. El preflight calcula continuidad sobre los recursos genéricos que realmente se publicarán: asignación efectiva más locks representables menos el canal de coach. Acepta asignación en todas las fases, lock en todas o una mezcla, sólo cuando su intersección es no vacía; conserva los recursos adicionales propios de cada fase y nunca cuenta el coach como continuidad genérica. No exige igualdad total ni acepta conjuntos vacíos. Entradas runtime null, primitivas, arrays ausentes/no-array o IDs inválidos retornan `UNSUPPORTED` sin lanzar. El array exterior se canonicaliza por ID, pero before/after preservan orden y cambian el fingerprint al invertirse.
- **Comidas y límites honestos:** sólo se materializa la comida global exacta aceptada; ventanas flexibles y scopes de participante/space/zone/unidad siguen `UNSUPPORTED`. Continúan sin soporte `byType`, cantidades no representables, `anyOf`, cámaras genéricas, capacidades no exclusivas, setup sin mapping reversible, transporte, locks de espacio y contratos incompletos. No se infieren coaches, feeders ni operaciones desde nombres, IDs conocidos o Focal A2.
- **Determinismo, inmutabilidad y Evidence:** envelope, problema y descendientes quedan profundamente frozen. La serialización ordena sólo colecciones set-like y conserva secuencias operativas. El artefacto versionado es JSON puro, sin logs, timestamps ni runtime. Dos generaciones directas fueron idénticas: 6.014 bytes y SHA-256 `515f9ede536bc7fd9a15dcb16ecbd9dc72ff0ca88943eb0ca84aff688ebec379`; publica grid solicitado/soportado, un coach compartido, ambos preflights, round-trips, contadores, inmutabilidad e invariancia.
- **Validación correctiva:** pasan adaptador `25/25`, preflight afectado `199/199`, ejecución combinada `224/224`, Planner Next `522/522` y suite global `1.953/1.953`; TypeScript y migraciones pasan, junto con build y `git diff --check`. `./validate-focal-a2-010.sh current` conserva exit code 0, status aceptado, 33 escenarios, 53 tareas, 0 pendientes, hard validity, 3 operaciones/6 segmentos, máximo 300.000, sin human seed, PartialPlan ni fallback y determinismo; no se modificó su validator, benchmark ni presupuesto.
- **Alcance y siguiente blocker:** no existe integración productiva ni ejecución de Planner Next desde `EngineInput`; tampoco output adapter, shadow mode, UI, API, DB, migración, RLS, ORC, V3/V4 o publicación. El siguiente blocker real es definir y validar por separado la ruta de ejecución/simulación y su adaptador de salida, sin convertir esta construcción read-only en consolidación.

## SPEC10-011 — Auditoría de disponibilidad hard en tareas protegidas

- **Objetivo y defecto corregido (`DB Safe Merge`):** la auditoría temporal de tareas `done`/`in_progress` ya no recorre sólo `assignment.effectiveResourceIds`. Consume el mismo `projectedResourcesByTaskId` que usa el adaptador y, por tanto, contrasta exactamente el coach y todos los recursos generic que se publicarían, incluidos los introducidos únicamente por resource locks.
- **Autoridad, canales y deduplicación:** cada pareja tarea protegida + plan-resource se audita una vez, defensivamente incluso ante solapamiento de canales. Un recurso assignment + lock, dos locks del mismo recurso o un lock redundante sobre el coach no duplican issue ni contador. Diagnostics preservan canal `coach`/`generic`, fuentes `direct`/`space`/`zone` en orden estable y todos los lock IDs aplicables, únicos y ordenados; locks de canceladas y recursos de otras tareas permanecen excluidos.
- **Resultado operativo:** una ventana `AVAILABLE` debe contener el intervalo protegido completo. El conflicto genera `PROTECTED_TASK_CONSTRAINT_NOT_REPRESENTABLE` antes del adaptador, incrementa `protectedTaskResourceAvailabilityConflictCount` por pareja única y conserva `problem: null`; los recursos sin disponibilidad utilizable mantienen las issues generales existentes sin duplicación ni ventana fabricada. Pending/interrupted no se convierten en protected y no se ejecuta búsqueda.
- **Tests y Evidence:** los tests focales cubren `done` e `in_progress`, locks compatibles e incompatibles, igualdad exacta, deduplicación de assignment/locks, coach redundante, diagnostics ordenados, inversión, canceladas e inmutabilidad. La Evidence pura `SPEC10-011-protected-task-resource-availability` contiene seis escenarios, fue generada dos veces con igualdad byte a byte y ocupa exactamente 3.652 bytes, SHA-256 `1fe7a9992c1181bcc5e9c1a56150e34da5f47e40e619a32587d891994b035383`, sin timestamps, runtime, logs ni paths locales.
- **Validación y Focal A2:** las validaciones focales de adaptador/preflight, suite Planner Next, suite global, TypeScript, migraciones, build, diff y el validator Focal A2 se ejecutan como puertas de esta iteración. Focal conserva el contrato aceptado de 33 escenarios, 53 tareas, 0 pendientes, hard-valid, 3 operaciones ancladas, 6 segmentos, máximo 300.000 ramas, determinismo y ausencia de human seed, PartialPlan y fallback.
- **Límites:** no se modifica la semántica de proyección, contratos, búsqueda, placement, ORC, adaptador de salida, publicación, consolidación, shadow mode, V3/V4, UI, API, DB, schema, migraciones o RLS. `EngineInput` continúa sin conectarse a ejecución productiva y esta iteración no llama ni ejecuta Planner Next.

## SPEC10-012R — Auditoría A2 reconstruida desde Evidence ejecutable

- **Objetivo y clasificación (`DB Safe Merge`):** sustituir la clasificación no plenamente derivada de Evidence de #615 por un catálogo tooling-only de 167 IDs, un manifest de requisito literal por fila, registros estructurados por capa, probes honestos y un clasificador que diferencia falta de auditoría, ambigüedad de fuente, gap contractual, rechazo explícito y soporte evidenciado.
- **Implementación y alcance:** la auditoría separa requerimiento, soporte por capa y conclusión. El probe end-to-end ejecuta preflight, adaptador, dispatcher oficial y validación; tres probes internos declaran `NOT_EXECUTED` fuera de su capa. No se exporta desde producción ni cambia contratos, motor, búsqueda, validación, ORC, producto, API, UI o DB.
- **Evidence y resultado operativo:** `npm run benchmark:planner-next:a2-coverage-audit` genera JSON determinista y el documento humano directamente desde el mismo objeto. La cobertura de planning y la preparación de producto se calculan por separado; la frontera de elegibilidad queda `SOURCE_AMBIGUOUS`, por lo que el gating devuelve `CLARIFY_DOMAIN` en vez de recomendar una implementación prematura.
- **Validaciones:** tests sintéticos cubren todas las ramas del clasificador, referencias reales, escenarios de benchmark, correcciones conocidas, familias Reality por composición exacta, total vocal 19, inmutabilidad, determinismo y aislamiento de producción; se ejecutan además las puertas focales, Planner Next, suite global, TypeScript, migraciones, build y diff.

## SPEC10-012R2 — Piloto de auditoría A2 con Evidence real

- **Objetivo (`DB Safe Merge`):** corregir el andamiaje provisional de PR #616 y la auditoría masiva inicial de este PR mediante un piloto pequeño que demuestre el mecanismo antes de ampliar cobertura.
- **Piloto explícito:** sólo se auditan 12, 13, 14, 16, 18, 19, 20, 41, 120, 121, 122, 123, 134, 135 y 136. Los 15 bindings son literales y consumen 26 source assertions con epígrafes oficiales exactos, observaciones de probes ejecutados, referencias a definiciones exactas de tests y selectors de benchmark sobre valores reales. La existencia del nombre del test no se presenta como ejecución; los probes y benchmarks son la Evidence ejecutable.
- **Probes y derivación:** se ejecutan estados protegidos, variantes de locks, disponibilidad de coach, tarea y cadena técnica, dependencia técnica desde EngineInput, distinción transporte/tarea ordinaria y tres comidas por ámbito. Las comidas leen ventana, scope, identidad y entity ID del input/issue realmente ejecutado; la cadena técnica conserva frontera `PLANNER_LAYER` y el resto del piloto `ENGINE_INPUT`. Source refs y observation IDs inexistentes degradan a `NOT_AUDITED`; no existe clasificación por IDs o reason codes constantes.
- **Alcance y exclusiones:** las demás capacidades quedan sin binding y `NOT_AUDITED / AUDIT`; 162–167 permanecen `PRODUCT_PHASE_NOT_IMPLEMENTED / PRODUCT`. Vocal, main, Reality, joint tasks, espacios y validación completa quedan para iteraciones incrementales posteriores. No se implementa ninguna capacidad ni se modifica producción.
- **Focal y trazabilidad:** la auditoría lee el artefacto Focal aceptado en lugar de copiar contadores o flags. Evidence y documento humano se regeneran desde el mismo objeto determinista; las validaciones y los conteos finales se registran en el PR #617 sin atribuir tests locales a Baseline CI.

## SPEC10-013 — Comida fija asignada a participante

- **Objetivo y clasificación (`DB Safe Merge`):** Planner Next acepta una comida fija `kind: "meal"` con ID estable y un único `contestantId` desde `actualMeal` o `protectedBreaks`, sin cambios de DB, UI, API ni publicación de producto.
- **Representación hard:** la utilidad canónica read-only ordena y valida las comidas y resta sus intervalos de `Person.availability`. Por ejemplo, `09:00–18:00` con comida `14:30–15:00` se adapta exactamente a `09:00–14:30` y `15:00–18:00`; no se crea una tarea ni una preferencia soft, y la comida global conserva `problem.protectedMeal`.
- **Identidad y fallos explícitos:** el break conserva mapping reversible en namespace `break`. Se reutilizan `MISSING_PARTICIPANT_REFERENCE`, `UNSUPPORTED_TIME_VALUE`, `DUPLICATE_ID`, `UNSUPPORTED_BREAK_SCOPE` y `PROTECTED_TASK_CONSTRAINT_NOT_REPRESENTABLE`; `UNREPRESENTABLE_PARTICIPANT_BREAK` explica solapes, salida de disponibilidad o consumo total. Las comidas por recurso, espacio, zona, unidad itinerante y las pausas flexibles continúan fuera de alcance.
- **Protección, determinismo y auditoría:** una comida que solapa una tarea `done` o `in_progress` se rechaza sin moverla. La proyección es inmutable, estable ante reordenación y sensible a cambios temporales; los diagnostics separan comidas de participante soportadas y no soportadas. Los tests focales cubren resolución, split central, aislamiento, identidad, fingerprints y conflictos protegidos.
- **Límites y siguiente blocker:** esta iteración no publica un bloque visual de comida ni selecciona horarios automáticamente. El siguiente ámbito pendiente de la familia scoped-meals es la comida por recurso, que debe abordarse mediante Evidence separada.

## SPEC10-013R — Planificación flexible de tareas de comida de concursante

- **Corrección y separación semántica (`DB Safe Merge`):** las pausas de comida ya asignadas permanecen como interrupciones fijas de disponibilidad en `assignedParticipantMealBreaks`; una tarea `pending` o `interrupted` nunca usa ese splitter. Las tareas flexibles se reconocen sólo por `operationalRole === "meal_break_placeholder"`, `breakKind` normalizado o igualdad exacta con `mealTaskTemplateId`; jamás por `templateName` ni por texto operativo.
- **Contrato y autoridades:** cada tarea fuente se adapta una vez a `ParticipantMealObligation`, conservando `sourceTaskId`, participante y estado, sin `spaceId`, recursos, coach, setup ni continuidad. La duración usa primero `durationOverrideMin` y después `contestantMealDurationMinutes`. La ventana es la intersección exacta de jornada, disponibilidad del participante, `mealWindow`/aliases y ventana explícita de tarea. `done` e `in_progress` requieren y conservan un `fixedInterval`; canceladas se excluyen.
- **Búsqueda hard y capacidad:** `participantMealCapacity.maxSimultaneous` es acumulativa y directa, sin espacios o recursos ficticios. El witness DFS exacto recalcula candidatos sobre tareas/comidas colocadas, selecciona primero la obligación con menos candidatos, usa identidad sólo como desempate, respeta el presupuesto de ramas y retrocede ante inviabilidad conjunta. La validación de hojas estructurales exige un witness completo, por lo que una rama que destruye las comidas se rechaza antes de aceptar el plan; el resultado publica `ScheduledParticipantMeal` reversible.
- **Validación y políticas:** `validatePlan` comprueba presencia exacta, identidad, participante, duración, ventana, disponibilidad, solape con tareas, intervalo protegido y capacidad. `COMPATIBILITY_PRESERVING` y `EXACT_CONSTRUCTIVE` producen resultados completos y hard-valid en el escenario SPEC10-013R, sin PartialPlan ni fallback.
- **Evidence y auditoría:** la Evidence canónica SPEC10-013R es determinista, ocupa 5.465 bytes y tiene SHA-256 `128b7ed115b4f95b25e036dcc1d4a96d39c5be731db3f172ba0a507ae5cb062d`, sin runtime, timestamp, logs o paths locales. El piloto conserva sus IDs: 134 deriva a `EVIDENCED_SUPPORTED`, 135 y 136 continúan `EXPLICITLY_UNSUPPORTED`, `scoped-meals` deriva a `PARTIALLY_SUPPORTED` y el selector genérico recomienda 135.
- **Límites:** no se modifica DB, schema, migraciones, RLS, UI, API, consolidación ni publicación de producto. La comida por recurso y por unidad itinerante permanece fuera de alcance; las capacidades de producto 162–167 no se declaran implementadas.

## SPEC10-013R2 — Integración constructiva de comidas flexibles de participante

- **Corrección crítica (`DB Safe Merge`):** el witness de comidas deja de ser una búsqueda nueva posterior al plan productivo. La misma autoridad exacta `assessParticipantMealFutureFeasibility` opera en `PROBE` y `MATERIALIZE`, recibe el presupuesto restante del ledger invocante y nunca lee un presupuesto independiente. `executePlannerNext` sólo normaliza el resultado ya elegido y rechaza defensivamente una hoja que haya omitido el witness.
- **Búsqueda y Future Feasibility:** Compatibility incorpora `participant-meals` como work item conjunto después de cada colocación residual y materializa el witness antes de aceptar la hoja. Exact usa el mismo assessor en candidatos parciales de core, decisiones standalone y hojas completas, consumiendo el ledger exacto. Una candidata productiva a las 13:00 que destruye el único hueco de comida se poda; la alternativa de las 16:00 se explora y acepta bajo ambas políticas.
- **Presupuesto, trazas y atomicidad:** las ramas de comidas forman parte de `branchesExplored`; nunca se consume `max + 1`. Evidence separa `finalSelectionOrder`, `attemptedSelectionTrace`, candidatos rechazados, blockers, backtracks y fingerprint del witness aceptado. Una inviabilidad o agotamiento publica arrays vacíos de tareas, setups, comidas de espacio y comidas de participante, sin PartialPlan, seed o fallback.
- **Validation y contratos:** Validation no importa ni ejecuta el scheduler. `scheduledParticipantMeals` tiene default `[]`, las obligaciones esperadas proceden siempre del problema y una comida ausente invalida el resultado. El preflight directo exige estado/fixedInterval coherentes, duración y ventanas sobre la rejilla canónica, identidades únicas, participante y capacidad. La semántica flexible exige `mealMode=flexible_meal_window`; `breakKind="meal"` genérico no clasifica.
- **Identidad protegida:** una tarea protegida y un assigned break sólo se deduplican mediante relación explícita `task.breakId ↔ break.id`; sin relación se produce `PARTICIPANT_MEAL_IDENTITY_CONFLICT`. Nunca se deduce correspondencia por participante, horario o nombre, y la obligación protegida consume capacidad una sola vez.
- **Evidence:** el benchmark canónico ejecuta capacidad conjunta, retroceso estructural e inviabilidad real con ambas políticas. Dos generaciones completas fueron idénticas: 6.785 bytes, SHA-256 `9a8f1dd68e08065e26e9a78b70f5141d6b3f9f08ff332d15e5299a9465e96864`, sin runtime, timestamp, logs o paths locales. La auditoría regenerada ocupa 352.850 bytes, SHA-256 `4fcde4581801cd7380cabc637ba624c7d994459e91ee31b926550af9e7e83d43`; 134 deriva a `EVIDENCED_SUPPORTED`, 135/136 permanecen `EXPLICITLY_UNSUPPORTED`, la familia deriva a `PARTIALLY_SUPPORTED` y el selector genérico recomienda 135.

### SPEC10-014 — comidas fijas por recurso

- Las tareas explícitas `resource_meal` se normalizan como intervalos fijos, se excluyen del trabajo productivo y dividen sólo las ventanas de los recursos asignados. Una ventana fuente 09:00–17:00 con comida 12:00–13:00 se adapta a 09:00–12:00 y 13:00–17:00.
...
## SPEC11-002 — Snapshot efectivo de plantillas por plan

- **Objetivo (`DB Safe Merge`):** cada plan conserva un catálogo operativo propio de plantillas, versionado por `(plan_id, source_template_id)`. Los cambios posteriores de `task_templates` dejan de reinterpretar duración, cámaras, ubicación, dependencias, requisitos de recursos, equipos itinerantes, flags de autocreación y setup de un día existente.
- **Persistencia y compatibilidad:** la migración `074_plan_task_template_snapshots.sql` crea una tabla server-only con RLS, privilegios exclusivos de `service_role`, unicidad por plan/plantilla y sin FK destructiva hacia el catálogo global. El backfill idempotente congela `plans × task_templates` con `source=legacy_backfill`; los planes nuevos usan `inherited` y una plantilla añadida posteriormente sólo puede entrar mediante una acción explícita `ad_hoc_from_default`.
- **Contrato y fingerprint:** `server/taskTemplateSnapshot.ts` es la autoridad única para camel/snake, dependencias legacy y múltiples, requisitos de recursos, equipos itinerantes y arrays canónicos. El fingerprint SHA-256 se deriva de todos los campos normalizados; no se persiste un hash redundante ni se usan algoritmos distintos entre SQL y TypeScript.
- **Creación y atomicidad:** `createPlan` valida el catálogo completo antes de insertar y persiste el lote de snapshots antes del resto de inicializaciones; cualquier fallo posterior elimina el plan y comprueba la compensación. `createDailyTask` hereda ubicación desde el snapshot diario y sólo consulta `task_templates` para colores visuales, fuera del contrato operativo. Los bloqueos manuales, que usan una inserción directa, extienden primero el catálogo diario mediante una inicialización ad hoc explícita. La autocreación de concursantes usa exclusivamente el catálogo diario y compensa concursante y tareas si esa inicialización falla.
- **EngineInput:** la ruta productiva carga las filas de `daily_tasks` sin `JOIN task_templates`, trata el catálogo diario como entrada hard, rechaza versiones, duplicados o snapshots ausentes y aplica después los overrides de instancia. Publica versión, número de filas, fuentes y fingerprint del catálogo; no llama `getTaskTemplates()` durante la planificación.
- **Validación local ejecutada:** 291/291 tests focales y de regresión pasaron: helper/migración/integración SPEC11-002, proyección real de `buildEngineInput`, proyecciones históricas de disponibilidad/espacios/coaches, 204 tests de preflight y 38 tests del adapter Planner Next. `check:migrations` confirma 074 como siguiente migración y `git diff --check` pasa.
- **Limitaciones de validación:** no se aplicó la migración contra una Supabase real. La instalación npm del sandbox quedó incompleta por un artefacto ausente en el mirror, por lo que `npm run check`, `npm test` y `npm run build` completos deberán ejecutarse en un entorno con las dependencias del lock disponibles antes del merge.
