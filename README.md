...
## SPEC10-012R2 — Bindings A2 derivados de Evidence real

- **Objetivo (`DB Safe Merge`):** corregir el andamiaje provisional fusionado en PR #616; sus conteos, estados familiares y recomendación anteriores no eran autoridad.
- **Implementación:** las 167 decisiones de fuente son explícitas; 100 bindings contienen únicamente referencias evaluables y los evaluadores publican `PASS`, `FAIL`, `NOT_FOUND` o `NOT_EXECUTED`. La clasificación, las nueve familias y `selectNextAction` se derivan de resultados observados. Los probes ejecutan la cadena EngineInput completa y las autoridades reales de joint tasks y technical chains.
- **Alcance y exclusiones:** tooling, Evidence y documentación solamente. No se implementan capacidades ni se modifican contratos, motor, ORC, UI, API, DB, schema, migraciones o RLS. Las capacidades 162–167 quedan separadas como fases de producto y no afectan a `fullA2PlanningCoverage`.
- **Resultado operativo:** la auditoría conservadora publica 99 capacidades auditadas, 62 `NOT_AUDITED`, 96 `SOURCE_AMBIGUOUS`, tres `EXPLICITLY_UNSUPPORTED` por `UNSUPPORTED_BREAK_SCOPE` y seis `PRODUCT_PHASE_NOT_IMPLEMENTED`. La recomendación derivada es `CLARIFY_DOMAIN` para la frontera de elegibilidad 141. Focal permanece aceptado con 33 escenarios, 53 tareas, 0 pendientes, hard-valid y determinista, sin seed humana, PartialPlan ni fallback y con presupuesto máximo de 300.000 ramas.
- **Validación:** Evidence generada dos veces con igualdad byte a byte, JSON válido y ausencia de runtime, timestamp, logs y paths locales; el resto de comandos obligatorios se registra en el PR y no se atribuye a Baseline CI hasta que el head final sea ejecutado por CI alojada.
