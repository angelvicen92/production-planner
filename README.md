...
- **Objetivo y alcance (`DB Safe Merge`):** se incorpora un preflight puro, determinista y estrictamente read-only entre `EngineInput` y la futura integración de Planner Next. El módulo materializa identidades canónicas separadas sólo a partir de IDs explícitos y audita tiempo, estados protegidos, dependencias, locks, participantes, espacios, zonas, recursos, breaks, transporte, setups, flujo principal, política y presupuesto. No crea un `PlannerNextProblem`, no planifica, no ejecuta motor ni modifica la entrada.
- **Evidence representativa:** los tres escenarios reales resultan honestamente `UNSUPPORTED`. Comparten ausencia de flujo principal, disponibilidad temporal de participantes/recursos/espacios, política, presupuesto, rol explícito y rejilla autoritativa; además `real-main-stage-with-backlog` y `real-protected-break-recovery` carecen de referencias de espacio y duraciones en parte de sus tareas, mientras `real-resource-lock-pressure` no puede representar exactamente su lock full de espacio. Los fingerprints de fuente/mapa son respectivamente `158769f…/68a72d0…`, `db7c3daa…/794472d6…` y `ff54da74…/fd6782b3…`.
- **Garantías:** el mapa reversible cubre los namespaces plan, task, lock, participant, space, zone, plan-resource, resource-item, resource-type, template, itinerant-team y break sin nombres, índices ni colisión entre namespaces. Issues y reason codes tienen orden canónico; SHA-256 excluye nombres no semánticos y orden accidental. Tests y benchmark congelan la entrada, comparan repetición e inversión de colecciones y exigen Evidence idéntica.
- **Siguiente decisión basada en Evidence:** ubicar contratos autoritativos para rejilla, roles, disponibilidad, flujo principal, política y presupuesto antes de diseñar el adaptador; no fabricar defaults ni degradar locks/capacidades para producir soporte artificial.
