...
- **Objetivo e implementación (`DB Safe Merge`):** la migración `066_resource_availability_snapshots.sql` incorpora el default global `default_availability_start/end` a cada `resource_items` y la copia diaria `availability_start/end` a cada `plan_resource_items`. La pareja `NULL/NULL` representa toda la jornada dinámica del plan; no almacena `00:00–23:59` ni materializa `plan.work_start/work_end`. Una ventana explícita exige dos valores canónicos `HH:mm`, con inicio estrictamente anterior al fin y sin cruce de medianoche.
...
- **Migración y backfill:** la migración idempotente `067_workday_space_availability_snapshots.sql` añade defaults y constraints, crea ambas tablas con FKs, unicidad y cascade desde plan, y completa planes existentes con parejas nulas mediante `ON CONFLICT DO NOTHING`, sin `UPDATE` de snapshots. La revisión SQL fue estática; no se aplicó sobre una DB Supabase real y el merge no la aplica automáticamente.
...
- **API contractual:** los namespaces `api.zones.availability` y `api.spaces.availability` tienen paths no intercambiados. Listados, inicialización y updates globales/diarios declaran respuestas Zod compartidas completas; runtime transforma filas Supabase snake_case a camelCase y valida la respuesta compartida, conservando IDs, parejas nullable, `source`, `zoneId` y timestamps.
- **Tests y validación local:** el helper temporal, catálogo global e inicialización pasan `29/29`; contratos compartidos y namespaces pasan `6/6`; TypeScript pasa sin diagnósticos y production build pasa (Vite 2.277 módulos más bundle server). `git diff --check` y la validación SQL estática pasan. `npm test` se ejecutó, pero el entorno local terminó el proceso global después de 248 resultados aprobados y sin fallo registrado, por lo que no se declara completado. La CI alojada cubre instalación, TypeScript y build —no se le atribuye `npm test`— y debe concluir `success` sobre el head final antes del merge.
- **Orden operativo de despliegue:** antes de desplegar este main deben estar aplicadas, en orden: `066_resource_availability_snapshots.sql` y `067_workday_space_availability_snapshots.sql`.
