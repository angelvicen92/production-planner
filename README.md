...
### SPEC09-010 — Auditoría de equidad del orden residual

- **Objetivo y alcance (`DB Safe Merge`):** SPEC09-009 ahorra 510 minutos brutos a siete participantes, introduce 375 minutos brutos de perjuicio a seis y deja una mejora neta de 135; por tanto, la optimización global no implica equidad individual. Esta iteración añade únicamente Evidence pura y trazabilidad: no cambia comparator, tuple, estimador, scoring, matching, forward checking, DFS, reglas hard, configuración, presupuesto ni plan, y el orderer continúa desactivado.
- **State fingerprint y asociación exacta:** cada punto de decisión recibe un SHA-256 diagnóstico sobre depth, slot, patrón, tareas colocadas y comidas reducidas a ID/start/end en orden canónico, e IDs disponibles ordenados. Excluye objetos, runtime, nombres y Evidence mutable, y no reutiliza fingerprints operativos. Un `WeakMap<ExactMainChoiceDescriptor, ResidualOrderingDecision>` asocia por identidad cada descriptor con el objeto de su decisión; la aceptación marca solo ese objeto, incluso entre ramas hermanas a igual profundidad. `acceptedPathDecisions` conserva registros aceptados aunque el diagnóstico general supere su límite, los ordena por profundidad y contiene las 19 selecciones main inequívocas.
- **Contrato del auditor:** compara itinerarios baseline/experimento, deltas, movimientos categorizados por contratos (`MAIN`, `FEEDER`, `STANDALONE`, `ANCHORED_SEGMENT`, `OTHER`) y gaps identificados por participante+tarea anterior+tarea posterior. Publica gaps eliminados, nuevos y compartidos con duración distinta, mecanismo observado, asociación con la decisión, cotas inferiores, errores de cota, visibilidad, agregados y rankings con desempate por ID. El `auditFingerprint` SHA-256 canoniza todo lo anterior, el camino, clasificación y recomendación codificada; es idéntico en repetición y entrada invertida: `4bb0626cbaf73ff05837f3669bbbc634b8c4e4c5d85688a79762f28ac9405616`.
- **Evidence Focal A2:** quedan mapeados los 19 participantes con tareas y cero empeorados `UNMAPPED`. El perjuicio visible en la cota suma 105 minutos (Nela 15 y Pere 90); el perjuicio emergente tras decisiones posteriores suma 270 (Carmen 90, Cristina 30, José Javier 120 y Marta 30). Los errores presencia/gap de la cota en los empeorados son respectivamente Carmen `195/90`, Cristina `60/60`, José Javier `125/125`, Marta `135/105`, Nela `45/−60` y Pere `60/−15` minutos. Son cotas inferiores, no predicciones exactas ni prueba de causalidad única.
- **Mecanismos observados:** Carmen y Nela terminan su última obligación más tarde; Cristina, José Javier, Marta y Pere expanden ambas fronteras. Los gaps compartidos cambian de duración sin crear ni eliminar identidades globales: el mayor aumento individual es José Javier (+120), seguido por Carmen (+90), Pere (+90), Cristina (+30), Marta (+30) y Nela (+15). La auditoría publica además todos los movimientos y a los siete mejorados para evitar un análisis sesgado.
- **Clasificación y recomendación única:** **CASO 2 — PERJUICIO EMERGENTE**, porque todos están mapeados y 270 minutos emergentes dominan los 105 visibles. Recomendación: **no añadir una guarda simple basada en la misma cota; estudiar contexto dinámico, competencia entre obligaciones o evaluación de hojas completas** en otro experimento aislado. Esta asociación con la decisión describe Evidence reproducible, no causalidad absoluta.
- **Protecciones y validación:** baseline exacto `LOW` (85.557 ramas y fingerprints aceptados), baseline de calidad, core aislado, experimento de políticas, orden residual experimental (70.704 ramas y fingerprints corregidos) y benchmark histórico SPEC08 permanecen preservados, hard-valid, deterministas, invariantes e inmutables. Los tests unitarios cubren planes iguales, fronteras, redistribución interna, categorías, cambios de gaps, agregados, rankings, los cinco casos, fingerprints, orden invertido, input inmutable, datos parciales y participantes sin tareas; el benchmark focal ejecuta baseline y experimento dos veces y con entrada invertida.

### SPEC09-011 — Descomposición dinámica del error residual

- **Objetivo y alcance (`DB Safe Merge`):** SPEC09-010 separó 105 minutos de perjuicio visible en la cota y 270 emergentes. Esta iteración añade exclusivamente Evidence posterior al plan: no cambia tuple, comparator, orden, selección del feeder, DFS standalone, scoring, poda, búsqueda, presupuesto, validador ni planificación; el orderer experimental continúa desactivado.
- **Selected trace estática:** la evaluación cacheada por identidad de descriptor contiene conjuntamente la key idéntica y una traza readonly derivada. Registra estado/decisión, operación y obligaciones previas reducidas a ID/intervalo, feeder ideal, transición y, por residual canónica, referencias, recursos ordenados, conteos de starts, dominio vacío y mejor alternativa estática con sus métricas. La enumeración no se duplica: el benchmark conserva 13.944 evaluaciones y 2.748 starts, y las 19 decisiones aceptadas contienen exactamente las 19 trazas seleccionadas; ramas y fingerprints operativos no cambian.
- **Contrafactuales diagnósticos:** la auditoría localiza feeder y residuales reales y compara sus intervalos con el feeder ideal y los best static starts. Evalúa movimientos individuales, residual combinado y escenario estático completo copiando el plan, sin reparar posiciones, ejecutar motor, consumir presupuesto, usar seed ni consolidar resultados. `validatePlan` es la autoridad hard; los blockers canónicos son informativos, no exhaustivos y subordinados a esa validación. Las oportunidades usan `real − contrafactual`, separan hard-válidas de bloqueadas y no deben interpretarse como ahorro realizable cuando el contrafactual es inválido.
- **Evidence Focal A2:** los seis empeorados quedan completos. Sus desplazamientos feeder respecto al ideal son Carmen −105, Cristina −60, José Javier −125, Marta −105, Nela −105 y Pere −75 minutos; los desplazamientos residuales absolutos totales son respectivamente 90, 0, 0, 30, 45 y 60. Hay cuatro targets residuales, cero hard-válidos y cuatro bloqueados. Las oportunidades positivas bloqueadas agregadas son feeder 395, residual combinada 210 y estática completa 605 minutos; las tres oportunidades positivas hard-válidas agregadas son cero. Predominan indisponibilidad del espacio vocal, solape de coach y transición de participante en feeders, y competencia/ transición de recursos —además de un solape de participante— en residuales.
- **Clasificación derivada:** los seis empeorados son `STATIC_TARGETS_BLOCKED`; el agregado es **CASO 4 — OBJETIVOS ESTÁTICOS BLOQUEADOS**. Recomendación única codificada: **no reordenar localmente; estudiar competencia dinámica o evaluación de hojas completas**. El fingerprint dinámico `23fb148849da5795388cf145aca80a80a77fea3587c35d4792314f92c7f40ec8` coincide en repetición e input invertido.
- **Límites y protecciones:** los contrafactuales se solapan, no forman una descomposición aditiva, miden mecanismos diagnósticos y no demuestran causalidad única ni constituyen planes alternativos aceptados. Se preservan baseline exacto `LOW` (85.557 ramas), baseline de calidad, core aislado, experimento de políticas, experimento residual (70.704 ramas), fingerprint de equidad SPEC09-010 `4bb0626cbaf73ff05837f3669bbbc634b8c4e4c5d85688a79762f28ac9405616` y benchmark histórico SPEC08; no hay DB, RLS, UI, routing, endpoints ni `PlanResult`.

### SPEC09-012 — Exploración standalone con core fijo

- **Objetivo y Evidence de partida (`DB Safe Merge`):** SPEC09-011 encontró seis empeorados completos, cuatro best static targets residuales y cero movimientos locales positivos hard-válidos; los cuatro targets estaban bloqueados y las oportunidades positivas bloqueadas sumaban 395 minutos en feeder, 210 en residuales y 605 en el escenario estático completo (`23fb148849da5795388cf145aca80a80a77fea3587c35d4792314f92c7f40ec8`). Un movimiento local mantiene inmóviles las demás tareas y, por tanto, no responde si otra asignación conjunta puede liberar recursos. Esta iteración observa completitudes completas de las nueve standalone sin reconstruir ni mover el core.
- **Core fijado y primera hoja protegida:** el benchmark deriva canónicamente IDs `main`, `vocal` y segmentos anchored, y extrae del plan residual las 44 tareas core y sus comidas sin reordenarlas. El core fijo conserva fingerprint `44f10279aa01fa7628c01962e9fbdd819d69486ae11df4fe4851de946600f07f`. La ejecución default `ACCEPT` reproduce la primera completitud actual con full fingerprint `38309867fb51dcb14515d152035b7076a4738cac04d3d8cea721ec7be0749fa8`, quality fingerprint `13a87e0d9b6983c18ca5a0162785058b67b10f8ea65d46644463f49063791c75`, hard validity, cero pendientes y selection order `04, 02, 07, 05, 10, 09, 08, 11, 12` sobre los IDs canónicos `reality-operation-*`. Consume 20.439 ramas (20.438 start checks), exactamente como primera búsqueda fixed-core; no sustituye el plan aceptado.
- **Continuación interna y autoridad hard:** `runExactStandaloneSearchForFixedCore` usa la misma selección dinámica, orden de starts, `scoreAuxiliaryTask`, `canPlaceTask` y DFS. Su callback opcional recibe copias readonly únicamente después de una hoja completa validada y retorna `ACCEPT`, `CONTINUE` o `BUDGET_EXHAUSTED`; no recibe ledger, no inyecta starts, no poda antes de hoja y su evaluación no cuenta como ramas. Default sigue aceptando la primera hoja. `CONTINUE` descarta atómicamente la hoja observada y sigue la DFS; ninguna alternativa se publica ni activa.
- **Ledger y contabilidad:** cada experimento fixed-core crea un ledger nuevo con el límite contractual exacto de 300.000 ramas y lo dedica solo a standalone. Estas ramas no son comparables directamente con las 70.704 del plan completo residual: aquellas incluyen construcción core, forward checking y primera completitud. La exploración consume exactamente 300.000 ramas, 299.905 start checks, 831 selecciones, 907 podas sin alternativa, 1.826 backtracks, profundidad máxima 9 y alcanza 95 hojas completas: 95 hard-válidas, cero inválidas, 95 fingerprints únicos y cero duplicados. `stopReason = BUDGET_EXHAUSTED`; no hubo agotamiento natural.
- **Evaluación externa y memoria:** cada hoja hard-válida se evalúa fuera de la búsqueda con métricas existentes contra el baseline exacto y la primera completitud. Se conservan contadores, fingerprints, métricas compactas, starts/selection order de representantes y una frontera de Pareto de tamaño real 16; no se guardan todos los planes ni se usa calidad para ordenar o podar. El fingerprint canónico del experimento es `982845ef913a3a03f02ec175261ea9b2207e67396345c9d758fdce319831a054`, idéntico en repetición e input invertido.
- **Dominancias y representantes observados:** no aparece operational dominance ni baseline-safe (0/0). Aparecen 63 completitudes harm-reducing y 63 equity-dominant. El menor gross harm observado es 300 minutos y el menor maximum individual harm 120; el representante `02012d3265cb6525aa1bf333495174a19cdd9e4cd59d4b716b9b9717e2aca79f` obtiene idle total 2.420, máximo idle 350 y gap máximo 225. El menor idle total observado es 2.390 (`03dc43bc73d537119746188e74aa8af08d3f034ba7e83242962ac03c1816e47c`, máximo idle 425, gap 225). El menor máximo idle y el menor gap son respectivamente 350 y 225, ambos representados lexicográficamente por `02012d…`; ese mismo punto es el representante lexicográfico de la frontera. Son mejores hojas **observadas**, no óptimos demostrados, porque el presupuesto se agotó.
- **Clasificación y recomendación única:** **CASO 2 — MEJORA DE EQUIDAD CON TRADEOFF CONTROLADO**. Existe margen conjunto dentro de las standalone con el mismo core: diseñar después un experimento de ordenación standalone que priorice esta Evidence sin podar; no adoptar todavía ninguna hoja y no modificar la construcción core. Aunque el presupuesto limita cobertura, la aparición de Evidence de caso 2 permite esta clasificación; no se afirma exhaustividad ni ausencia de alternativas mejores.
- **Alcance, protecciones y resultado operativo:** no cambian scoring, orden productivo standalone, matching, forward checking, core search, comparator/tuple residual, política `LOW`, presupuesto, hard rules, validadores, contratos, DB/RLS, UI, routing ni `PlanResult`. Se preservan core aislado (47.482 ramas), baseline exacto (85.557), baseline de calidad, experimento de políticas, residual (70.704), auditorías SPEC09-010/011 y benchmark histórico SPEC08. Repetición e inversión demuestran determinismo, invariancia e input/core/comidas inmutables. La ruta productiva comparte aún un único ledger core+standalone, acepta la primera hoja y conserva sus ramas/fingerprints; ninguna hoja alternativa quedó activada.
