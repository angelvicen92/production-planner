...
### FOCAL-A2-010R1 — Consolidación de Anchored Accompaniment

- **Objetivo:** consolidar, sin ampliar capacidades ni presupuestos, el soporte exclusivo de anchors `main` introducido por FOCAL-A2-010.
- **Implementación:** la materialización devuelve un resultado tipado atómico con motivos ordenados; el preflight valida nesting, reutilización, participantes, referencias y la intersección real de recursos continuos. El coste operativo histórico permanece separado de la viabilidad futura: cero alternativas poda de forma explícita y los conteos sólo desempatan lexicográficamente después del score.
- **Validación y Evidence:** una evaluación canónica alimenta continuidad, ventanas y completitud por contrato; las posiciones rechazadas publican conteos derivados por reason code. El benchmark directo exige una atestación tipada y queda en `FOCAL_A2_SPEC08_VALIDATION_REQUIRED`; `validate-focal-a2-010r1.sh` ejecuta las comprobaciones obligatorias antes de emitirla y publica V4 atómicamente.
- **Resultado operativo:** la Evidence fresca FOCAL-A2-010R1 demuestra 53 tareas hard-valid (19 main y 19 vocal), tres anchors, seis segmentos, tres operaciones ancladas y nueve standalone: 12 operaciones itinerantes completas y 375 minutos productivos, sin parciales.
- **Alcance y exclusiones:** no se añaden anchors no-main, integración de producción, DB, RLS, migraciones ni seed humana. Planner Next sigue aislado de producción. La deuda residual es ampliar los fixtures sintéticos de transición y los poison controls a un harness reusable, sin trasladar esas comprobaciones al motor.
