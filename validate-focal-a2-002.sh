#!/usr/bin/env bash
set -uo pipefail
artifact=planner-next-focal-a2-feeder-closure-v1.json
legacy=planner-next-focal-a2-baseline-v1.json
failed=planner-next-focal-a2-feeder-closure-v1.failed.json
tmp1=$(mktemp); tmp2=$(mktemp); trap 'rm -f "$tmp1" "$tmp2"' EXIT
fail(){ printf '{"version":"planner-next-focal-a2-feeder-closure-v1","accepted":false,"reason":"%s"}\n' "$1" > "$failed"; exit 1; }
