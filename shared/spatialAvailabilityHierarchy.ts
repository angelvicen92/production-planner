...
export type SpatialAvailabilityReason =
  ...
  | "INVALID_SPACE_TIME_ORDER" | "SPACE_OUTSIDE_ZONE";
export type SpatialAvailabilityResult =
  | Readonly<{ valid: true; workDay: AvailabilityWindow; zone: Readonly<{ effectiveWindow: AvailabilityWindow; mode: "INHERITED_WORKDAY" | "EXPLICIT" }>; space: Readonly<{ effectiveWindow: AvailabilityWindow; mode: "INHERITED_ZONE" | "EXPLICIT" }> }>
...
const invalid = (reason: SpatialAvailabilityReason, workDay: AvailabilityWindow | null): SpatialAvailabilityResult => Object.freeze({ valid: false, reason, workDay });

function validateChildPair(value: NullableAvailabilityWindow, level: "ZONE" | "SPACE"): { inherited: true } | { inherited: false; window: AvailabilityWindow } | { reason: SpatialAvailabilityReason } {
  ...
  if (typeof value.start !== "string" || typeof value.end !== "string" || !TIME.test(value.start) || !TIME.test(value.end)) return { reason: `INVALID_${level}_TIME_FORMAT` };
...
export function resolveEffectiveSpaceAvailabilityHierarchy(input: SpatialAvailabilityInput): SpatialAvailabilityResult {
  ...
  if (typeof work?.start !== "string" || typeof work?.end !== "string" || !TIME.test(work.start) || !TIME.test(work.end) || work.start >= work.end) return invalid("INVALID_WORKDAY", null);
  ...
  return Object.freeze({ valid: true, workDay, zone: Object.freeze({ effectiveWindow: zoneWindow, mode: zonePair.inherited ? "INHERITED_WORKDAY" : "EXPLICIT" }), space: Object.freeze({ effectiveWindow: spaceWindow, mode: spacePair.inherited ? "INHERITED_ZONE" : "EXPLICIT" }) });
}
