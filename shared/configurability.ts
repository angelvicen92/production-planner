...
export const configurabilityRegistry: readonly ConfigurabilityCapability[] = Object.freeze([
  ...
  productive({...base("OPTIMIZATION","Optimización","Reglas de planificación","PLANNER","niveles/pesos",["GENERAL","DAY_SNAPSHOT","DAY_OVERRIDE"]),generalSource:"optimizer_settings",dailyPersistence:"plan_optimizer_snapshots effective + stable baseline + explicit source",override:"Typed edit/restore/refresh operations",generalUi:"Optimización",dayUi:"Effective configuration review with Restore inherited",engineInputProjection:"optimizer snapshot + optimizerSnapshotSource",preflight:"snapshot normalization and daily reference validation",engineConsumer:"motores productivos",validator:"optimizer snapshot + DB constraints",evidenceFingerprint:"optimizerSnapshotFingerprint + plan_config_revisions",permissionsRls:"snapshot tables revoked from clients + service-only atomic RPC",tests:["server/planOptimizerSnapshot.spec.ts","server/planOptimizerSnapshotRefreshPreview.spec.ts","server/dayConfiguration.spec.ts"]}),
