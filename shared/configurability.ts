...
export const configurabilityRegistry: readonly ConfigurabilityCapability[] = Object.freeze([
  productive({...base("WORKDAY_WINDOW","Horario de la jornada","Jornada y comidas","PLAN","HH:mm",["GENERAL","DAY_SNAPSHOT","DAY_OVERRIDE"]),generalSource:"program_settings.default_work_start/default_work_end",dailyPersistence:"plans.work_start/work_end",override:"Formulario de creación y PATCH plan",generalUi:"Ajustes generales del programa > Horario habitual de la jornada",dayUi:"Creación y edición del día",engineInputProjection:"workDay",preflight:"buildEngineInput",engineConsumer:"V3/V4",validator:"insertPlanSchema/updatePlanSchema",evidenceFingerprint:"effective revision",tests:["client/src/components/create-plan-dialog.test.ts","server/storage.createPlanDefaults.spec.ts","engine/buildInput.operationalSnapshotGate.spec.ts"]}),
  ...
  productive({...base("GLOBAL_MEAL_BREAK","Pausa global de comida","Jornada y comidas","PLAN","HH:mm",["GENERAL","DAY_SNAPSHOT","DAY_OVERRIDE"]),generalSource:"program_settings",dailyPersistence:"plans meal window",override:"PATCH plan",generalUi:"Configuración general",engineInputProjection:"meal/mealMode",preflight:"buildEngineInput",engineConsumer:"V3/V4",validator:"plan schema",evidenceFingerprint:"effective revision",tests:["engine/buildInput.operationalSnapshotGate.spec.ts"]}),
  ...
  gap("BLOCKED",["daily_tasks participa en la proyección y tiene RLS desactivada."],{...base("PARTICIPANTS","Participantes y disponibilidad","Participantes","PARTICIPANT","personas / HH:mm",["DAY_SNAPSHOT","INSTANCE_OVERRIDE"]),dailyPersistence:"contestants + daily_tasks",engineInputProjection:"contestantAvailabilityById/tasks",preflight:"buildEngineInput",engineConsumer:"motores productivos",validator:"schemas existentes",permissionsRls:"BLOCKED: daily_tasks RLS desactivada",tests:["engine/buildInput.spec.ts"]}),
  ...
  gap("PARTIAL",["buildEngineInput no proyecta itinerantTeamAvailability desde autoridades productivas."],{...base("ITINERANT_UNITS","Unidades itinerantes","Recursos","ITINERANT_UNIT","unidades / HH:mm",["GENERAL","DAY_SNAPSHOT","INSTANCE_OVERRIDE"]),engineInputProjection:"tasks únicamente; falta itinerantTeamAvailability",tests:["engine/buildInput.spec.ts"]}),
  ...
  productive({...base("OPTIMIZATION","Optimización","Reglas de planificación","PLANNER","niveles/pesos",["GENERAL","DAY_SNAPSHOT","DAY_OVERRIDE"]),generalSource:"optimizer_settings",dailyPersistence:"plan_optimizer_snapshots",override:"DAY_OVERRIDE",generalUi:"Optimización",engineInputProjection:"optimizer snapshot",preflight:"snapshot normalization",engineConsumer:"motores productivos",validator:"optimizer snapshot",evidenceFingerprint:"optimizerSnapshotFingerprint",tests:["server/planOptimizerSnapshot.spec.ts"]}),
